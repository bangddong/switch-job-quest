...
| `brainstorming` | 코드 작성 전 설계 및 접근법 도출 |
| `writing-plans` | 구현 계획 문서 작성 |
| `executing-plans` | 작성된 계획 태스크별 실행 |
| `tdd` | RED → GREEN → REFACTOR 사이클 |
| `systematic-debugging` | 근본 원인 파악 후 수정 |
| `verification-before-completion` | 완료 선언 전 증거 기반 검증 |
...
> 이 섹션은 auto-compaction과 수동 `/compact` 모두에 자동 주입된다. 아키텍처 규칙과 작업 상태는 여기서 보호한다.
...
`## Compact Instructions` 섹션이 auto-compaction 시 자동 주입되므로 아키텍처 규칙과 작업 상태는 자동 보호된다.
수동 `/compact`는 **token 절약 + 다음 작업 힌트 주입** 목적으로, 아래 상황에서 선택적으로 실행한다.

**권장 실행 시점:**
- 한 세션에서 PR 3개 이상 완료된 경우
- 긴 디버깅/탐색 세션 이후 새 작업 시작 전
- 복잡한 신규 기능 시작 전 context 정리

**실행 시 템플릿:**
```
# PR 완료 후 정리
/compact Keep: branch=<현재 브랜치명>, PR #<번호> 머지 완료, 다음 작업=<항목>

# 디버깅 세션 후
/compact Keep: branch=<현재 브랜치명>, 원인=<발견한 원인>, 수정 방향=<적용할 방향>

# 새 기능 시작 전
/compact Keep: branch=<현재 브랜치명>, 다음 기능=<기능명>, 관련 파일=<파일 경로>
```
...
| 상황 | 적용 스킬 |
|------|-----------|
| 새 기능 / 동작 변경 시작 | `brainstorming` → `writing-plans` |
| 구현 계획 실행 | `executing-plans` |
| 코드 작성 전 | `tdd` |
| 버그 발생 / 테스트 실패 | `systematic-debugging` |
| 완료 선언 전 | `verification-before-completion` |
| 독립적인 작업 2개+ 동시 존재 | `dispatching-parallel-agents` |

관련 스킬이 1%라도 있다고 판단되면 **무조건 해당 스킬을 먼저 읽고 시작한다.**
"이건 간단하니까 스킬 생략" → 경고 신호. 스킬을 먼저 확인하라.
