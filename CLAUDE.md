...
- AWS 콘솔 등 GUI 단계는 브라우저 도구로 직접 캡처 → `docs/images/eks-tutorial/step-XX-<설명>.png` 저장,
  튜토리얼 해당 단계에서 참조. 로그인 필요하면 사용자에게 요청.
- 터미널 출력은 이미지 금지 — **텍스트 코드블록**. 캡처는 텍스트로 못 담는 GUI 화면만.
- 브라우저 캡처 불가 화면은 파일명과 함께 사용자에게 캡처 요청 + 문서에 `<!-- TODO: step-XX 캡처 -->` 자리표시.
