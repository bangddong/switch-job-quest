...
| 기록 대상 | 목적지 | 비고 |
|---|---|---|
| 결정·에러·해결 (EKS 트랙) | `docs/eks-migration-log.md` | 이벤트 즉시 append |
| 미룬 **QA** 지적 | `.claude/review-ledger.md` | 🔒 `assert-qa-run.sh` 가 등재를 강제 |
| **Blindspot 이 찾았지만 이번에 안 하는 것** | **계획서의 해당 항목 절** | 원장·일지 둘 다 아니다 (아래) |
| 사용자 직접 실행 필요 | `.claude/TASKS.md` | 단계별 명령 포함 |
| 파생 불가능한 상태·결정 | `.claude/CONTEXT.md` | 브랜치·PR 번호는 **금지** |

> 🔴 **Blindspot 발견에 목적지가 없어서 실제로 유실됐다 (2026-09-15).** 항목 3 의 17건 중
> **U-8 하나만** 일지에서 빠졌고 아무도 몰랐다. 원장은 QA 지적 전용(`<브랜치>/<F-ID>` 형식을
> 훅이 검사한다)이라 받지 않고, 일지는 시간순이라 *"다음에 뭘 해야 하나"* 로 **검색되지 않는다.**
> → **계획서에 적는다.** 같은 세션 QA 가 독립적으로 `F-2`(원본 계획 문서 미갱신)를 지적했다 —
> 두 경로가 같은 곳을 가리켰으므로 개인 실수가 아니라 구조적 구멍이다.
...
| **"가드에 구멍이 있으니 막자"** | **하네스 동결 규칙**(아래) — 제품 작업이 막힌 증거가 없으면 원장에만 적는다 |
| **"지금은 배선 못 하지만 코드는 남겨두자"** | 지워라. 한 번도 안 돈 코드 214줄 + 원장 항목을 그렇게 이고 갔다(L-24). 되살릴 건 git 이력에 있다 |
...
> **2026-09-18 이관** — `.claude/CONTEXT.md` 가 1017줄로 80줄 규칙을 12.7배 위반해 재배치했다(원장 L-10).
> 이 항목은 **특정 트랙이 아니라 레포 전체의 작업 방식**에 관한 것이라 여기가 맞다.

## 하네스 동결 규칙 (2026-08-17)

**`.claude/`·`.github/` 작업은 "실제 제품 작업이 이것 때문에 막혔다"는 증거가 있을 때만 한다.**
증거가 없으면 원장에 적고 넘어간다. 가드의 이론적 구멍은 착수 사유가 아니다.
...
마지막 제품 *기능*은 **#361 (08-06)**. 그 뒤로 `feat` 0건인 채 하네스가 자기 사고를 수습하는
사슬이 이어졌다: `#377`(훅 12개 mode 644 수리) → 되살아난 가드가 위임을 통째로 차단 →
`#383`(수습) → 그 수습이 만든 오탐·fail-open → `#384`(수습 + 회귀 스위트 152건).
**#384에서 처리한 지적 20건 중 4건은 수정 과정에서 새로 만든 것이다.**

- 가드를 **조이는** 변경은 특히 위험하다 — 조일 때마다 새 우회가 열렸다(4라운드 실측, L-22).
- 이 레포는 **이직 준비용**이다. 평가 대상은 `be/`·`fe/`이지 `.claude/`의 가드 스위트가 아니다.
- 해제 조건: 제품 작업이 실제로 차단되면 그 PR 안에서 최소한으로 고친다.

### 가드를 실제로 고칠 때의 규약 3줄 (2026-09-18 이관 — 회고 블록에서 건짐)

원래 `.claude/CONTEXT.md` 백로그의 긴 회고에 묻혀 있었다. 회고는 폐기해도 이 셋은 남긴다.

1. **차단 케이스와 오탐 케이스를 함께 돌린다** — 한쪽만 보면 반대 방향으로 깨진다.
2. **가드를 고칠 때 회귀 스위트를 함께 고친다** — 스위트가 낡으면 가드보다 먼저 거짓말한다.
3. 🔴 **꺼진 가드를 켜는 것은 무해하지 않다** — #377 에서 mode 644 훅 12개를 살렸더니
   되살아난 가드가 위임을 통째로 차단했다. *"나머지도 같은 과잉 차단을 갖고 있을 수 있다."*

### 해제 조건 ② — **사용자가 먼저 꺼낸 경우** (2026-09-16, 원장 L-54 종결)

원래 해제 조건은 위 하나뿐이라, 레포 소유자가 자기 레포의 규칙을 지시로 넘을 수 없었다.
그래서 같은 지적이 매번 재생산됐다(L-54: QA 3라운드 연속 F-7).

| 조건 | |
|---|---|
| 🔴 **누가 먼저 꺼냈나** | **사용자가 먼저** 말한 경우만 해제된다. *"이거 고칠까요?"* 로 **내가 유도해 받은 승인은 해제 사유가 아니다** — 물으면 대개 승인이 오므로, 유도를 허용하면 이 조건은 그 자리에서 무력해진다 |
| 착수 **전** | **실측으로 범위를 좁힌다.** *"노후화된 것 같다"* 는 착수 사유가 아니다. 규칙을 **실제로 위반한 항목만** 남기고 나머지는 원장행 |
| 범위 | **문서 변경으로 한정.** 스크립트·훅 신설 금지 — 조일 때마다 새 우회가 열렸다(L-22, 4라운드 실측) |
| 기록 | 원장 등재 |

> **이 조건을 만든 날의 실행 사례**: 사용자가 *"지침이 노후화된 것 같다"* 고 **먼저** 물었다.
> 실측 결과 — 동결 규칙 이후 하네스 *로직* 커밋은 11/40 이고 **전부 실제 작업에 딸려온 것**
> (자기수습 사슬 0건), 경로·에이전트 참조 **깨진 것 0건**, 실제 규칙 위반은 **1건**
> (`CONTEXT.md` 1017줄 / 규칙 80줄 — 이미 원장 **L-10**). 죽은 스킬 3종은 상시 비용이 0 이라 원장행.
> 🔑 **질문 하나가 전면 개정이 아니라 1건으로 수렴했다.** 수렴시키는 것이 이 조건의 요점이다.
