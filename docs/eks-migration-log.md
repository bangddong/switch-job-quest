...
>
> 🔑 **2026-09-09 교차검증 — 산정 모델이 맞았고, 빠진 항목이 무엇인지도 드러났다.**
> ```
> 09-08 실측 (budget ActualSpend)        $2.453
> + 09-09 세션 산정 (31분 34초)           $0.0716
> + 영속 EBS 1일치 ($0.91/월 ÷ 30)        $0.030
> ─────────────────────────────────────────────
>   산정 합계                             $2.555
>   실측 (09-09 조회)                     $2.557    → 차이 $0.002
> ```
> 🔴 **처음엔 $2.6710 으로 적었다가 QA(F-2·F-6)에 걸렸다.** 두 가지를 틀렸다:
> ① *"마지막 실측은 08-12"* 라고 단정했는데 **09-08 에 이미 `ActualSpend $2.453` 이 있었다**
>    (이 파일 안에 있었는데 못 찾고 단정했다). ② 그 칸은 **조회값 전용**인데 산정치를 넣었다 —
>    바로 아래에 인용한 규칙을 **인용하면서 어겼다.**
> 🔑 그리고 조회해보니 **산정에서 빠진 것은 세션 비용이 아니라 영속 EBS 의 일일 누적**이었다.
> 세션 단위로만 더하면 *"안 쓴 날의 비용"* 이 통째로 사라진다 — 영속 리소스는 세션과 무관하게 쌓인다.
> → **다음 세션 시작 시 `aws budgets describe-budgets` 로 한 번 조회해 대조할 것**(무료, 10초).
>
...
- `[비용]` **명시적 `date` 스탬프로 계산한다 — 파일 mtime 을 쓰지 않는다**(09-08 에 그걸로
  ...
  **누적 실측 $2.557 / $200 (1.28%)** (세션 후 `aws budgets describe-budgets` 조회).
  ⚠️ 산정 합계는 $2.555 였고 실측과 **$0.002 차이** — 상단 교차검증 블록 참조.
