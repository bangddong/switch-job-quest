...
| 🔴 **영속 EBS** `vol-041e5a6705f7b4975` 10GiB gp3 | **2026-09-13** (재생성) | **없음 (destroy해도 남음)** | **≈ $0.91/월** | **$2.936 소진 / $200 (1.47%)** — 2026-09-15 `aws budgets describe-budgets` **실측**<br>CE 서비스별 합계 `$2.9177` 로 **독립 일치**(24h 지연 오차 내)<br>~~$2.823 (2026-09-13)~~ |
...
| 🔴 **`AWS Cost Explorer` API 호출** (리소스가 아니라 **행위**) | 2026-07 | — | **$0.01 / `get-cost-and-usage` 요청** | **누적 $0.190 = 전체의 6.5%** — #417 세션 전체($0.0848)의 2배. 2026-09-15 등재 |

> 📌 **표에는 조회값만 넣는다** (08-12 확정 규칙, 아래 참조). 산정치는 표 밖인 여기서 대조한다.
>
> ⚠️ **09-09 시점에 산정↔실측 대조는 하지 않는다 — 지금은 할 수 없다.**
> Budgets/CE 는 최대 **24h 지연**되므로(아래 캐비앗) 당일·전일 수치는 과소계상이고,
> 그 값으로 산정 모델을 검증하면 **lag 을 정확도로 착각한다.**
> → 대조는 **다음 세션 시작 시** 한다: `aws budgets describe-budgets`.
>
> 🔴 **무료가 아니다 (2026-09-15 정정).** 위 줄에 *"무료, 10초"* 라고 적혀 있었는데
> `aws budgets describe-budgets` 는 무료가 맞지만 **서비스별 내역을 보려면 `ce get-cost-and-usage`
> 가 필요하고 그건 `$0.01/요청`** 이다. 09-15 대조에서 3회 = **$0.03** 을 썼다 — 그날 조회로
> 확인한 9월 ELB 총액($0.0225)보다 크다. 🔑 ***재는 행위가 재려는 대상보다 비쌀 수 있다.***
> → **세션마다 돌리지 말고**, 대조가 필요할 때 **한 번에 넓은 기간·그룹으로** 뜬다.
>
> 🔴 **이 자리에 교차검증 산술을 적었다가 QA F-8 에 걸려 지웠다 (2026-09-09).**
> 09-08 세션 실비용 **$0.1300 을 식에서 빠뜨렸는데도** 오차가 $0.002 로 나왔다 —
> 그건 모델이 맞다는 증거가 아니라 **두 실측치가 비슷하게 lag 돼 있다는 신호**였다.
> 게다가 이 파일이 30줄 아래에서 *"당일·전일 수치는 항상 과소계상으로 보라"* 고
> 이미 경고하고 있었다. **내 문서가 내 결론을 반박하고 있었다.**
> 🔑 그 앞 라운드의 오류(*"마지막 실측은 08-12"* — 09-08 에 $2.453 이 있었다)를 고치려다
> 새 오류를 만든 것이다. **이 세션에서 같은 형태가 네 번 반복됐다:
> 숫자는 맞는데 그 숫자가 답하는 질문이 내가 던진 질문이 아니었다.**
>
...
**비용**: 노드 1대 = EC2 `$0.0208/h` + 공인 IPv4 `$0.005/h` ~~= `$0.0258/h`~~ → 🔴 **루트 EBS 포함 `$0.0283/h`** (09-04 정정, 아래 참조).
~~50분 세션 2대 $0.128 vs 3대 $0.150 → **차액 $0.021.**~~
🔴 **정정 (09-04, 루트 EBS 포함)**: 2대 $0.1318 vs 3대 $0.1554 → **차액 $0.0236.**
...
### [비용] 세션 종결 — destroy 10:16:52, **29 destroyed**, 고아 0

| 구간 | 시각 | 소요 |
|---|---|---|
| apply | 09:43:46 ~ 09:52:49 | **9m03s** (컨트롤플레인 6m11s · 노드그룹 1m48s · 애드온 4종) |
| ESO + 시크릿 + PV/PVC + postgres | ~09:53 ~ 09:57 | ~4분 |
| 측정 ① (406Mi 분리) | ~09:55 | 1분 |
| 앱 3개 배포 | 09:57:57 ~ 09:58:40 | ~2분 (전부 1회 rollout 성공) |
| 측정 ② + e2e | 10:00 ~ 10:03 | 3분 (400 두 번 포함) |
| C-5 차단/허용 쌍 | 10:03 ~ 10:06 | 3분 |
| destroy | 10:07:08 ~ 10:16:52 | **9m44s** |

~~**총 과금 33m06s × $0.1790/h ≈ $0.0987.**~~ 🔴 **정정 (퀴즈 Q4, 아래 참조)** —
루트 EBS 를 빠뜨렸다. **33m06s × $0.1865/h ≈ $0.1029.** 누적 $1.9043 → **$2.0072 / $200 (1.00%)**.

📌 **예상 65분 / $0.19 → 실제 33분 / $0.099.** 절반이다. 사전 점검을 $0 구간에서 전부 끝내고
(이미지 재빌드·plan·산술) 과금 구간에는 **실행만** 남긴 것이 컸다.
🔑 08-31 은 반대였다 — 과금 중에 진단(Free Tier 거부 원인 추적)을 했고 51분을 썼다.
**"과금 구간에서 무엇을 하지 않을지"가 비용을 결정한다.**

**고아 전수 = 전 항목 0건**: tofu state 0 · EKS 0 · ELB 0 · NAT 0 · EC2 0 · 고아 EBS 0 ·
RDS 인스턴스/수동스냅샷/자동스냅샷 0 · Secrets Manager 0(삭제대기 포함).

**영속 인벤토리 = 일치**: `vol-0518b6d0dcd2b0d70` 10 GiB `ap-northeast-2a` `available` **1개**
— `PERSISTENT-RESOURCES.md` 의 *"정확히 1개"* 와 부합.

### [메모] Stage C 블로커 상태 갱신

| 블로커 | 상태 |
|---|---|
| **C-4** (노드 용량) | ✅ **해소** — 노드 3대로 3서비스 기동 확인. 실측 여유 1689Mi |
| **C-5** (NetworkPolicy 가 실제로 막는가) | ✅ **해소** — 차단/허용 쌍 통과 |
| **C-3** (ECR 이미지) | ✅ 해소 — 3서비스 동일 태그·arm64 |
| **C-8** (`DEVQUEST_AI_HTTP_BASE_URL`) | ✅ 해소 — e2e 가 실제로 ai-api 를 탔다 |
| 406Mi 분리 (L-43) | ✅ **측정 완료** — DS 104Mi/노드 · Deploy 302Mi |
| `requests: 512Mi` 근거 | 🔴 **측정했고 과다 확인(2배)** — 부하 상태 재측정이 다음 과제 |

### [메모] 🔴 **정정이 네 번 연속 불완전했다 — 공통 원인은 "세기 전에 완결을 썼다"**

퀴즈 Q4 에서 노드 단가를 `$0.0258 → $0.0283/h` 로 정정하면서, **그 단가에서 파생된**
*"50분 세션 차액 $0.021"* 은 그대로 뒀다. 같은 문서 안에 두 값이 공존했다(QA F-1).
그걸 고치며 *"둘 다 세었으므로 완결"* 이라 적었는데 그것도 거짓이었다(재QA F-3).
그 정정을 또 산문만 고치고 **바로 위 표를 방치**해 모순되는 두 표가 공존했다(재QA F-7).

**최종 집계 — 같은 값이 있던 곳은 총 6곳이다** (각 지점을 QA 가 커밋 diff 로 개별 대조):
`plan.md:333` · `variables.tf:89` · `eks-migration-log.md:2328` · `CONTEXT.md:229` ·
`CONTEXT.md:241` · `review-ledger.md:80`(L-43)

| 회차 | 무엇을 고쳤나 | 무엇을 빠뜨렸나 |
|---|---|---|
| 08-31 "정정" | medium 단가 실측 | IPv4 하나 더 |
| 09-03 "재정정" (#407) | IPv4 포함 | 루트 EBS |
| 09-04 퀴즈 Q4 | 루트 EBS 포함 | **파생값(차액)** |
| 09-04 QA F-1 | 파생값 4곳 (QA 3 + 자체 1) | **`CONTEXT.md:241` · 원장 L-43** |
| 09-04 재QA F-3 | 나머지 2곳 → **6곳 완료** | 이 절의 **표**(산문만 고쳤다) |
| 09-04 재QA F-7 | 표를 하나로 합침 (이 표) | — |

🔑 **정정은 "숫자를 바꾼 곳" 에서 멈춘다.** 그 숫자로 **계산된** 값은 다른 문장에 있어
같은 grep 에 안 걸리고, **같은 내용의 표**는 산문을 고칠 때 시야 밖에 있다.

🔑 **네 번 연속으로 "이번엔 전부" 라고 적었고 네 번 다 틀렸다.** 원인은 하나다 —
**세어보기 전에 완결을 문장으로 먼저 썼다.**

> 👉 **규칙: *"전부"·"완결"* 이라는 단어를 쓰려면 그 앞줄에 열거 결과가 있어야 한다.**
> 마지막 회차에서야 그렇게 했다 — 취소선 밖 11건을 하나씩 분류해 나열한 뒤 판정했고,
...
### [해결] QA 가 판정 규칙의 하한값을 무너뜨렸다 — **근거도 숫자도 틀렸다**

리뷰 F-1(MEDIUM). 내가 적었던 하한 근거:

> *"하한 384Mi: 그 아래면 MaxHeap 136Mi 인데 prod 실측 힙 커밋이 117MB 다.
>  여유가 19MB 뿐이라 Metaspace 데스 스파이럴(2026-07-14 사고) 재현 위험이 있다."*

QA 지적 두 가지 다 맞다:
① Fly(힙 상한 179Mi·실사용자 트래픽)의 수치를 EKS(384Mi 가정·부하테스트)에 **환경을 건너뛰고** 붙였다
② 2026-07-14 사고는 **`-XX:MaxMetaspaceSize=160m` 고정값**에서 났다. 그 값은 `limits` 와 무관하게
   항상 160m 이므로 **힙 여유 논증과 인과관계가 없다.**

그리고 검산해보니 **숫자도 틀렸다.** JVM 예약 상한이 컨테이너 상한을 넘으면 안 된다는 부등식:
...
|---|---|---|---|
| **384Mi (내가 쓴 값)** | 134Mi | 390Mi | **−6Mi** 🔴 |
| 394Mi | 137Mi | 393Mi | 1Mi |
| **448Mi (정정)** | 156Mi | 412Mi | **36Mi** |
| 900Mi (현행) | 315Mi | 571Mi | 329Mi |

→ 하한을 **448Mi** 로 올리고 근거를 **구조적 부등식**으로 교체. 원문은 취소선으로 보존.

🔑 **QA 의 지적이 정정 후 반대 방향의 진짜 위험으로 되살아났다.** *"Metaspace 는 고정값이라
`limits` 와 무관"* 이 맞다면, **`limits` 를 조일수록 Metaspace 고정비의 비중이 커진다** —
900Mi 에서 18%, 448Mi 에서 **36%**. prod 는 128m 으로 줄였다가 실제로 Metaspace OOME 를 맞았다
(= 커밋이 128MB 를 넘었다는 실측). **`limits` 를 조이면 조여지는 것은 힙뿐이다.**

🔑 **이 세션 두 번째로 "근거를 세우기 전에 결론 숫자를 먼저 썼다".** 앞선 것은 거짓 문법 통과였고
이번엔 하한값이다. 09-04 일지가 남긴 규칙(*"'전부'를 쓰려면 앞줄에 열거 결과가 있어야 한다"*)의
같은 병이다 — **검산 결과가 문장보다 먼저 있어야 한다.**

📌 부수: QA 가 `SPRING_APPLICATION_JSON` 키를 코드로 확정해줬다 —
...
### [해결] 🎉 부하 측정 완료 — 59,889 요청 전부 200

`apply` 11:07:33 → 3노드 Ready(allocatable **1365Mi**, 08-31 실측과 일치) → ESO 3종 `SecretSynced=True`
→ 영속 볼륨 생존(**Flyway 13 · 테이블 15 · 뱅크 26행**, 09-04 와 동일) → 앱 3개 1회 rollout, 재시작 0.

**레이트리밋 상향 검증 (설계한 판별점이 실제로 작동했다)**
```
explain #1..#6 -> 200 200 200 200 200 200
```
6번째가 200 = 상향이 먹었다. 5회까지는 상향과 무관하게 성공하므로 **6번째만이 판별점**이다.
...
── 상태코드 분포 ──
  59889 200
```
429·5xx·`ERR` **0건**. ai-api 로그 `[STUB]` **19,970회**(= 59889/3) → 부하가 실제로 3서비스를 전부 탔다.

#### 측정 결과 (cgroup v2, `ws = current − inactive_file`)

| 서비스 | idle | 워밍업후 | 부하중 | **`memory.peak`** | 회복(+60s) |
|---|---|---|---|---|---|
| `core-api` | 347 | 351 | 375~389 | **401Mi** | 377 |
| `ai-api` | 181 | 182 | 191~202 | **212Mi** | 201 |
| `daily-api` | 290 | 294 | 324~341 | **363Mi** | 333 |
| `postgres` | 34 | 35 | 55~58 | **63Mi** | 58 |

🔑 **회복이 거의 없다.** 부하 종료 60초 뒤에도 값이 그대로다 — **SerialGC 는 OS 에 반환하지 않는다**
(09-05 F-7 예측대로). → **`requests` 는 idle 이 아니라 peak 을 덮어야 한다.**

📌 `postgres` 는 `anon` 이 **11Mi→32Mi** 뿐이고 나머지가 페이지 캐시(`file` 24Mi)다.
`memory.current` 만 봤으면 필요량을 과대평가했을 것 — `inactive_file` 을 빼는 선택이 값을 했다.

#### 🔴 가장 값진 실측 — `core-api` 영역별 (부하 중, 컨테이너 총 376Mi)

| 영역 | used | commit |
|---|---|---|
| heap 합계 | 70.3Mi | **139.3Mi** |
| **Metaspace** | **124.6Mi** | **125.6Mi** |
| CodeCache | 40.0 | 40.6 |
| Compressed Class Space | 16.7 | 17.2 |
| **nonheap 합계** | 181.3 | **183.4Mi** |
| **JVM 커밋 총합** | | **322.7Mi** |

**nonheap(183Mi) 이 heap(139Mi) 보다 크다.** 힙은 상한 **316Mi 중 70Mi 만** 쓴다.
→ 09-05 가설 ***"`limits` 를 조여도 조여지는 것은 힙뿐"* 이 실측으로 확인됐다.** 힙 상한을 깎는 건
**이미 안 쓰는 여유**를 깎는 것이다. 실제 소비자는 **Metaspace 125.6Mi** — 로드된 클래스가 정하는 값이고
`limits` 와 무관하다.

네이티브 오버헤드 = 376 − 322.7 ≈ **53Mi** (스레드 스택·GC 구조·직접버퍼·malloc arena).

**교차검증**: GC 로그와 prometheus 가 독립적으로 일치했다.
```
core-api  GC(240) Pause Young 96M->57M(139M)     ← 커밋 139M
prometheus  합계 heap  used 70.3  commit 139.3   ← 일치
ai-api    GC(118) 41M->24M(60M)      daily-api  GC(253) 78M->47M(113M)
```

#### 판정 규칙(§4.2) 기계적 적용 — 측정값을 보고 규칙을 고치지 않았다

| 서비스 | W_peak | req 신규 | (현행) | lim 신규 | (현행) | MaxHeap | 예약여유 |
|---|---|---|---|---|---|---|---|
| `core-api` | 401 | **480** | 512 | 576 | 900 | 201Mi | 119Mi |
| `ai-api` | 212 | **256** | 512 | **448** ←하한 | 900 | 156Mi | 36Mi |
| `daily-api` | 363 | **448** | 512 | 512 | 900 | 179Mi | 77Mi |
| `postgres` | 63 | **96** | 256 | 128 | 512 | — | — |
| **합** | 1039 | **1280** | **1792** | 1664 | 3212 | | |

**requests −512Mi (−29%).** 🔑 **하한 448Mi 가 실제로 일을 했다** — `ai-api` 는 규칙대로면 320Mi 인데
그러면 예약 상한(156+160+96=412)이 컨테이너 상한을 넘는다. 09-05 QA F-1 정정이 없었으면
**384Mi 로 잡아 구성 모순을 만들었을 것이다.**

📌 규칙의 구멍 하나 발견: **하한 448Mi 는 JVM 컨테이너에만 적용된다.** 근거가 JVM 예약 상한
부등식이므로 `postgres` 에는 성립하지 않는다. 규칙 문장에 그 구분이 없었다 → 계획서에 명시할 것.

#### 노드 2대 성립 여부 (산술)

```
비관 가정(시스템 Deployment 가 전부 한 노드): 노드A 959Mi · 노드B 1261Mi
앱 필요 1280Mi ≤ 2220Mi,  가장 큰 앱 480Mi ≤ 959Mi → 조각화 통과
```
→ **D-011(3대)이 뒤집힌다.** 단 **아래 사유로 실측 검증은 못 했다.**

### [막힘] 🔴 **과금 중에 사용자에게 질문했고, 대기 127분이 그대로 과금됐다**

측정이 끝난 11:29 에 *"노드 2대 검증까지 이어갈까요?"* 를 물었다. 사용자는 **2시간 뒤에** 답했다 —
지극히 정상이다(대기 중인 사람이 아니다). 그동안 **클러스터 3노드가 계속 돌았다.**

13:36:15 에 dead man's switch 가 발동해 자동 destroy 했다.
```
[2026-09-06 13:36:15 KST]  ✅ tofu destroy 완료 — 과금 종료. 마커 청소.
Destroy complete! Resources: 29 destroyed.
```
그래서 답을 받았을 때 클러스터는 이미 없었다:
```
Unable to connect to the server: dial tcp: lookup ...eks.amazonaws.com: no such host
aws eks describe-cluster → ResourceNotFoundException: No cluster found for name: devquest-eks
```

#### [비용] 🔴 예상 $0.10 → **실제 $0.4622**

| 구간 | 시간 | 비용 |
|---|---|---|
| 실제 작업 (apply~측정 완료) | 11:07:33~11:29, 21분 | $0.0653 |
| **대기 (질문 답변 기다림)** | **11:29~13:36, 127분** | **$0.3948 ← 순손실** |
| **합** | **2h 28m 42s** | **$0.4622** |

누적 $2.0072 → **$2.4694 / $200 (1.23%)**. 이 트랙 **단일 세션 최고 비용**이고,
그중 **85%가 아무것도 안 한 시간**이다.

**고아 0건** — EKS·EC2·ELB·NAT·RDS·Secrets 전부 0, `tofu state` 0,
영속 볼륨 `vol-0518b6d0dcd2b0d70` 1개만 잔존(인벤토리와 일치). teardown 자체는 깨끗했다.

#### 🔑 **이건 안전장치의 실패가 아니라 성공이다 — 그리고 내 절차의 실패다**

리퍼는 정확히 설계대로 동작했다. 없었으면 답을 받을 때까지, 혹은 그 이상 계속 돌았다.
**$0.39 를 잃은 대신 무한 과금을 막았다.** 08-31 에 만든 장치가 처음으로 **실제로 돈을 지켰다.**

문제는 **내가 과금 창 안에서 블로킹 질문을 한 것**이다. 계산해보면 명확하다:

```
재apply + destroy 왕복 = 9분 + 10분 = 19분 ≈ $0.059
대기 손익분기 = 19분.  사람에게 묻는 질문이 19분 안에 답 오는 일은 드물다.
```

> 🔴 **규칙: 과금 구간에서 사용자 질문을 하지 않는다.**
> 범위 결정은 **apply 전에** 끝낸다. 세션 중 새 선택지가 보이면 **먼저 destroy 하고 묻는다** —
> 재apply 19분($0.059)이 대기 1시간($0.19)보다 싸다.
> 불가피하게 물어야 하면 **질문과 동시에 destroy 를 건다.**

🔑 **09-04 가 세운 원칙(*"과금 구간에서 무엇을 하지 않을지가 비용을 정한다"*)을 내가 어겼다.**
그때는 *"진단을 과금 전으로"* 였고, 이번엔 **"의사결정을 과금 전으로"** 다. 같은 원칙의 다른 면인데
09-05 준비 문서에도, SOP 에도 **"질문"은 적혀 있지 않았다** — 절차가 다루지 않은 축이었다.

📌 부수 관찰(고치지 않음, 하네스 동결 규칙): 하트비트는 **Stop 훅**이라 턴이 끝날 때만 갱신된다.
`heartbeat` mtime 은 11:08 인데 작업은 11:29 까지 이어졌다 — 도구 호출이 연속되는 동안엔 턴이
안 끝나므로 갱신이 없다. **이번 결과는 안 바뀐다**(11:29 갱신이었어도 13:29 에 죽었다).
다만 *"3시간짜리 연속 작업"* 에서는 살아 있는 세션을 죽일 수 있다 → **원장 등재**.

### [메모] 2대 검증은 못 했다 — 결론의 지위를 낮춰 적는다

`kubectl set resources` 로 새 값을 넣으려던 순간 클러스터가 이미 없었다.
따라서 **노드 2대는 "산술로 성립"까지이고 "실측 확인"이 아니다.**

⚠️ 09-03 의 교훈이 여기 그대로 적용된다 — 그때도 산술이 옳았고 09-04 실측이 확증했다.
하지만 **08-31 은 산술 없이 믿었다가 유료 세션 하나를 날렸다.**
→ **D-012 를 `🟡산술확정·미검증` 으로 연다.** 다음 세션의 **첫 15분**에 검증한다
(이번엔 질문 없이 — 이미 정해진 절차만 실행).

---

## 2026-09-07 — 부하 테스트 재설계 (무과금, $0)

**계기**: 사용자 지적 — *"부하테스트에 대한 신뢰도가 부족해"*. **맞는 지적이라 결론을 강등했다.**

### [결정] 09-06 결론을 부하 품질 의존 여부로 갈랐다

| 결론 | 지위 | 부하 품질 의존 |
|---|---|---|
| nonheap 183 > heap 139 · **Metaspace 125.6Mi** | 🔴 실측 | **아니오** — 영역별 스냅샷 |
| *"`limits` 를 조여도 조여지는 건 힙뿐"* | 🔴 확인 | **아니오** |
| SerialGC 가 OS 에 반환 안 함 | 🔴 실측 | 아니오 |
| `W_peak` 401/212/363/63Mi | 🟡 **이 부하에서의 값 = 하한** | **예** |
| `requests` 1280Mi 안전 · 노드 2대 | ⚪ **약함** | **예** |

→ **D-012 를 `🟡산술확정·미검증` → `⚪보류 — 입력 신뢰도 부족`** 으로 강등.
*"재설계된 부하를 통과하기 전까지 매니페스트를 바꾸지 않는다"* 를 결정 블록에 박았다.

🔑 **근본 결함은 하나였다 — 생성기가 자기 한계를 보고하지 못했다.**
서버가 한가한 것과 클라이언트가 못 밀어넣는 것이 **똑같이 "에러 0"** 으로 보인다.
이 트랙이 반복해 온 ***"부재가 성공과 똑같이 생긴"*** 실패의 또 한 사례다.
계획서 §2 가 *"부하를 건다"* 만 정하고 **"부하가 유효한지 어떻게 아는가"** 를 안 정한 결과다.

### [해결] `k6` 로 교체 — `dropped_iterations` 하나 때문이다

`grafana/k6:0.58.0` (arm64 확인: `linux/amd64 linux/arm64`).
`constant-arrival-rate` = **오픈 루프**. 목표 RPS 를 못 지키면 **그 사실을 보고한다.**

| 판정 | 신호 |
|---|---|
| **서버 포화** | `dropped_iterations == 0` 이면서 p95 급등 / `http_req_failed` 상승 |
| **생성기 병목** | `dropped_iterations > 0` → 이 회차 데이터는 **버린다** |

**로컬 실측 (colima, whoami 대상):**
```
① 정상   RATE=25 × 10s → http_reqs 753 (75.29/s = 25×3) · failed 0.00% · dropped 0 · p95 973µs · exit 0
② 반증A  서버 없음      → http_req_failed 100.00% (153/153)                              · exit 99
③ 반증B  RATE=20000    → http_reqs 56628 (10341/s 성공) · dropped_iterations 81126       · exit 99
```

🔑 **③이 09-06 부하의 병을 그대로 보여준다.** 서버는 10,341 req/s 를 **정상 처리했는데**
생성기가 목표를 못 채웠다. **셸 루프였다면 요청 수만 적게 나오고 "전부 200"으로 보고했을 것이다.**
k6 는 종료코드 99 로 **실패시킨다** — 이게 09-06 에 없던 통제다.

### [메모] colima 마운트 함정에 **두 번째로** 걸렸다

`-v /tmp/journey.js:/j.js:ro` → 컨테이너 안에서 **디렉터리**가 된다(colima 가 그 호스트 경로를
VM 에 안 넘기고 docker 가 빈 디렉터리를 만든다). 09-05 에 `sh -n` 거짓 통과로 겪은 것과 **같은 원인**.
```
level=error msg="read /j.js: is a directory"
```
이번엔 **에러가 시끄러워서** 즉시 알아봤다(09-05 는 `sh -n` 이 exit 0 을 내 조용히 통과했다).
→ 규칙: **이 기기에서 `docker run -v` 로 `/tmp` 경로를 넘기지 않는다. stdin 을 쓴다.**

### [메모] 🔴 경제성 — 이건 절약이 아니라 학습이다

`requests` 최적화로 얻는 것은 **노드 1대 = $0.0283/h**. 09-06 에 **$0.46** 을 썼다.
**회수에 16시간이 걸린다.** 절약 목적이면 진작 그만뒀어야 한다.
→ 그렇다면 목표는 *"requests 를 줄인다"* 가 아니라 **"부하 테스트를 제대로 하는 법을 배운다"** 이고,
그 기준으로 보면 09-06 의 셸 루프는 **배움으로서도 부실**했다. 재설계의 진짜 근거는 여기다.

### [비용] $0 — AWS 접촉 0건. 누적 **$2.4694 / $200 (1.23%)** 변동 없음

## 2026-09-08 — 부하 생성기 격리 (무과금, $0)

**[막힘]** k6 램프 유료 세션을 착수하려다 `k8s/loadtest/k6.yaml` 점검에서 발견:

```
$ grep -n 'nodeName\|nodeSelector\|affinity' k8s/loadtest/k6.yaml
(없음)
```

계획서 §9.5 는 *"측정 대상과 다른 노드에 고정"* 을 요구하는데 **매니페스트에 그 장치가 하나도 없었다.**
그대로 apply 했으면 스케줄러가 k6 를 측정 대상과 같은 노드에 올릴 수 있었고, k6 는 requests
cpu 500m — t4g.small 2 vCPU 의 **25%** 다. 앱의 CPU 를 빼앗은 채로 그 앱을 측정하게 된다.
**09-06 부하를 폐기하게 만든 실패(생성기 오염을 알 방법이 없음)와 정확히 같은 종류다.**

**[막힘]** 그런데 `podAntiAffinity` 만 붙이는 1차 해법이 **성립하지 않았다.**
측정 대상이 4개(`ai-api`·`core-api`·`daily-api`·`postgres`)라 노드 2~3대를 **전부 채울 수 있다**(비둘기집).

| 방식 | 결과 |
|---|---|
| `required` anti-affinity 단독 | 빈 노드가 없으면 k6 **Pending 에 갇힌다** — 과금 중에 |
| `preferred` 로 완화 | 보장이 사라져 **조용히 같은 노드에 올라간다** = 원래 문제 |

🔑 **빈 노드는 저절로 생기지 않는다. 절차로 비워야 한다.**

**[결정]** 3중 장치 + 비우기 절차 (계획서 §9.7 신설):

| 장치 | 역할 |
|---|---|
| `nodeSelector: devquest.io/role=loadgen` | 라벨 붙인 노드에만 간다 |
| `tolerations: node.kubernetes.io/unschedulable` | `cordon` 은 taint 다 — 없으면 비워 둔 노드에 k6 자신도 못 들어간다 |
| `podAntiAffinity` **required** | **가드** — 앱이 그 노드에 있으면 Pending 으로 시끄럽게 실패 |

세 번째는 오염을 *막는* 장치가 아니라 **침묵을 막는** 장치다.
비우기는 `k8s/loadtest/pin-loadgen-node.sh` 가 한다(과금 창에서 손으로 치지 않는다):
측정 대상이 가장 적은 노드 선택 → cordon + 라벨 → 그 노드의 **측정 대상만** 삭제 → **0개 검증**.
`drain` 을 안 쓴 이유: 시스템 파드까지 건드린다. 옮기려는 건 측정 대상 4종뿐이다.

**[해결]** 무과금 검증 — 가짜 `kubectl` 로 세 경로 전부 실행:

```
케이스1 정상    → 가장 적은 노드 선택 → 이동 → ✅ 0개 · exit 0
케이스2 못 비움  → 🔴 "남은 측정 대상 1개" · exit 1
케이스3 노드 1대 → 🔴 "격리할 수 없다" · exit 1
```

⚠️ 하네스 자체 버그를 두 번 잡았다(`cordon` 인자 위치, `delete` 의 파드명 위치를 `$4` 로 오파싱).
**목이 틀리면 스크립트가 멀쩡해도 실패로 보인다** — 케이스2 가 처음에 거짓 실패를 냈다.
`sh -n` 은 깨진 스크립트로 **반증 테스트를 먼저** 통과시킨 뒤에 신뢰했다(09-06 의 false-pass 재발 방지).

**[메모]** AZ 함정 없음 — `nodes.tf` 가 모든 노드를 영속 EBS 와 **같은 단일 AZ 서브넷**에 고정한다.
어느 노드를 비워도 postgres 가 붙을 노드가 같은 존에 남는다(RWO detach/attach 만 발생).

**[결정]** 램프는 `node_desired_size=3` (앱 2 + 생성기 1). `-var node_desired_size=2` 로 내리면
생성기 노드가 없다. 원장 **L-47** 의 "노드 2대 검증"과 예산이 겹치므로, 판정 시
생성기 노드에 얹힌 시스템 **Deployment** 의 requests 를 빼고 봐야 한다 — 안 그러면
앱 노드가 실제보다 넉넉해 보인다. L-47 에 이 보정 요구를 추가했다.

**[비용]** $0. AWS 리소스 변경 없음 — 매니페스트·스크립트·문서만.

**[해결]** QA 리뷰 6건(HIGH 0 · MEDIUM 4 · LOW 2) 전부 수정. 특히 **F-6 은 자해였다** —
같은 커밋에 넣은 "세션 절차" 주석이 `kubectl get nodes | tail -1` 로 **아무 노드나** 고르라고
적고 있었다. 바로 위 15줄에서 *"비둘기집이라 아무 노드나 고르면 안 된다"* 고 설명해 놓고서다.
과금 창에서 주석을 그대로 쳤으면 격리가 깨진 채 측정이 돌았을 것이다.
→ 절차의 **단일 출처를 스크립트로** 못 박고, 주석에는 "여기에 절차를 다시 적지 말 것"을 남겼다.

| ID | 등급 | 조치 |
|---|---|---|
| F-1 | MEDIUM | 축출 루프의 `kubectl delete` 에 `|| true` — 개별 실패가 `set -e` 로 파이프라인을 죽여 **④ 검증에 도달조차 못 하던** 것을 수정. 성패 판정은 ④ 가 한다 |
| F-2 | MEDIUM | 재실행 안전성 — 라벨된 노드가 있으면 **재사용**, 2개면 모호로 `exit 1`. 실패 시 `uncordon` 원위치 명령 출력. 재실행마다 새 노드를 cordon 해 Pending 스톰이 나던 경로 제거 |
| F-3 | MEDIUM | anti-affinity 에 `namespaces: [default]` 명시. 기본값이 *"파드 자신의 네임스페이스"* 라 컨텍스트가 어긋나면 **가드가 조용히 무력화**됐다. `networkpolicy-ai-api.yaml` 의 전례와 같은 실패 유형 |
| F-4 | LOW | 성공 출력을 `-o wide`+`$7` 대신 `custom-columns` 로 — RESTARTS 가 `"3 (5m ago)"` 가 되면 컬럼이 밀린다 |
| F-5 | LOW | `notready` 판정을 **측정 대상만** 으로 한정. 무관한 파드(과거 Job 의 Error 잔해)로 원인 불명 타임아웃이 나던 것 |
| F-6 | MEDIUM | 자기모순 주석 제거 → 스크립트로 위임 명시 |

**[막힘]** 수정 중 `case` 문을 `$( )` 명령치환 안에 넣었더니 문법 에러:

```
pin-loadgen-node.sh: line 114: syntax error near unexpected token `;;'
```

원인은 **bash 3.2 파서** — `case` 패턴의 `)` 가 명령치환의 닫는 괄호로 먼저 해석된다.
macOS 기본 `sh` 가 bash 3.2 다. `pod_not_ready()` 헬퍼 함수로 빼서 해소.
⚠️ 이어서 `sed` 치환에서 `&&` 의 `&` 가 **매치 전체로 확장**돼 줄이 뭉개졌다 — ruby 로 교체.

**[메모]** 목 하네스 버그를 **세 번** 잡았다: `cordon` 인자 위치 · `label node <NAME>` 의
노드명이 `$2` 가 아니라 `$3` · `NAME,NODE,APP` 패턴이 `NODE,APP` 를 **부분문자열로 포함**해
덜 구체적인 분기로 샌 것. **목이 틀리면 멀쩡한 스크립트가 실패로 보인다** — 케이스1 이 두 번
거짓 실패를 냈다. *검증 도구도 검증 대상이다.*

**[해결]** QA 재검토(델타) — F-1~F-6 전건 `fixed` 확인. 신규 2건도 수정:

**F-7 (MEDIUM) 이 이번 라운드의 진짜 수확이다.** 내가 넣은 검증 로직이

```sh
... | grep -c x || true      # "정말 0개"와 "kubectl 실패로 입력이 빔"이 똑같이 0
```

이라 **④ 검증 도중 kubectl 이 실패하면 `left=0 notready=0` 이 되어 ✅ 를 조기 선언**할 수 있었다.
🔑 *"부재가 성공과 똑같이 생긴"* — **이 PR 이 없애려던 바로 그 실패를 검증 단계에 재도입한 것이다.**
이 트랙에서 같은 형태가 반복해 나온다(09-06 에러 0건 · NetworkPolicy 무시 · Grafana 자리표시).

→ `pod_snapshot()` 으로 조회를 분리하고 **종료코드를 본다.** 실패한 샘플은 버리고 다시 잰다.
   센티널 `-1` 을 유지해 *"한 번도 못 쟀다"* 와 *"쟀는데 0 이 아니었다"* 를 메시지에서 가른다 —
   전자는 **성공도 실패도 아니라 미확인**이다. 부수 효과로 루프당 kubectl 호출 2회 → 1회,
   `left` 와 `notready` 가 같은 스냅샷에서 나와 서로 어긋나지 않는다.

**F-8 (LOW)**: 재사용 분기가 *"새로 cordon 하지 않는다"* 고 출력하는데 `cordon`·`label` 은
분기와 무관하게 항상 실행됐다(멱등이라 무해하나 메시지가 거짓). 문구를 사실에 맞췄다.

**[해결]** 검증 7경로 (무과금): 정상 / 재실행 재사용 / 라벨 2개 모호 / 노드 1대 / delete 실패 /
**조회 상시 실패** / **격리 성공 후 ④ 조회만 실패**. 마지막 둘에서 `✅ 출현 0회` 를 확인했다.

## 2026-09-08 — k6 램프 유료 세션 (부하 무릎 탐색)

**[메모]** 세션 시작 준비 (아직 $0). 사전 점검 결과:

| 항목 | 값 |
|---|---|
| 도구 | tofu · kubectl · aws · ruby 전부 존재 |
| 자격증명 | `arn:aws:iam::<account>:user/bootstrap-admin` |
| 기존 리소스 | EKS **0** · EC2 running **0** (깨끗) |
| ECR 이미지 | core-api · daily-api · ai-api 전부 `14cb335`(09-03), **그 이후 `be/` 변경 0건** → 최신 |
| 예산 실측 | ActualSpend **$2.453** / $200 |

**[결정]** 노드 **3대**(앱 2 + 생성기 1). `node_desired_size=2` 로 내리면 #413 이 만든
생성기 격리가 성립하지 않는다(빈 노드 없음). L-47 의 "2대 검증"은 앱이 2노드에 들어가는 것으로
확인하되, **생성기 노드에 얹힌 시스템 Deployment 의 requests 를 빼고** 판정한다.

**[해결]** `tofu apply` 완료 — **29 added, 0 changed, 0 destroyed. 9분 28초** (10:21:27 → 10:30:55).
노드그룹 **1m48s**, 애드온 vpc-cni/kube-proxy 14s · coredns 14s · ebs-csi **45s**.
마커 생성 확인(`applied_at_h=2026-09-08 10:21:22 KST`) → **dead man's switch 무장**
(09-04 에 여기서 조용히 꺼졌던 지점 — 이번엔 `&& tofu apply` 형태라 정규식에 걸렸다).

노드 3대 전부 `Ready` · k8s **v1.36.3-eks** · 전부 **ap-northeast-2a**(영속 EBS 와 동일 AZ) ·
allocatable `1397828Ki` = **1365Mi** · pods **11**.

**[해결] 원장 L-43 종료 — 시스템 파드 requests 를 DaemonSet ÷ Deployment 로 쪼갰다.**

```
NODE            POD                                   OWNER       MEM_Mi  CPU_m
ip-10-0-12-6    aws-node-2l8rd                        DaemonSet        0     50
ip-10-0-12-6    coredns-6b5dbd976b-k4swz              ReplicaSet      70    100
ip-10-0-12-6    ebs-csi-node-6pgx6                    DaemonSet      104     30
ip-10-0-12-6    kube-proxy-f8sgj                      DaemonSet        0    100
ip-10-0-2-186   ebs-csi-controller-778b46d4db-b2gmv   ReplicaSet     232     60
ip-10-0-6-80    (DaemonSet 3종만)                                    104    180
```

| 구성 | 실측 | 종전 인식 |
|---|---:|---|
| DaemonSet (노드당 고정) | **104Mi** | 쪼개진 적 없음 |
| `coredns` 1개 | **70Mi** | 🔴 `addons.tf` 주석의 `~100Mi`(🟡미확인)는 **틀렸다** — upstream 기본값을 인용한 것이었다 |
| `ebs-csi-controller` | **232Mi** | **시스템 최대 소비자.** coredns 의 3.3배인데 아무도 주목한 적 없다 |
| 합계 | 104 + 70 + 232 = **406Mi** | 08-31 단일 노드 실측 **406Mi 와 정확히 일치** → 교차 검증 |

🔑 **노드별로 보면 균등하지 않다** — 여유가 `12-6` 1191Mi · `2-186` **1029Mi** · `6-80` **1261Mi**.
*"노드당 959Mi"* 같은 단일 숫자로 계산하던 종전 방식은 **가장 빡빡한 노드를 못 본다.**

**[결정]** 생성기 노드 = **`ip-10-0-6-80`** — 시스템 **Deployment 가 하나도 없는** 유일한 노드다.
여기를 비우면 나머지 두 노드가 coredns(70) + ebs-csi-controller(232)를 그대로 지므로,
**원장 L-47 이 요구한 "생성기 노드의 시스템 파드를 빼고 판정" 보정이 배치로 저절로 성립한다.**
(`pin-loadgen-node.sh` 는 측정 대상이 가장 적은 노드를 고르는데, 배포 전이라 셋 다 0 이다 →
`NODE=` 로 명시 지정한다. 스크립트가 그 경로를 지원하고 09-08 무과금 검증에 포함돼 있다.)

**[해결]** 배포·검증 완료 (10:31~10:38). ESO 3종 `SecretSynced/True` · static PV `Bound` ·
postgres 비밀번호 동기화 `ALTER ROLE` + **파드 IP 로 검증**(`select 1` → `1`) ·
영속 데이터 유지 확인(**Flyway 13 · 질문뱅크 26행**) · NetworkPolicy 적용 ·
`enableNetworkPolicy:"true"` 애드온 실측 반영 확인 · `aws-node 2/2`(노드에이전트 사이드카 존재).

⚠️ **`/actuator/health` 가 503 이지만 정상이다** — 원인 `Mail health check failed`.
학습 클러스터에 메일 자격증명을 안 넣는다(보안 제약). k8s 가 보는 `/health/readiness` 는 `UP` 이고
파드는 `1/1`. **집계 health 와 readiness 그룹은 다른 것을 본다** — 이걸 모르면 여기서 시간을 태운다.

**[해결]** 레이트리밋 상향 — `kubectl set env` 로 **런타임 주입**(계획서는 yaml 수정을 적었으나
그러면 우회 설정이 레포에 커밋된다). 검증: `explain` 연속 7회 **전부 200**.
종전 capacity 5 였다면 6번째가 429 다 — **6번째가 유일한 판별점**이라는 09-05 설계가 그대로 작동했다.

**[해결] 생성기 격리(#413)가 실전에서 3중 장치 전부 작동했다.**
`NODE=ip-10-0-6-80` 지정 → cordon+라벨 → daily-api·postgres 축출 → **0개 검증 통과(exit 0)**.
k6 가 **cordon 된 노드에 떴다** — toleration 이 없었으면 k6 자신도 못 들어갔을 자리다.

재배치 결과 = **원장 L-47 의 답**:

| 노드 | 시스템 | 앱 | 합계/가용 | 여유 |
|---|---|---|---:|---:|
| `12-6` | DS 104 + coredns 70 | ai-api 512 + postgres 256 | 768 / 1191 | 423Mi |
| `2-186` | DS 104 + ebs-csi-controller 232 | core-api 512 + daily-api 512 | 1024 / 1029 | **5Mi** |

🔑 **현행 requests(1792Mi)로도 2노드에 들어간다 — 단 여유 5Mi.** D-012 는 *"1280 으로 줄여야
2대가 된다"* 고 봤는데, **줄이지 않아도 들어간다**(다만 실질 여유가 0). 전제가 부분적으로 틀렸다.
🔑 그리고 `6-80` 에는 시스템 **Deployment 가 하나도 없어서**, 이 노드를 빼도 나머지 둘은
**진짜 2노드 클러스터와 동일한 시스템 부담**을 진다 → L-47 이 요구한 보정이 **배치로 성립**했다.

**[해결] 램프 결과 — 무릎을 못 찾았다. 서버가 포화되지 않았다.**

```
스테이지  RATE  실측 req/s   p95      dropped  failed        판정
1          25      75.03   22.95ms        0   0/4503        유효 (JIT 콜드)
2          50     150.02   10.89ms        0   0/9003        유효
3         100     300.00    8.63ms        0   0/18000       유효
4         200     599.95    8.16ms        0   0/36003       유효
5a        400    1193.31   47.48ms      129   0/71616       🔴 폐기 (생성기 병목)
5b        400    1199.14   15.01ms       12   0/71967       🔴 폐기
5c        400    1199.47   17.54ms        0   0/72003       ✅ 유효 (PREVUS=800)
```

🔑 **p95 가 부하와 함께 *내려간다*** (22.95 → 10.89 → 8.63 → 8.16). JIT 워밍업이다.
→ **스테이지 1 을 기준선으로 쓰면 안 된다.** 중단 임계 *"스테이지1 p95 × 3"* 은
콜드 스타트를 기준으로 삼는 셈이라 **너무 관대하다.** 계획서 §9.3 의 판정 규칙을 고쳐야 한다.
...
그런데 그 회차(5b)의 dropped 가 129→12 로 좋아져서 **하마터면 *"VU 를 늘려 고쳤다"* 고 쓸 뻔했다.**
`vus_max` 를 확인해서 잡았다. 제대로 하려면 **Pod 만** 뽑아 적용해야 한다:
```bash
ruby -ryaml -e 'docs=YAML.load_stream(File.read("k8s/loadtest/k6.yaml")); puts YAML.dump(docs.find{|d| d["kind"]=="Pod"})' | kubectl apply -f -
```
⚠️ **정직하게**: 같은 설정(PREVUS=100)의 두 회차가 dropped **129 vs 12** 로 갈렸다. 편차가 크므로
*"VU 사전할당이 원인"* 이라고 **단정할 수 없다.** 말할 수 있는 것은 **800 에서 0 이 나왔다**는 사실뿐이다.
(5c 에서 실제로 쓰인 VU 는 **47** 뿐이었다 — 부족해서가 아니라 **할당 지연**이 의심되나 미확정.)

**[해결] 부하 해제 후 메모리가 돌아오지 않았다** — core-api 408→408, ai-api 208→208, daily-api 434→430.
**SerialGC 는 OS 에 반환하지 않는다**는 09-06 발견의 재확인. GC Pause 횟수: core 394 · daily 121 · ai 76.

**[결정] W_peak 실측과 재산정** (working set, 스테이지 5c 기준):

| 서비스 | baseline | **W_peak** | 현행 req/lim | **신규 req/lim** | 근거 |
|---|---:|---:|---:|---:|---|
| core-api | 352 | **408** | 512 / 900 | **480 / 576** | ×1.15 / ×1.35 |
| ai-api | 185 | **208** | 512 / 900 | **256 / 448** | lim 은 320 이지만 **JVM 하한 448** |
| daily-api | 292 | **434** | 512 / 900 | **512 / 640** | |
| postgres | 34 | **68** | 256 / 512 | **96 / 128** | JVM 아님 → 하한 미적용 |
| **합계 requests** | | | **1792** | **1568** | **−224Mi** |

2노드 적재 검증: **1568** ≤ 2220. 빈패킹 = `2-186` 480+512=**992** ≤ 1029 · `12-6` 320+256=**576** ≤ 1191.
**여유가 5Mi → 37Mi·615Mi 로 바뀐다.**

🔴 **정정 (QA F-1).** 처음에 합계를 **1344Mi(−448Mi)** 로 적었다. 공식값(ai 256 · pg 96)으로 더했는데
**그 둘은 공식을 따르지 않기로 한 항목**이다(ai 는 스텁 보정 320, postgres 는 제외 256).
개별 행은 320·256 으로 옳게 적어 놓고 **합계만 옛 값으로 남았다** — 표 안에서 자기모순이었다.
🔑 ***한 커밋 전에 내가 쓴 메타 교훈 그대로다*** — *"숫자는 맞는데 그 숫자가 답하는 질문이 다르다"*.
1344 는 **"공식을 그대로 따랐다면" 의 합계**이고, 내가 물은 것은 **"실제로 적용한 값" 의 합계**였다.
블로그 헤드라인이 될 절감 수치가 **2배로 부풀려져** 있었다.

⚠️ **표본 채택의 한계 (QA F-4)**: W_peak 을 각 스테이지의 **마지막 샘플(10/10)** 로 잡았다.
계획서 §9.3 은 *"샘플의 max"* 를 요구하는데 **전 샘플을 비교하지 않았다** — 관측된 값들이
거의 단조증가라 `마지막 ≈ max` 로 판단했다(스테이지1 실측: core 375→375→375 평평).
SerialGC 가 OS 에 반환하지 않는다는 사후 확인이 이 가정을 뒷받침하지만, **스테이지 내부에서
일시 스파이크 후 하락했을 가능성까지 배제하지는 못한다.** 다음 세션엔 스크립트가 max 를 직접 내게 할 것.

⚠️ **이 W_peak 은 하한이다.** ①`ai-api` 는 AI 스텁이라 실제 Anthropic 호출 메모리가 아니다
②**서버가 끝내 포화되지 않아** 진짜 무릎을 못 봤다 — 1200 req/s 는 이 앱의 현실 트래픽을 한참 넘는다.
즉 *"이보다 더 필요할 일이 없다"* 가 아니라 *"여기까지는 확실히 이만큼이다"* 다.

**[결정] postgres 는 재산정에서 제외한다 — 공식이 여기선 위험한 방향으로 틀린다.**
공식대로면 `96Mi / 128Mi` 다. 그런데 매니페스트에 `shared_buffers` 를 **명시하지 않아**
postgres 는 기본값 **128MB** 를 쓴다 → **limits 128Mi = 버퍼 풀만으로 상한**. 데이터가 늘면 OOMKill.

🔑 **측정값 68Mi 는 "설정된 용량"이 아니라 "지금 만진 페이지의 양"이다.** 26행짜리 거의 빈 DB 이고,
부하를 아무리 잘 걸어도 **같은 행만 조회한다**(계획서 §9.5 가 이미 인정한 한계).

⚠️ **#410 에서 JVM 하한 448Mi 를 postgres 에 적용하면 안 된다고 발견한 것의 *거울상*이다.**
그때는 공식이 과하게 **키웠고**(128→448, 3.5배), 이번엔 과하게 **줄인다**(512→128, 1/4).
**규칙은 측정 대상이 무엇인지 모른다** — JVM 인지 DB 인지, 잰 값이 *용량*인지 *사용량*인지.
→ 줄이려면 `shared_buffers` 명시가 **선행**돼야 한다. 값은 `256Mi/512Mi` 유지하고 사유를 매니페스트에 남겼다.

**[결정] 매니페스트에 실제로 반영했다** (*"일지는 기록이지 반영이 아니다"* — CLAUDE.md 빨간 깃발):

| 파일 | requests | limits |
|---|---|---|
| `k8s/base/core-api.yaml` | 512 → **480Mi** | 900 → **576Mi** |
| `k8s/base/daily-api.yaml` | 512 → **512Mi**(유지) | 900 → **640Mi** |
| `k8s/base/ai-api.yaml` | 512 → **320Mi** | 900 → **448Mi**(JVM 하한) |
| `k8s/base/postgres-static.yaml` | 256Mi 유지 | 512Mi 유지 (사유 주석) |

🔴 **`ai-api` 는 공식값 256 대신 320 으로 올렸다** — 측정 대상이 **AI 스텁**이라 실제 Anthropic
호출의 버퍼·파싱 메모리가 빠져 있다(D-008 이 감수한 대가). 계획서 §9.5 의 *"requests 산정 시
별도 여유를 줄 것"* 을 따른 것이지 공식을 어긴 게 아니다.

⚠️ **이 값들은 아직 실환경에서 안 돌려봤다.** 다음 세션의 첫 검증 항목이다.

**[해결]** `tofu destroy` — **29 destroyed**, exit 0. `tofu state list` **0건**.
AWS 실물 재확인: EKS **0** · EC2 running **0** · RDS **0** · LB **0** · NAT **0**.
EBS 는 `vol-0518b6d0dcd2b0d70`(10GiB, `Persistent=true`) **1개만** — **고아 0건.**

**[비용]** 과금 구간 **41m49s** (10:21:22 apply → 11:03:11 전부 0 확인).

| 계산 | 값 |
|---|---|
| 보수적 상한 (전 구간 $0.1865/h) | **$0.1300** |
| 분해 (노드 38m38s×$0.1865 + CP만 3m11s×$0.10) | $0.1254 |
| **채택** | **$0.1300** (상한) |

누적 **$2.4694 → $2.5994 / $200 (1.30%)**. 예상 $0.124~0.171 범위 안.

🔴 **비용 계산을 한 번 틀렸다가 잡았다.** destroy 출력 파일의 `stat -f %m` 을 종료 시각으로 썼더니
**30m37s** 가 나왔는데, 그러면 종료가 **10:51:59** — destroy 를 시작한 **10:53:05 보다 이르다.**
앞뒤가 안 맞아서 발견했다. 09-04 에 루트 EBS 를 빠뜨려 $0.0987→$0.1029 로 정정한 것과 같은 종류다.
**교훈: 파일 mtime 을 이벤트 시각의 대용으로 쓰지 말 것** — 버퍼링·플러시 시점이 개입한다.
시각은 `date` 로 **명시적으로 찍어 둔 것**만 쓴다.

**[메모]** 세션 마커 제거 완료 → `session-status.sh` 가 *"클러스터 미가동"* 으로 복귀.
dead man's switch 는 이번 세션 내내 무장돼 있었고 **발동할 일이 없었다**(전 구간 능동 작업).

**[해결] QA 리뷰 4건 (HIGH 0 · MEDIUM 1 · LOW 3) 전건 수정.**

🔴 **F-1(MEDIUM)이 뼈아프다 — 내가 *한 커밋 전에 쓴 메타 교훈*을 그대로 밟았다.**
재산정 합계를 **1344Mi(−448Mi)** 로 적었는데, 그건 **공식을 그대로 따랐을 때**의 값이다.
그런데 `ai-api`(스텁 보정 320)와 `postgres`(제외 256)는 **공식을 안 따르기로 한 항목**이고,
**합계만 옛 값으로 남았다.** 개별 행에는 320·256 을 옳게 적어 놨으니 **표 안에서 자기모순**이었다.

```
문서:  480 + 512 + 256 + 96  = 1344Mi  (−448)   ← 공식값 기준
실제:  480 + 512 + 320 + 256 = 1568Mi  (−224)   ← 매니페스트 기준
노드A 여유: 839Mi(문서) → 615Mi(실제)
```

***"숫자는 맞는데 그 숫자가 답하는 질문이 내가 던진 질문이 아니었다"*** — 09-08 퀴즈에서 뽑은
메타 교훈을 **적은 그 커밋의 다음 작업에서 재발**시켰다. 그것도 **블로그 헤드라인이 될 절감 수치**를
2배로 부풀린 형태로. 같은 오류가 일지·계획서·원장·CONTEXT **4곳에 복사**돼 있었다.
🔑 **교훈을 적는 것과 적용하는 것은 다른 일이다.** 표를 고칠 때는 **합계 행을 다시 계산**한다.

| ID | 등급 | 조치 |
|---|---|---|
| F-1 | MEDIUM | 4개 문서의 합계·절감·여유 수치 정정 (1344→1568 · −448→−224 · 839→615Mi) |
| F-2 | LOW | `ai-api.yaml` 의 *"core-api 와 동일하게 잡는다"* 주석이 재산정 후 거짓이 됐다(320/448 vs 480/576) → 해소 표시 |
| F-3 | LOW | `preAllocatedVUs` 기본값 800 의 **검증 범위가 RATE=400 한 곳뿐**임을 명시. 기본을 높은 쪽에 둔 이유(실패 비용 비대칭)도 함께 |
| F-4 | LOW | W_peak 을 **마지막 샘플**로 잡은 것의 한계 명시. 다음 세션엔 스크립트가 max 를 직접 내게 할 것 |
...
- `[비용]` **명시적 `date` 스탬프로 계산한다 — 파일 mtime 을 쓰지 않는다**(09-08 에 그걸로
  ...
  합계                                                 $0.0716
  ```
  **누적 실측 $2.557 / $200 (1.28%)** (세션 후 `aws budgets describe-budgets` 조회).
  ⚠️ **산정↔실측 대조는 다음 세션으로 미룬다** — Budgets 가 최대 24h 지연이라
  당일 조회로는 검증이 안 된다(상단 블록 참조, QA F-8).
...
- `[메모]` **LBC 가 evict 를 유발하지 않았다** — 노드 3대에 6·6·6 균등 배치, 비정상 파드 0,
  Evicted 이벤트 0.
  🔴 **그러나 이것이 `requests` 명시의 효과를 증명하지는 않는다 (09-11 퀴즈 Q2-2 에서 정정).**
  처음엔 *"requests 96Mi/192Mi 명시가 유효했다"* 고 적었는데 **과한 주장이었다.** 두 가설을 대보면:
  ```
  가설 A  requests 명시가 evict 를 막았다        → 예측: Evicted 0
  가설 B  requests 없어도 어차피 안 났다          → 예측: Evicted 0     ← 같다
  ```
  **두 가설이 같은 결과를 예측하므로 판정력이 0이다.** 그리고 B 가 그럴듯하다 —
  노드 3대(allocatable 4095Mi)에 앱 1568Mi + 시스템 ~1218Mi = 2786Mi 로 **여유가 ~1300Mi** 였고,
  **여유가 큰 노드에서는 BestEffort 도 evict 되지 않는다**(evict 는 압박 상태에서 일어난다).
  즉 이 세션은 A 와 B 를 **구별할 조건 자체를 만들지 않았다.**
  ✅ 확인된 것: `requests` 를 명시해도 **아무 문제가 없었다**(스케줄 실패·Pending 0).
  ⚪ 미확인: 명시가 **필요했는지**. 검증하려면 노드를 2대로 줄여 압박 상태를 만들어야 한다.
  🔑 그래도 **명시는 유지한다** — 실패 비용이 비대칭이기 때문이다. 과하게 잡으면 메모리를
  조금 낭비하고, 안 잡으면 **BestEffort 가 조용히 앱을 밀어낸다.** 이건 사전 판단으로 옳고,
  사후 증명과는 별개 문제다. **둘을 섞어 적은 것이 오류였다.**
  ⚠️ LBC 파드는 **distroless 라 셸이 없어** cgroup 실사용을 못 쟀다. 96Mi 의 적정성도 **미확인**.
...
## 2026-09-14 — 선행 조건 2a: 시크릿 환경 축 (무과금, $0)

> ⚠️ **이 절은 09-15 에 소급 기록했다.** 규칙은 *"이벤트 발생 즉시 append"* 인데 09-14 세션은
> 클러스터를 띄우지 않아(코드·문서만) 일지를 한 줄도 안 남겼다. **무과금이 기록 면제가 아니다** —
> 이 파일에는 이미 `## 2026-09-08 — 부하 생성기 격리 (무과금, $0)` 가 있다.
> 소급분은 PR #418·#420 과 `.claude/CONTEXT.md` 에서 복원했으므로 **당시 명령 출력은 없다.**

- `[결정]` JWT 서명 키(`random_password.jwt_secret`)를 `2-cluster` → `0-bootstrap` 으로 이동.
  근거는 D-004·L-14 (*"수명이 같아야 하는 것은 같은 레이어에"*). destroy-after-use 레이어에 있으면
  클러스터를 부술 때마다 키가 바뀌고 = 전 사용자 강제 로그아웃(토큰 만료 30일).
- `[막힘]` **규칙을 글자 그대로 복사하면 보안 경계가 사라진다.** Blindspot Pass 가 착수 전에 잡았다.
  L-14 의 앞선 세 적용(postgres 비밀번호 · 영속 EBS · 백업 S3)은 전부 **학습 전용 단일 환경**이라
  섞일 것이 없었는데, **JWT 는 처음으로 learning/prod 두 환경에 걸친다.** 키 하나를 영속 레이어에
  두면 수명은 맞추면서 환경을 섞어 **학습 클러스터가 prod 유효 토큰을 발급**하게 된다
  — `2-cluster/secrets.tf` 가 GitHub OAuth 에 대해 **스스로 금지해둔 형태**다.
- `[해결]` **환경 축 도입**: `var.jwt_environments = ["learning","prod"]` + `for_each` 로 키 2개,
  시크릿 이름 `<cluster>/<env>/app`, ESO key 파라미터화(기존 `*_PLACEHOLDER` sed 관례 재사용 — 새 도구 0).
  값 전달은 `0-bootstrap/outputs.tf`(sensitive) → `2-cluster/remote-state.tf` 의
  `data "terraform_remote_state" "bootstrap"`. **Secrets Manager 리소스는 2-cluster 에 그대로** 둬서
  ARN·IAM·`recovery_window_in_days = 0`·$0.40/월 회피가 전부 불변.
- `[메모]` `random_password` 는 AWS 리소스가 아니라 **tfstate 안의 값**이다 → 비용 $0,
  생성 시 `[id=none]`, **`aws ... describe-*` 어느 명령에도 안 나온다.** 조회 경로는 하나뿐:
  ```
  tofu -chdir=infra/aws-eks/0-bootstrap state list | grep random_password
  # 기대: 3줄 (postgres_master, jwt learning, jwt prod). 값은 출력하지 말 것
  ```
  → `PERSISTENT-RESOURCES.md` 에 「비용은 $0 인데 영속인 것」 절 신설.
- `[해결]` CI 실측이 로컬 plan 을 독립 확인:
  ```
  Plan: 2 to add, 0 to change, 0 to destroy
  jwt_secrets = (sensitive value)
  Apply complete! Resources: 2 added, 0 changed, 0 destroyed
  ```
- `[막힘]` QA F-2 — *"IAM 이 경계를 강제한다"* 를 조건 없이 썼는데 **틀렸다.**
  `2-cluster/backend.tf` 의 state key, `variables.tf` 의 `cluster_name` 기본값,
  `irsa-eso.tf` 의 `${var.cluster_name}-eso` 가 **전부 단일값**이라 두 환경의 2-cluster 가
  **아직 공존할 수 없다.** → **경계는 절반만 세워졌다**(세워진 절반은 IAM 이 지킨다)고 정정.
- `[결정]` 항목 2b(Grafana `instance-id`)는 **재판정으로 해소.** `instance-id` 는
  `Base64("$instanceId:$apiKey")` → `Authorization: Basic` 의 **username** 이라 시크릿이 아니고,
  *"학습/prod 메트릭 혼입"* 은 **#355(08-03)에서 자리표시 3종을 지우며 끝난 사고**를
  09-11 계획서가 **현재형으로 다시 적은 것**이었다. 남은 `fdaa::/16` 절 삭제만 항목 3 으로 이관.
- `[비용]` **$0** — 클러스터 미가동. 누적 영속분만: EBS 10GiB ≈ $0.91/월 + S3 2버킷.

## 2026-09-15 — 선행 조건 3 착수: HTTPS 경로의 $0 선행분

- `[결정]` **항목 3 을 유료/무료로 쪼갠다.** 계획서는 *"한 유료 세션(~$0.1)"* 으로 묶어뒀는데,
  구성요소를 뜯어보니 **절반이 클러스터를 필요로 하지 않는다.**

  | | 클러스터 필요 | 비용 |
  |---|:-:|---|
  | ACM 퍼블릭 인증서 발급 + Cloudflare CNAME 검증 | ❌ | $0 |
  | `SecurityConfig` 의 `hasIpAddress('fdaa::/16')` 절 삭제 | ❌ | $0 |
  | Ingress annotation 3개 + 실 HTTPS 확인 + 2a 실검증 | ✅ | ~$0.1 |

  🔑 **Cloudflare 수동 검증은 사용자 손을 타고 전파 대기가 예측 불가**다. 그걸 컨트롤플레인
  $0.100/h 가 도는 동안 하면 순손실이다 — #416 이 *"ALB 를 마지막 7분만 켜서 전체의 4%"* 로
  보여준 것과 같은 레버(**단가는 apply 전에만, 시간은 세션 중에만** 당길 수 있다, #417).
- `[결정]` **호스트명 = 학습 전용 서브도메인**(prod `api.quest.dhbang.co.kr` 은 건드리지 않는다).
  기각한 대안: *"api. 인증서만 미리 발급하고 DNS 는 전환일에"* — 세션마다 할 일이 0 이라 매력적이나
  브라우저 실물 HTTPS 를 못 보고 `ssl-redirect` 검증이 curl 기반이 된다. 학습이 목적이므로 기각.
  ⚠️ 대신 **ALB 는 세션마다 재생성되므로 DNS 이름이 바뀐다** → 세션마다 Cloudflare CNAME 수동 갱신
  (~2분, 무료). 이 비용을 알고 고른 것이다.
- `[결정]` 인증서는 **`0-bootstrap`(영속 레이어)** 에 둔다 — D-004·L-14 의 **다섯 번째 적용**.
  검증이 수동이라 세션마다 반복하면 안 된다(= 수명이 클러스터가 아니라 도메인에 묶인다).
- `[막힘]` **`aws_acm_certificate_validation` 을 넣으면 CI 가 멈춘다.** `infra-deploy.yml` 이
  `layer: [0-bootstrap, 1-network]` 를 main 머지 시 `tofu apply -auto-approve` 하는데,
  그 리소스는 CNAME 이 전파될 때까지 블로킹한다(기본 75분 뒤 실패). → **`aws_acm_certificate` 만**
  두고 검증 레코드를 output 으로 내보낸다. 이건 *"plan 은 통과하는데 apply 가 깨진다"* 계열
  (Stage 2 의 한글 description 과 같은 층위)이라 **코드를 쓰기 전에** 못 박아둔다.
- `[메모]` 착수 전 Blindspot Pass 실행 — 특히 *"`fdaa::/16` 삭제가 지금 돌고 있는 Fly prod 에서
  무엇을 닫는가"*. 계획서의 *"EKS 에서 아무것도 열지 않는다"* 는 **EKS 쪽 이야기일 뿐**이고,
  `be-cd` 가 main push 마다 Fly 로 자동 배포하므로 머지 즉시 prod 에 반영된다.

### 산정 ↔ 실측 대조 (09-11·09-13 분 미대조였던 것, 무과금 조회)

- `[비용]` **누적 실측 $2.9177** (07-01~09-15, `RECORD_TYPE=Usage` = 크레딧 적용 **전**).
  `aws budgets describe-budgets` 의 `$2.936` 와 독립 일치(24h 지연 오차 내).
  **CONTEXT 에 적혀 있던 `$0.481` 은 07-31 값**이라 8·9월 $2.43 이 빠져 있었다.
  ```
  aws ce get-cost-and-usage --time-period Start=2026-07-01,End=2026-09-16 \
    --granularity MONTHLY --metrics UnblendedCost \
    --filter '{"Dimensions":{"Key":"RECORD_TYPE","Values":["Usage"]}}' \
    --group-by Type=DIMENSION,Key=SERVICE
  ```
  ⚠️ **기본 조회(필터 없음)는 ≈$0 으로 나온다** — CE 는 크레딧 적용 **후** 순액을 보여주는데
  예산은 `include_credit = false` 라 총액이다. **두 숫자가 다른 것이 정상**이고, 필터 없이 보면
  *"아무 비용도 안 난다"* 로 오독한다. 이 레포가 반복해서 데인 형태(**부재처럼 보이는 것**)와 같다.

  | | 7월 | 8월 | 9월(~15) | 누적 |
  |---|---:|---:|---:|---:|
  | EC2-Other (유휴 EBS 등) | 0.0240 | 0.9161 | 0.4380 | 1.378 |
  | EKS 컨트롤플레인 | 0.1595 | 0.2396 | 0.4534 | 0.853 |
  | ECR 스토리지 | 0.0150 | 0.1073 | 0.0618 | 0.184 |
  | **AWS Cost Explorer (조회 행위)** | 0.1000 | 0.0700 | 0.0200 | **0.190** |
  | RDS·VPC·ELB·SecretsMgr·S3 | 0.189 | 0.032 | 0.092 | 0.313 |
  | 소계 | 0.488 | **1.365** | 1.065 | **2.918** |

- `[해결]` 🔑 **영속 비용 모델이 실측으로 검증됐다.** 8월 합계에서 세션분·조회분을 빼면 베이스라인이 남는다:
  ```
  8월 합계 $1.3652 − EKS 세션 $0.2396 − CE 조회 $0.0700 = $1.0556
  PERSISTENT-RESOURCES.md 추정                          = $1.08      (오차 2.3%)
  ```
  즉 `ECR $0.17 + EBS $0.91 + S3 ~$0.00` 은 **맞는 모델**이었다.
  ⚠️ 뺄셈이 성립하는 이유는 *"8월에 세션이 없어서"* 가 **아니다** — 아래 정정 참조. EKS 항목을
  통째로 뺐기 때문이고, 세션에 딸린 EC2 인스턴스·NAT 는 EC2-Other/VPC 에 섞여 남아 있다.
  **그래서 $1.0556 은 영속분의 상한이다**(추정과 2.3% 로 붙은 것은 그만큼 세션 잔여가 작다는 뜻).
- `[막힘]` **`AWS Cost Explorer` 누적 $0.190** (`get-cost-and-usage` **$0.01/요청**) = 누적의 **6.5%**,
  #417 세션 전체($0.0848)의 **2배**. 🔑 ***비용을 재는 행위가 비용 항목이다.***
  ⚠️ 처음엔 *"어디에도 없는 미등재 항목"* 이라고 적었는데 **틀렸다** —
  `## 2026-08-12 — 비용 추적표 정정 … 증분 $0.04 (CE 호출)` 가 **이미 한 번 인지했다.**
  정확한 서술은 ***"인지됐지만 상시 항목으로 옮겨지지 않았다"*** 이고, 그래서 한 달 뒤
  같은 비용이 같은 크기로 또 났다. **일회성 기록과 상시 모델 등재는 다른 일이다.**
  → 대조는 **세션 시작마다가 아니라 필요할 때만**, 그리고 **한 번에 넓게** 뜬다(오늘 3회 = $0.03 썼다).
- `[메모]` **세션 컴퓨트는 누적의 소수다.** 세션마다 초 단위로 산정해온 $0.06~$0.10 을 다 더해도
  20% 남짓이고, 나머지는 **켜둔 채 잊은 것(EBS·ECR)** 과 **재는 것(CE)** 이다.
  ⚠️ 이건 *"세션을 짧게 하라"* 가 아니라 ***"세션 최적화의 한계수익이 낮다"*** 는 뜻이다 —
  #417 이 *"노드 수가 세션 길이보다 비용을 더 움직인다"* 로 한 칸 옮겼던 관점을 한 칸 더 옮긴다.
- `[해결]` ⚪→🔴 **8월 EKS $0.2396 의 정체 확인.** 처음에 *"8월엔 세션이 거의 없었다"* 고 적었는데
  **틀렸다.** 이 파일에 8월 유료 세션이 셋 있다:
  ```
  grep -n "^## 2026-08" docs/eks-migration-log.md
  → 08-07 Stage 3b 검증 세션 · 08-12 L-14/L-15 검증 세션(유료) · 08-31 Stage C 3서비스 배포 세션(유료)
  ```
  🔴 **원인**: `CLAUDE.md` 하네스 동결 규칙의 *"8월 커밋 31건 중 18건이 하네스"* 를 읽고
...
- `[메모]` ⚠️ **U-17 — 내가 어제 적은 근거가 약하다.** *"ALB 는 세션마다 재생성되므로 DNS 이름이
  ...
  같은 이름이 여러 곳에 반복되고, 자리표시가 있고, **이 일지 항목 자신도 매칭된다.**
  🔴 **정정하면서 구체적인 줄 수를 적었더니 그것도 곧 틀렸다** (QA F-5) — 정정 문장 자체가
  같은 패턴에 매칭돼 **내가 세어 적은 순간 숫자가 늘어났다.** 그래서 **줄 수를 아예 적지 않는다.**
  🔑 **문서에 적는 검증 명령은 자기를 세지 않는 것을 세야 한다** — 여기서는 `sort -u` 로
  **고유 값**을 센다(위 블록). 같은 병을 같은 세션 CE 대조에서도 겪었고(그때는 `-- ':!*.md'`),
  이번엔 **고치는 문장이 다시 재발시켰다**: 자기참조는 한 번 끊어야지 세어서 맞출 수 없다.
...
- `[해결]` 🔑 **부수 수확 — `tofu plan ... | tee` 가 실패를 삼킬 수 있었다.** 파이프는 **마지막**
  명령(`tee`)의 종료코드만 내므로 pipefail 이 없으면 plan 실패가 초록으로 지나간다:
  ```
  bash --noprofile --norc -eo pipefail -c 'false | sed "s/x/y/"'  → exit 1
  bash -e                              -c 'false | sed "s/x/y/"'  → exit 0   ← 삼킨다
  ```
  → 파이프 스텝에 **`set -euo pipefail` 을 인라인으로** 박아 해소했다.
  ⚠️ **처음엔 *"GH 기본 셸이 `bash -e` 라 pipefail 이 없다"* 를 근거로 썼는데 취소한다**(QA F-3).
  위 비교는 **로컬 bash 플래그 비교일 뿐** GH 러너가 셸 미지정 시 어떻게 부르는지는 증명하지 않는다.
  🔑 ***런타임 기본값에 기대는 대신 스크립트가 직접 켜면 이 불확실성이 사라진다*** —
  그리고 이 레포엔 이미 그 관용구가 있었다(`ecr-push.yml` 의 `set -euo pipefail`).
  **마스킹을 붙이려고 파이프를 추가하는 과정에서 발견했다.**
- `[막힘]` 🔴 **그 마스킹용 파이프가 새 회귀를 만들었다 (QA F-1).** `sed` 는 파이프 출력일 때
  **full buffering** 이라 apply 가 끝나야 로그가 한꺼번에 쏟아진다. **과금 중에 진행 상황을 못 보는
  것은 실제 비용이다.** 실측(3줄·1초 간격):
  ```
  sed      28.3  28.3  28.3   ← 한꺼번에
  sed -u   28.3  29.3  30.3   ← 매초
  ```
  → `sed -u` 로 해소. 🔑 ***안전장치를 끼워 넣을 때 관측 가능성을 깨뜨리지 않았는지 보라.***
  기존 `tee` 는 line-buffered 라 문제가 없었는데, 내가 그 앞에 `sed` 를 끼우면서 깨졌다.
- `[메모]` ⚠️ **내 반증 하네스가 먼저 틀렸다.** zsh 에서 테스트했는데 `:c` 가 먹혀
  *"sed 가 문자를 먹는다"* 로 보였다. **재현 확인**(QA 가 zsh 실행 불가로 검증 못 했다고 해서 직접 쟀다):
  ```
  zsh  -f -c 'FAKE=999988887777; echo "arn:x:$FAKE:certificate/abc"'  → arn:x:999988887777ertificate/abc
  bash    -c 'FAKE=999988887777; echo "arn:x:$FAKE:certificate/abc"'  → arn:x:999988887777:certificate/abc
  ```
  **#417 의 *"반증 하네스의 `ROOT` 가 `/` 가 되어 두 arm 모두 통과"* 와 같은 층위** —
  🔑 ***검사 대상보다 검사 도구가 먼저 틀릴 수 있다. 러너와 같은 셸로 재라.***
- `[결정]` **과거 로그는 지우지 않는다.** 포크·아카이브는 회수 불가라 삭제의 실효가 낮다.
  계정 ID 는 자격증명이 아니라 **대상 특정 공격의 입력값**이므로, 앞으로 안 새게 하는 것이
  실질적인 조치다.
  ⚠️ **처음엔 *"이미 6주간 공개됐다"* 고 적었는데 근거가 없다**(QA F-4). 레포가 현재 PUBLIC 인 것과
  생성일(2026-03-19)은 확인되지만 **비공개→공개 전환 시점 기록을 레포에서 찾지 못했다**
  (`CONTEXT.md`·`CONTEXT.archive.md`·이 일지 grep 전부 0건). **기간을 특정하지 않는다** —
  결정(지우지 않는다)은 기간에 의존하지 않는다.
...
- `[해결]` **QA F-4 반증 — `-var node_desired_size=3` 은 이 스테이지에도 필요했다.**
  ...
  ```
  메모리  postgres 256 + core-api 512 + daily-api 512 + LBC 96 = 1376Mi
