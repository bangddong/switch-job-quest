...
### [메모] QA F-1 — **단가는 고쳤는데 그 단가로 계산한 값을 안 고쳤다** (같은 병의 4번째)

퀴즈 Q4 에서 노드 단가를 `$0.0258 → $0.0283/h` 로 정정하면서, **그 단가에서 파생된**
*"50분 세션 차액 $0.021"* 은 그대로 뒀다. 같은 문서 안에 두 값이 공존했다.

| 회차 | 무엇을 고쳤나 | 무엇을 빠뜨렸나 |
|---|---|---|
| 08-31 "정정" | medium 단가 실측 | IPv4 하나 더 |
| 09-03 "재정정" | IPv4 포함 | 루트 EBS |
| 09-04 퀴즈 Q4 | 루트 EBS 포함 | **파생값(차액 $0.021)** |
| 09-04 QA F-1 | 파생값 4곳 | — |

🔑 **정정은 "숫자를 바꾼 곳"에서 멈추기 쉽다.** 그 숫자로 계산된 값은 다른 문장에 있어서
같은 grep 에 안 걸린다. 👉 **단가를 고치면 그 단가가 등장하는 곳이 아니라
`그 단가로 계산한 것`을 찾아야 한다.**
📌 QA 가 3곳을 찾았고 오케스트레이터가 4번째(`CONTEXT.md` 226행, 큰 테이블 행 안)를 추가로 찾았다.

~~**둘 다 세었기 때문에 합쳐서 완결됐다.**~~
🔴 **이 문장이 거짓이었다 (재QA F-3 이 반증).** 실제로는 **5곳**이었다 — `CONTEXT.md` **241행**
(`다음 작업` 트래커)에 같은 값이 하나 더 있었고, 226행만 고치고 "완결" 을 선언했다.
그 뒤 6번째(`review-ledger.md` L-43 의 *"세션당 +$0.021"*)까지 나왔다. **총 6곳.**
...
| 09-04 QA F-1 | 파생값 3곳 + 자체 발견 1곳 | **CONTEXT 241행 · 원장 L-43** |
| 09-04 QA F-3 | 나머지 2곳 | — (전수 열거로 확인) |

🔑 **네 번 연속으로, 정정하면서 "이번엔 전부" 라고 적었고 네 번 다 틀렸다.**
공통 원인은 하나다 — **세어보기 전에 완결을 문장으로 먼저 썼다.**
👉 규칙: *"전부"* 라는 단어를 쓰려면 **그 앞줄에 열거 결과가 있어야 한다.**
마지막 회차에서야 그렇게 했다(취소선 밖 11건을 하나씩 분류해 나열한 뒤 판정).
