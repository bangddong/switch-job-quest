...
| 🔴 **영속 EBS** `vol-0518b6d0dcd2b0d70` 10GiB gp3 | **2026-07-31** | **없음 (destroy해도 남음)** | **≈ $0.91/월** | **$2.6710 소진 / $200 (1.34%)** (09-09 기준 산정) |

> ⚠️ **위 표의 `$2.6710` 은 *산정* 이지 *실측* 이 아니다 (2026-09-09, QA F-2 로 갱신).**
> **마지막 실측은 08-12 의 `$1.0391`** 이고, 그 뒤 세션(08-31·09-03·09-06·09-08·09-09)은
> 각 세션의 과금 시간 × 단가를 더한 값이다. 단가표가 틀렸거나 세지 않은 리소스가 있으면 어긋난다.
> → **다음 세션 시작 시 `RECORD_TYPE=Usage` 로 한 번 조회해 산정치와 대조할 것**(무료, 10초).
> 벌어져 있으면 산정 모델에 빠진 항목이 있다는 뜻이다.
>
