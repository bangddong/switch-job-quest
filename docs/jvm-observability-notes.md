...
- ~~**잔여 여유 25.4 MiB(16%)**~~ → 🔴 **재측정: 여유 21.2 MiB(13%) (2026-09-18).**
  ...
  **조건은 *"`be/**` 배포 없이 ~85일"*** — 재배포가 JVM 을 리셋한다. 관측된 최대 배포 공백은 15일이라
  당장은 🟢 지만, 이 레포는 **#361(08-06) 이후 `feat` 커밋이 0건**이다.
...
- 🔑 **그래서 이 수치가 prod→K8s 이관의 선행 조건을 직접 판정한다** — K8s 노드엔 swap 이 없다.
  ...
  ***핵심은 "swap 이 없다" 가 아니라 "Fly 의 앱 상한(409)이 K8s limits(576)보다 작아서
  ...
  > 🔴 **재판정 (2026-09-24) — 이 비교도 축이 틀렸다.** 위는 **용량**을 재는데, 중요한 것은
  > ***지금 발자국이 각 RAM 천장에 들어가는가*** 다. 표본 3개 실측(동일 JVM, 재시작 없음):
  > ```
  > 총 익명 발자국(RSS + swap)   09-18 419.70  →  09-24 428.47 MiB   (+0.063 MiB/h)
  > Fly  RAM 천장 409  →  ❌ 안 들어간다 — 그래서 53.6 MiB 가 swap 에 나가 있다
  > K8s  RAM 천장 576  →  ✅ 들어간다, 여유 147.5 MiB
  > ```
  > ***Fly 는 swap 이 있어서 여유로운 게 아니라, RAM 천장이 발자국보다 낮아서 swap 이 필수인 것이다.***
  > 🟡 단 cgroup v2 `memory.max` 는 page cache 도 세므로 147.5 가 그대로는 아니다(부호는 안 바뀐다).

  > 🔴 **처음엔 `459 + 256 = 715` 로 적어 격차를 139 MiB 로 과대평가했다 (QA F-1).**
  > `459` 는 **VM 총 메모리**(비앱 오버헤드 ~50 포함)이고 `576Mi` 는 **컨테이너 전용 한계**다 —
  > **층위가 달랐다.** 이 절이 이미 확립한 앱 레벨 상한(`anon-rss ~409`, 커널 로그 8건 전수)에
  > 맞춰야 사과 대 사과가 된다. ***내 실측값은 맞았고 비교 대상을 잘못 골랐다.***
