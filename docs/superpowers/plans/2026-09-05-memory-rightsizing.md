...
| 지표 | 출처 | 왜 |
|---|---|---|
| p50/p95/p99 지연 | k6 | 서버가 힘든지의 **1차 지표** |
| `dropped_iterations` | k6 | **생성기 병목 판별** |
| CPU 사용 | cgroup `cpu.stat` `usage_usec` 델타 | 부하 강도의 독립 지표 |
| **CPU 스로틀** | `cpu.stat` `nr_throttled`·`throttled_usec` | 🔑 지연 급등이 **메모리가 아니라 CPU 제한** 때문인 경우를 가른다 |
| GC 빈도 | `kubectl logs \| grep -c Pause` 델타 | 힙 압박의 직접 신호 |
