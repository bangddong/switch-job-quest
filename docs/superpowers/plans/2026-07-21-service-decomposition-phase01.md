...
- **롤백 가능**: 피처플래그(`devquest.ai.transport=inprocess|http`)로 언제든 in-process 복귀. Phase 1 머지
  후에도 프로덕션 기본값은 검증 완료까지 in-process.
...
- [ ] **Step 1:** `AiCallLog` 테이블 **소비처 조사** — 읽는 화면/쿼리/집계가 core에 있나?
      (없음 → A 단독 확정 / 있음 → A+C 병행)
- [ ] **Step 2:** 조사 결과 + 최종형(A 단독 / A+C)을 하단 "결정 로그"에 기록 (구현은 Task 1.3)
- [ ] **검증:** 코드 변경 없음(조사·결정만). 소비처 목록을 근거로 첨부
...
독립 기동 · 피처플래그 기본 inprocess. **아직 단일 배포, HTTP 미사용.**
...
- Edit: `core-api` — `client-ai` **컴파일 의존 제거**(HTTP 어댑터로 대체), `AiCheckService` 등 트랜잭션 경계 점검
...
- [ ] **Step 2:** 어댑터 구현 + `client-ai` 컴파일 의존 제거(런타임은 ai-api HTTP)
...
- [ ] **Task 0.2 결과:** AiCallLog 소비처 조사 목록 + 최종형(A 단독 / A+C)
