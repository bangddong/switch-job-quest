...
> 🔴 **여기 있던 2대 용량 계산·배치·"착수 전 필수" 블록은 D-011 (2026-09-03) 로 대체돼 삭제했다.**
> 취소선으로 남기지 않은 이유: 그 본문이 **틀린 주장 두 개**를 담고 있어 바로 아래 D-011 과
> 정면으로 충돌했고(QA F-2 지적), 50여 줄을 취소선으로 덮으면 읽을 수 없게 된다.
> 지운 내용의 요지와 **왜 틀렸는지**는 D-011 본문에 그대로 옮겨 적었다:
>
> | 지운 주장 | D-011 의 판정 |
> |---|---|
...
> | *"`node_max_size` 는 이미 2 다 — desired 만 바꾸면 된다"* | 이제 **3** 이다(이 PR). `-var node_desired_size=3` |
>
> 원문은 git 이력에 있다(이 문단을 만든 커밋의 부모).
