...
> 항목 3 의 `sed` 관련 사건 2건은 **둘 다 다른 실패 모드**다 — ①마스킹 파이프의 `sed` 가 `pipefail` 로
> 안 잡히는 침묵 실패(#426 QA F-5) ②`tofu output` 이 빈 값을 종료코드 0 으로 낼 수 있어 **미리 넣은**
> `case` 형태 검사(`eks-tutorial-steps.md:2035-2074`). *"문서가 지시를 빠뜨렸다"* 와 다르다.
>
