...
> 이관하면 그 방어선이 사라진다.
>
> 🔴 **재판정 (2026-09-24) — B-5 는 `✅해소` 다. 방향은 맞았고 긴급도가 54배 과대였다.**
> ```
> Fly  용량 665 (409 + swap 256)  발자국 428.47  여유 236.53  런웨이 157일 / 64일
> K8s  용량 576 (swap 0)          발자국 428.47  여유 147.53  런웨이  98일 / 40일
> → K8s 가 더 좁다는 방향은 옳다. 틀린 것은 "런웨이 2.6일"(3 MB/h 가정, 실측 0.063 = 54배 과대).
> ```
> 배포 간격이 최대 15일이라 **40일 런웨이로도 충분**하다 → **⑤ 현행 유지**.
>
> ⚠️ **나는 이 재판정을 한 번 틀리게 썼다** — *"발자국이 Fly RAM 천장 409 엔 ❌, K8s 576 엔 ✅ 이므로
> **방향이 뒤집힌다**"*. Fly 에만 RAM-only 천장을, K8s 에는 총량을 댄 **비일관 비교**였고 QA 가 HIGH 로
> 잡았다. ***swap 은 임시방편이 아니라 `#245` 에서 의도적으로 넣은 방어선이고 지금 작동 중이다.***
>
> 🔑 **살아남는 관찰**: 전환하면 지금 swap 에 있는 **53.55 MiB 가 전부 resident** 가 된다.
> **Fly 의 `RSS`(374.9)만 보고 K8s `limits` 를 잡으면 53.6 MiB 과소산정**이다.
> 상세·표본: 「항목 4 — 종료」(2026-09-24).
...
⚠️ **별건 — 소스 주석 2곳의 오프바이원** (2026-09-19 전수 확인 · **2026-09-25 해소**):
...
> ✅ **해소 — 2026-09-25, `fix/ai-stub-port-count`.** 둘 다 `16` 으로 고치고, 재발 경로를 닫기 위해
> **모집단을 문장 안에 인터페이스 이름으로 박았다**(`AI 포트` → `AiEvaluatorPort`).
>
> 🔴 **그 과정에서 내가 이 지적을 한 번 잘못 철회했다. 그 오답을 남겨둔다 — 형태가 반복되기 때문이다.**
> 나는 *"`TechInterviewStubEvaluator:15` 의 `17` 은 맞다"* 고 판정했다. 근거는 **`AI 포트` 라는 명사가
> 레포에서 18을 뜻한다**는 것이었고(`BaseAiHttpAdapter.kt:14`·`AiTransportConfig.kt:65`·
> `AiTransportSwitchTest.kt:69`), 18 − 스텁 1 = 17 이므로 맞다고 썼다.
>
> **QA 가 두 가지로 깼다.**
> ⓐ 그 용어는 **일관되지 않는다** — `Judge0HttpAdapter.kt:13` 은 *"17개 AI 포트만 HTTP로 뽑히고
>   **Judge0만** 영구히 in-process에 남는"* 이라며 같은 말을 **`Judge0Port` 를 제외한 17**로 쓴다.
> ⓑ 🔑 **결정적인 것 — 그 문장은 자기 술어에 모집단을 이미 담고 있었다.**
>   *"…스텁되지 않는다 — 여전히 실제 **`ANTHROPIC_API_KEY`가 필요하다**"*. 그런데
>   `Judge0Adapter.kt:12,37` 은 `devquest.judge0.api-key` → `X-RapidAPI-Key` 다. **`Judge0Port` 는
>   ANTHROPIC 키를 쓰지 않으므로 이 문장의 대상 집합에 들어올 수 없다.** ⇒ 대상 = 17 − 1 = **16**.
>
> ***나는 모집단을 문장 밖(다른 파일의 명사 용례)에서 찾았고, 문장은 자기 안(술어)에 답을 갖고 있었다.***
> 바로 위에 📌 *"모집단을 먼저 확정한다"* 를 써놓고 **그 확정을 엉뚱한 곳에서 했다.**
> 📌 관례 (교정판): ***개수를 인용한 문장의 모집단은 그 문장의 술어로 확정한다 — 같은 명사의 다른 용례로
> 확정하지 않는다.*** 명사는 흔들리고 술어는 검증 가능하다.
>
> 🔴 **이것은 #435 F-1 과 같은 계열의 두 번째 사례다**(거기서는 Fly 에 RAM-only 천장을, K8s 에 총량을
> 대고 결론을 뒤집었다). **공통 형태 = 뒤집을 때 비교 기준을 내가 마침 본 쪽에서 가져온다.**
>
> 부산물: **`AI 포트 N개` 라는 표현은 이 레포에서 쓸 수 없다**(두 뜻으로 쓰인다) → 원장 등재.

> 🔑 **착수 조건이 비용에서 값으로 뒤집혔다 (2026-09-25).** 아래 *"다음 `be/` 변경에 묶는다"* 는
> **해소됐다** — `be-cd` 재배포가 당시엔 순비용이었지만, 2026-09-24 실측으로 **메타스페이스 87.6% ·
> 상한 도달 88일**이 열렸고 백로그 완화 선택지 ⓒ가 *"배포/재시작 주기 보장"* 이다.
> **재배포가 JVM 을 리셋해 그 카운터를 0으로 돌리므로 지금은 재배포 자체가 값이다.**
...
> (**소스가 틀렸다**는 뜻이고, 인용이 맞다는 뜻이다 — ⚠️ **이 문장을 *"그러므로 17이 맞다"* 로 읽지 말 것.
> 2026-09-25 에 내가 정확히 그렇게 오독했다.** 위 해소 블록 ⓑ 참조).
...
#### ~~⚠️ `AiStubConfig.kt:28` 주석 오프바이원 — **이 PR 에 담지 않는다**~~ → ✅ **해소 (2026-09-25)**

~~①-b 의 별건(*"나머지 **17개** `AiEvaluatorPort`"* → **16**)은 한 단어 수정이지만 `be/**` 다.~~
~~**`be-cd` 가 main push 에 걸려 Fly prod 를 재배포한다.** 주석 한 줄을 고치려고 재배포하지 않는다.~~

`fix/ai-stub-port-count` 에서 **2곳 모두** 고쳤다. 상세·재판정 경위는 ①-b 의 해소 블록 참조.
착수 근거는 **메타스페이스 87.6%(상한 88일)** — 재배포가 JVM 을 리셋하므로 이제 재배포가 값이다.
...
### ③ 🔴 B-5 재판정 — **방향은 맞았다. 틀린 것은 긴급도(54배)다**

> 🔴 **이 절은 한 번 틀리게 썼고 QA 가 HIGH 로 잡았다 (F-1).** 처음에 *"비교 축이 틀렸다 —
> 발자국 428.5 가 Fly RAM 천장 409 에는 ❌, K8s 576 에는 ✅ 이므로 **방향이 뒤집힌다**"* 라고 썼다.
> **비일관 비교였다** — Fly 에만 **RAM-only** 천장(409)을 대고 K8s 에는 **총량**(576)을 댔다.
> 같은 축(총 가용량)으로 자기일관되게 다시 계산하면:
>
> ```
> Fly  용량 665 (409 + swap 256)  발자국 428.47  여유 236.53  런웨이 157일(장창) / 64일(단창)
> K8s  용량 576 (swap 0)          발자국 428.47  여유 147.53  런웨이  98일(장창) / 40일(단창)
> → 어느 창을 써도 Fly 가 더 길다. **K8s 가 여전히 더 좁다.**
> ```
>
> ***swap 은 신뢰할 수 없는 임시방편이 아니라 `#245` 에서 의도적으로 넣은 방어선이고 지금 설계대로
> 작동 중이다.*** 그것을 배제할 근거를 나는 제시하지 못했다. **B-5 의 방향은 옳다.**

**그래서 B-5 는 `🔄부분무효` 가 아니라 `✅해소` 다 — 방향이 맞되 여유가 충분하다.**

| | B-5 (2026-09-11) | 실측 (2026-09-24) |
|---|---|---|
| 방향 | K8s 가 더 좁다 | ✅ **맞다** (여유 147.5 vs 236.5) |
| 격차 | 89 MiB | ✅ 맞다 (665 − 576) |
| **긴급도** | **런웨이 2.6일** (3 MB/h 가정) | 🔴 **40~98일** — **가정이 54배 과대였다** |

배포 간격이 최대 15일이므로 **40일 런웨이로도 충분**하다. → **⑤ 현행 유지**.

### ③-2 그래도 살아남는 관찰 — **전환 시 swap 이 resident 로 바뀐다**

위 재판정으로 *"방향이 뒤집힌다"* 는 죽었지만, 그것을 쓰게 만든 관찰 하나는 유효하다.

```
지금 Fly     RSS 374.92  +  swap 53.55  =  발자국 428.47
전환 후 K8s  swap 이 없으므로 **53.55 MiB 가 전부 resident** 가 된다 → RSS ≈ 428.47
```

🔴 **Fly 의 `RSS` 만 보고 K8s `limits` 를 잡으면 53.6 MiB 를 과소산정한다.**
현재 576Mi 는 428.5 를 담지만, *"Fly 에서 375 니까 448Mi 면 되겠다"* 는 판단은 **즉시 OOMKill** 이다.
이관 계획서에서 limits 를 손댈 때 이 함정을 먼저 볼 것.

🟡 그리고 cgroup v2 의 `memory.max` 는 anon 만이 아니라 **page cache 도 센다** — 여유 147.5 는
상한이지 실제값이 아니다(부호가 바뀔 크기는 아니다).
...
> 🔴 **여기서 근거를 하나 조작할 뻔했다 (QA F-2, MEDIUM).** 처음엔 *"다만 근거가 약하다 — 이 레포는
> **#361(08-06) 이후 `feat` 커밋이 0건**"* 이라고 썼다. **거짓이다.** `git log --since=2026-08-06 -- be/`
> 에 `feat(be)` 가 **3건** 있다(#395 08-27 · #397 08-28 · #403 08-31).
> ***그 커밋들은 바로 윗줄 배포 간격표에 내가 직접 나열해둔 것들이다*** — 같은 문단 안에서
> 데이터와 주장이 모순됐다. 출처는 `CLAUDE.md` 하네스 동결 규칙인데 **그 문구 자체가 stale** 하다
> (2026-08-16/17 작성 후 `be/` 에 feat 이 계속 들어왔다). 🔑 ***레포 문서를 인용할 때도 날짜를 본다 —
> `CLAUDE.md` 라고 해서 현재형인 것은 아니다.*** `CLAUDE.md` 정정은 하네스 동결 규칙에 걸리고
> **내가 먼저 꺼낸 것**이라 원장으로 넘긴다.

정정 후 판정은 **오히려 더 안전한 쪽**이다 — 배포가 최근까지 꾸준했으므로 85일 공백은 관측 밖이다.
소유는 `.claude/CONTEXT.md` 제품 백로그.
...
### ⑦ QA 가 잡은 것 (2026-09-24)

| ID | 등급 | 내용 |
|---|---|---|
| **F-1** | 🔴 **HIGH** | **B-5 재판정이 비일관 비교였다.** Fly 에만 RAM-only 천장(409)을, K8s 엔 총량(576)을 댔다. 같은 축으로 재계산하면 **Fly 여유 236.5 > K8s 147.5** 로 **원래 방향이 유지**된다. → ③ 을 *"방향이 뒤집힌다"* 에서 *"방향은 맞고 긴급도가 54배 과대"* 로 교체, B-5 를 `🔄부분무효` → **`✅해소`** 로 |
| **F-2** | MEDIUM | *"#361(08-06) 이후 `feat` 0건"* 인용이 **거짓**. `feat(be)` 3건(#395·#397·#403)이 있고 ***그 커밋들은 바로 윗줄 배포 간격표에 내가 직접 나열해둔 것들이다.*** 출처 `CLAUDE.md` 문구가 stale → **원장 L-65** |
| **F-3** | LOW | ④ 의 귀류법이 `§2`(*Serial 은 상한 도달 시에만 언로드*)에 **암묵 의존**. 명시 교차인용 추가 |

🔴 **F-1 은 이 세션 첫 HIGH 이고, 형태가 앞의 것들과 다르다.** 세기·인용 오류가 아니라
***내가 기존 결론을 뒤집으면서 뒤집을 만한 비교를 하지 않은 것***이다 — 한쪽엔 좁은 잣대(RAM only),
다른 쪽엔 넓은 잣대(총량)를 댔다. 같은 축으로 재면 원래 결론이 그대로 섰다.
📌 **관례: 기존 결론을 뒤집을 때는 *새 값*이 아니라 *같은 축으로 잰 두 값*을 나란히 놓는다.**

🔴 **F-4 (MEDIUM, 2라운드) — 가장 무거운 것은 이것이다. QA 가 *나를* 잡았다.**
***나는 세 건을 `fixed` 로 스스로 찍고 나서 재QA 를 돌렸다 — 검사기에 정답을 먼저 보여준 것이다.***
규정은 `fixed` 의 조건을 *"재QA 로 해소 확인"* 으로 못박아뒀으므로 **재QA 이전의 `fixed` 는 적법하지 않다.**
QA 가 mtime 으로 검출했다(내가 밝힌 게 아니다).

🔑 **#431 과 같은 형태다.** 그때는 내 오류를 Blindspot 프롬프트의 *"다시 조사하지 말 것"* 절에
전제로 주입해 **위임의 가치인 독립 검증을 내가 껐다.** 이번엔 프롬프트가 아니라 **파일**로 같은 일을 했다.
***실해는 판정이 아니라 앵커링이다*** — 이번엔 QA 가 독립 재검산해 결과가 맞았지만,
**결과가 맞았다는 것이 절차가 맞았다는 뜻은 아니다.** → 원장 **L-66**(하네스 동결, QA 가 꺼냄).

⚠️ 그 `sed` 는 **python 이 SyntaxError 로 죽은 직후 그대로 실행됐다**(`&&` 미사용).
***같은 세션 두 번째다*** — TASK-11 번호 중복 때 거짓 커밋 메시지를 만든 것과 **동일한 기전**이다.
📌 **관례: 스크립트와 그 결과를 쓰는 명령을 `&&` 로 묶는다. 실패는 전파돼야 한다.**

🔑 **F-2 가 더 아프다.** 근거가 된 `git log` 출력을 **내가 직전에 직접 찍어 화면에 띄워놓고**,
그 옆에 그것과 모순되는 문장을 적었다. *"출처를 안 보고 기억을 썼다"* 가 아니라
***출처를 보고도 기억을 썼다.***
한 번의 조회로 묶을 수 있으면 묶는다 — 간격을 나중에 추정하면 반드시 틀린다.
