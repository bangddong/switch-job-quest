...
2. **노드 용량** 🟡 — ~~Spring 앱 ~300~450MB × 3 ≈ 1~1.4GB + 시스템 파드 → **t4g.small(2GB) 빠듯**.
   ...
   >
   > 🔴 **재정정 (2026-09-03, D-010·D-011) — 위 문단 전체가 무효다. 셋 다 해소됐고, 하나는 틀렸다.**
   >
   > | 위 서술 | 지금 |
   > |---|---|
   > | *"Allocatable 실측 0건"* | ✅ **실측 (08-31)**: capacity 1841Mi / **allocatable 1365Mi** / pods 11. 시스템 파드 requests 406Mi → **노드당 가용 959Mi** |
   > | *"t4g.medium 단가 근거 미기재"* | ✅ **실측**: `$0.0416/h` = small 의 정확히 2배. ⚠️ 단 **이 계정은 medium 을 launch 하지 못한다**(신 Free Tier 플랜, D-010) → 파드 상한 17 은 **잴 의미가 없다** |
   > | 🔴 *"노드 2개는 **파드 슬롯만** 푼다 — 메모리 벽은 그대로"* | **틀렸다.** 파드 하나(512Mi)는 어차피 한 노드(959Mi)에 들어간다. 스케일 아웃이 푸는 것은 **총량**이고, 실제로 **파드 슬롯은 애초에 제약이 아니었다**(11칸 중 6칸 여유). 막은 것은 메모리 **하나**다. 이 문장 때문에 D-009 가 "노드 2대" 를 기각했고, 그 기각이 **08-31 유료 세션을 통째로 날렸다** |
   > | *"순증 11 이 아니라 8 / 7"* | 사실이나 **무의미하다** — 슬롯은 남아돈다. 현재 재개 경로는 **노드 3대**(D-011) |
   >
   > 👉 현재 유효한 판단은 `plans/...-phase02.md` 의 **D-011** 이다.
   > 상세는 같은 문서 §Stage C 착수 블로커.
