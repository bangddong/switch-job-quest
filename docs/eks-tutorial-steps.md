...
| **2026-09-09** | **유료 완전 재현** (31분 34초, $0.0716) — 이 문서**만** 보고 Stage 0→3b 최종 상태까지. 막혀도 코드를 열지 않는다는 규칙을 지켰다 | **조건부 통과 — 결함 2건 발견·수정.** 도달했고 코드를 열어야 했던 지점은 **없었다.** 다만 그대로 따랐을 때 **오판을 유발하는** 결함이 2건 있었다: ①`db_mode` 확인이 거짓 양성(→ RDS 실수 기동 유도) ②3b-7 기대 출력이 이 설정에서 나올 수 없음. 둘 다 같은 세션에서 수정 |
...
> ⚠️ **이 검사에도 한계가 있다 — 적어둔다 (QA F-3).** `grep -q` 는 plan 출력에 그 문자열이
> *있는지*만 본다. 이 문서의 용법(**아무것도 없는 상태에서 새로 짓는** 데모, 그래서
> `0 to change, 0 to destroy` 가 불변식)에서는 등장 = 생성이라 안전하다. 하지만 rds 로 한 번
> 띄운 뒤 in-cluster 로 되돌리는 상황이라면 plan 에 **destroy 대상으로도** 그 이름이 뜨므로
> *"rds 모드"* 로 오탐한다. 그런 왕복을 할 거면 `resource_changes` 의 `change.actions` 를
> 봐야 한다. **이 자리에서 검사가 갈린 게 벌써 세 번째다**(개수 → `grep -c` → 유무) —
> 다음 사람이 네 번째를 만들지 않도록 한계를 먼저 적는다.
>
...
⚠️ **파이프에 `pipefail` 이 없으므로 위 형태 검사가 유일한 방어선이다.** 치환 실패 시
`CERT_ARN_PLACEHOLDER` 문자열이 그대로 ALB 에 전달되고, 그것도 조용히 실패한다.
