...
variable "node_instance_type" {
  ...
  #              daily-api 차례에 A 423 · B 477 로 **둘 다 부족 → Pending**
  #
  #     👉 **3대는 비관 가정에서도 성립한다** = 결과가 미측정 값에 의존하지 않는다.
  #        2대는 어느 가정에서도 "안전하다"고 말할 수 없다. 차액은 **50분 세션당 $0.021** 이다
  #        (노드 1대 = EC2 $0.0208/h + 공인 IPv4 $0.005/h = $0.0258/h, 둘 다 pricing API 실측).
