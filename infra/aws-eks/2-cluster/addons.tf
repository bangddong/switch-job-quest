...
resource "aws_eks_addon" "ebs_csi" {
  ...
  #
  # 🔄 **정정 (2026-08-31, D-009 로 노드가 t4g.medium 이 됐다)** — 위 계산의 전제가 바뀌었다.
  #   실측(`aws ec2 describe-instance-types`, 08-31): t4g.medium = ENI 3 × (IPv4 6 − 1) + 2 = **17**.
  #   Stage C 예상 사용량: 시스템 4(aws-node·kube-proxy·coredns×1) + EBS CSI 2 + ESO 3
  #                        + 앱 4(postgres·core-api·ai-api·daily-api) = **13/17**.
  #   → 여유 4. **replicaCount=1 을 유지할 이유가 "상한에 부딪힌다"에서 "여유를 남겨둔다"로 약해졌다.**
  #   그래도 되돌리지 않는다: ①노드 1대에서 coredns 2개는 어차피 HA 가 아니다(같이 죽는다)
  #   ②여유 4칸은 검증용 임시 파드·롤아웃 여유로 쓰는 편이 낫다.
  #   🔑 **노드를 2대 이상으로 늘리면 이 판단이 뒤집힌다** — 그때는 coredns 를 2로 되돌릴 것(기존 규율 유지).
