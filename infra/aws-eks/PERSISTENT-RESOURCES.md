...
> 🔴 **행선지 갱신 (2026-09-22)**: 선행 조건 7 은 **선행 조건에서 제외**되어 이관 계획서로 갔다.
> 이 항목은 `plans/2026-09-11-prod-eks-migration-prereqs.md` 「항목 7 — 착수 설계」⑥ **U-T** 에
> 등재했다. 🔑 **계획서 항목 7 행은 이 두 줄을 한 번도 참조한 적이 없었다** — 레포가 항목 7 을
> 직접 호명한 곳은 여기 2줄 + 원장 L-50 + `0-bootstrap/variables.tf` 인데 전부 단방향이었다.
>
> ✅ **전이를 양방향으로 실측했다 (2026-09-16 유료 세션).**
> ```
> Ingress apply 후   Renewal: ELIGIBLE     InUseBy: 1
> teardown 후        Renewal: INELIGIBLE   InUseBy: 0     ← 되돌아온다
> ```
> 🔴 **되돌아오는 것이 핵심이다.** destroy-after-use 모델에서는 ALB 가 세션 중에만 존재하므로
> 이 인증서는 **거의 항상 `INELIGIBLE`** 이다. 즉 *"붙이면 갱신된다"* 가 위안이 되지 못한다 —
> 갱신 시점(만료 60일 전)에 클러스터가 꺼져 있으면 갱신이 시도되지 않는다.
> → ~~선행 조건 7~~ **이관 계획서** 에서 **①상시 ALB 를 두거나 ②만료를 캘린더로 감시하거나 ③재발급을 절차화**
> 중 하나를 택해야 한다. 재발급은 $0 이고 Cloudflare 작업 2분이므로 ③이 가장 싸 보인다(미결정).
>
...
### 🔴 반복해야만 만나는 **실패 6종** — 번호의 정의는 여기가 유일하다

> **이 레포의 23곳이 *"실패 6종 ②"* 처럼 번호로만 참조한다**(`.tf` 3 · `k8s/` 9 · 문서 11).
> 원래 `.claude/CONTEXT.md` 에 있었으나 2026-09-18 에 여기로 옮겼다 — **참조가 가장 밀집한 곳**이고,
> 아래 ⑥ 경고가 이미 이 파일에 있었다. ***요약하거나 번호를 바꾸지 마라. 23곳이 해독 불가가 된다.***
>
> 🔑 **문서만 읽어선 안 잡힌다.** 그리고 보통 학습자는 클러스터를 부술 이유가 없어 이 경험 자체를
> 못 한다(kind 는 EBS 가 없고, 회사에선 플랫폼팀이 소유). 우리는 비용 때문에 **어차피 매번 부수므로**,
> 그 사이클에 볼륨 재바인딩을 얹으면 **추가 작업 없이 매 세션 연습**된다 —
> destroy-after-use 규율이 희소한 반복 기회를 공짜로 만든다.

| # | 실패 | 증상 |
|:-:|---|---|
| **①** | **AZ 불일치** | 파드 **영구 Pending**. 노드가 매번 같은 AZ 에 안 뜬다 — **운 나쁜 날에만** 터진다 |
| **②** | `reclaimPolicy: Delete` 실수 | 볼륨 **동반 삭제** |
| **③** | PV `volumeHandle` ↔ 실제 volume ID 불일치 | attach 실패 |
| **④** | **`claimRef` 잔존** | PV 가 새 PVC 를 안 받음 — **최다 정체 지점** |
| **⑤** | StatefulSet `volumeClaimTemplates` 가 PVC 자동 생성 | static PV 와 충돌 |
| **⑥** | 기존 볼륨을 **포맷** | **복구 불가** |

⚠️ **⑥이 유일한 비가역 사고다.** 트리거는 PV 의 `csi.fsType` 을 `ext4` 에서 바꾸는 것 — **바꾸지 마라.**
