...
resource "aws_acm_certificate" "learning" {
  ...
  #
  #    ⚠️ **처음엔 *"배선은 하네스 동결 규칙에 걸려 안 한다"* 고 적었는데 규칙 오적용이었다**
  #       (QA F-6). 동결 규칙의 문언은 **`.claude/`·`.github/`** 이고 `docs/eks-session-sop.md`
  #       는 `docs/` 라 **범위 밖**이다. 규칙을 문언 확인 없이 인용해 *"필요한 일을 안 하는
  #       핑계"* 로 쓴 꼴이 됐다.
  #
  #    실제 이유는 **범주**다: SOP §9 는 *"고아 전수 검증 **= 0**"* 이고 거기 담기는 것은
  #    **세션이 만든 뒤 남으면 안 되는 것**(클러스터·ALB·NAT·스냅샷)이다. ACM 인증서는
  #    세션이 만들지 않고 **항상 1건 존재하는 것이 정상**이라, §9 에 넣으면 매 세션 오탐이 난다.
  #    그건 `ebs-postgres.tf` 가 이미 경고한 실패 모드다 — *"오탐이 반복되는 검사는 곧
  #    무시된다 → 진짜 고아도 함께 놓친다"*.
  #    올바른 집은 `PERSISTENT-RESOURCES.md` 의 **§확인 명령 ①(영속 인벤토리)** 이고,
  #    이 PR 에서 `aws acm list-certificates` 를 **거기에 이미 추가했다.**
  tags = {
