...
| **4** — AWS LB Controller → ALB Ingress | 🚧 **코드만** | 코드 2026-09-11 | **IaC·매니페스트·안전장치 작성 완료, 유료 검증 미실시.** IRSA(`irsa-alb.tf`) + Ingress(`k8s/base/ingress.yaml`, path 2개) + 리퍼 ALB 회수 + 회귀 11건($0). 착수 전 Blindspot Pass 로 전제 3건 정정: ①서브넷 `role/elb` 태그 **이미 있음** ②LBC 는 **애드온 아님**(실측 `None`) → helm ③두 서비스가 `/api/v1/daily-question` 을 **겹쳐 서빙**(strangler 잔재) → 순서 의존 라우팅. 그 뒤 추가 발견: 공식 IAM 정책 **8955자 > 상한 6144자**라 `jsonencode(jsondecode())` 필수 <!-- verify: infra/aws-eks/2-cluster/irsa-alb.tf ~ policy[[:space:]]*=[[:space:]]*jsonencode\(jsondecode --> |
