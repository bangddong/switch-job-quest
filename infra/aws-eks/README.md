...
> 🔴 **정정 (2026-09-23) — 위 숫자는 QA F-1 이 기각한 값이다.** `459` 는 **VM 총 메모리**(비앱
> 오버헤드 ~50 MiB 포함)이고 `576Mi` 는 **컨테이너 전용 한계**라 **층위가 다르다**. 앱 레벨
> 상한(`anon-rss ~409`, 커널 OOM 로그 8건 전수)에 맞춰야 사과 대 사과다:
> **Fly 665(409 + swap 256) vs K8s 576 → 89 MiB 적다.** 방향은 같지만 격차가 **1.6배 부풀어
> 있었고**, 그 수치가 대응 선택지 ①(limits 상향폭)의 판단에 직접 들어간다.
> ⚠️ 백분율을 나란히 쓰지 않는다 — 옛 `24%` 는 576 을, 새 `13%` 는 665 를 분모로 쓴다.
...
> 🔎 **교차 참조 (2026-09-22) — 아직 `✅유효` 다. 반전 여부는 미결이다.**
> `docs/superpowers/plans/2026-09-11-prod-eks-migration-prereqs.md` 의 **선행 조건 7**
> (*상시 운영 전제로 안전장치 개정*)이 이 결정의 반전을 전제하고 있었다. 항목 7 은
> **이관 계획서로 이관**됐고, 그 계획서의 **0번 항목**이 바로 이 결정의 재판정이다.
> 🔴 **그 계획서는 D-002 를 모른 채 크레딧 수명을 독립으로 다시 계산했고 같은 값을 냈다**
> (≈1.4개월 vs 여기 5~7주). 위 ⚠️ 경고 — *"크레딧 잔액이 남아 보일 때 특히 흔들린다"* — 가
> 정확히 발화한 사례다. **반전하려면 `design-change-procedure.md` 전 단계**이며, 위 표의
> 숫자를 다시 실측할 것(오늘 기준 잔여 ≈$194, 플랜 만료 2027-01-15).
