...
| **#423** | **공개 Actions 로그·요약에서 AWS 계정 ID 마스킹 ($0).** 🔴 **#422 머지가 유출을 드러냈다** — apply 로그에 `[id=arn:aws:acm:...:<계정ID>:certificate/...]`. **`output` 에 `sensitive = true` 를 걸었는데도 나왔다**: sensitive 는 `Outputs:` 블록만 가리고 저 줄은 **provider 가 찍는 리소스 `id`** 다 → ***ARN 을 갖는 모든 리소스가 같은 경로로 샌다.*** 내 sensitive 판정은 **틀리지 않았지만 충분하지 않았다.** 처음 새는 것도 아니었다(Blindspot U-4 의 `ecr_repository_urls`). 유출 3경로(apply 로그 · plan→Step Summary · ECR IMAGE_URI→Summary). 🔑 **`::add-mask::` 만으로 부족** — **Step Summary 는 마스킹 대상이 아니라고 보고** 로그는 add-mask, 요약은 `sed -u`/파라미터 확장으로 **각각** 지운다. ⚪ **이 전제는 검증 못 했다**(GH 런타임 동작 · QA 도 `curl` 차단) → 숨기지 않고 주석·일지에 ⚪ 표기. 틀려도 **과잉 마스킹**이라 안전한 방향. 🔑 **fail-open 을 반증으로 잡았다**: 변수가 비면 `sed` 가 치환 0건에 **종료코드 0** — *부재가 성공과 똑같이 생긴 실패* → 12자리 검증 후 `exit 1`(반증 6종). 🔴 **그런데 내가 넣은 안전장치가 회귀를 만들었다 (QA F-1)** — `sed` 는 파이프 출력일 때 **full buffering** 이라 apply 로그가 실시간으로 안 흐른다(실측 `sed` 28.3/28.3/28.3 vs `sed -u` 28.3/29.3/30.3). 기존 `tee` 는 line-buffered 라 멀쩡했는데 **내가 앞에 끼워서 깼다.** 🔑 ***과금 중 진행 상황을 못 보는 것은 실제 비용이다 — 안전장치를 넣을 때 관측 가능성을 깨뜨리지 않았는지 보라.*** 🔑 **부수: `| tee` 가 실패를 삼킬 수 있었다**(파이프는 마지막 명령 종료코드만 낸다) → `set -euo pipefail` 인라인. ⚠️ 처음엔 *"GH 기본 셸에 pipefail 이 없다"* 를 근거로 썼는데 **철회**(QA F-3) — 로컬 bash 플래그 비교는 **러너의 invocation 을 증명하지 않는다.** ***런타임 기본값에 기대는 대신 스크립트가 직접 켜면 불확실성이 사라진다.*** 🔴 **검사 도구가 먼저 틀렸다**: zsh 에서 재다가 `:c` 가 **zsh 파라미터 수식자**로 먹혀 *"sed 가 문자를 먹는다"* 로 오독했다 — #417 의 *"반증 하네스의 `ROOT` 가 `/` 가 되어 두 arm 모두 통과"* 와 같은 층위. ***러너와 같은 셸로 재라.*** ⚠️ *"6주간 공개"* 도 근거 없어 삭제(QA F-4) — 공개 전환 시점 기록이 레포에 없다. **결정(과거 로그 미삭제)은 기간에 의존하지 않는다.** ⚖️ `.github/` 라 **하네스 동결 규칙 대상**이고 제품 차단 증거 없이 **사용자 지시**로 진행 — **L-54 의 두 번째 사례** | 2026-09-15 |
