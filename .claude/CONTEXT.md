...
| 항목 | 내용 |
|------|------|
| 진행 중인 트랙 | **Phase 2 — `daily-service` 추출**. Stage A ✅ · B ✅ · 🔴 **Stage C 1차 시도 실패 (08-31 유료 세션, $0.129) — 원인은 계정 Free Tier 인스턴스 타입 제한.** `t4g.medium` 은 이 계정에서 **launch 자체가 안 된다**(ASG 5회 `Failed`; ⚠️ `describe-nodegroup` 의 `health.issues` 는 **빈 배열**이라 EKS 층에서 안 보인다 — 노드가 안 뜨면 **ASG 활동 로그를 먼저**). → **D-009 폐기 · D-010 신설 · C-4 재개봉.** 🔑 **레포 기록 0건이던 `Allocatable` 을 쟀다**: t4g.small = capacity 1841Mi / **allocatable 1365Mi** / pods 11. 시스템 파드 requests 406Mi → **노드당 가용 959Mi**. **파드 슬롯은 제약이 아니었다**(11칸 중 6칸 여유). 🔴 **재개 경로 = 노드 3대** (`-var node_desired_size=3`, **D-011 · 09-03 $0 산술로 2대에서 상향**). 이미지 재빌드 불필요. ⚠️ **2대는 안전을 증명할 수 없다** — 비관 가정(모든 노드 여유 959Mi)에서 세 번째 512Mi 가 어디에도 안 들어가 **Pending** 이 되고, 낙관 가정에서도 **스케줄 순서에 따라** 갈린다. **3대는 비관 가정에서도 성립** = 결론이 미측정 값에 의존하지 않는다. 차액은 50분 세션당 **$0.021**. 🔑 **D-010·L-43 의 오류 둘을 함께 정정**: ①*"1024 > 959 즉시 초과"* → 스케줄러는 안 맞는 노드에 **놓지 않는다**. 증상은 OOM 이 아니라 `Pending`, 원인은 **조각화** ②*"`podAntiAffinity` 로 해소"* → **불충분** (시스템 Deployment 배치가 통제 안 됨 → 노드그룹을 쪼개야 함) → **L-43 을 블로커에서 관찰 항목으로 강등.** 📌 남은 미확인: 406Mi 의 **DaemonSet ÷ Deployment 분리**(클러스터 필요 — `describe-addon-configuration` 은 스키마만 주고 기본값을 **안 준다**, 09-03 확인) · **`requests: 512Mi` 자체가 근거 없는 수치**(Fly 512MB 예산 유래, 실측 0건 — 낮출 수 있으면 D-011 도 다시 연다) · **C-5 NetworkPolicy 가 실제로 막는지** |
...
| PR/커밋 | 내용 | 날짜 |
|---------|------|------|
| **#407 · #408** | **★ 착수 전 $0 검산이 계획을 뒤집었다 — Stage C 재개 = 노드 2대 → **3대** (D-011).** 08-31 이 남긴 *"1순위 = 노드 2대"* 를 클러스터 없이 산술로 검산했더니 성립하지 않았다. 406Mi 의 DaemonSet÷Deployment 분리가 **미측정**이라 양극단을 다 따졌다: **비관**(모든 노드 여유 959Mi)이면 2대는 512Mi 세 개 중 하나가 **어디에도 안 들어가 Pending**, **낙관**(A 959·B 1245)이어도 **스케줄 순서에 따라** 갈린다. 🔴 **3대는 비관 가정에서도 성립** = 결론이 미측정 값에 **의존하지 않는다**. 차액은 세션당 **$0.021**(노드 1대 = EC2 $0.0208 + 공인IPv4 $0.005) — *검증 불가능한 스케줄링 제약을 과금 중에 거는 것보다 2센트로 여유를 산다.* 🔴 **틀린 주장 3개를 함께 정정**: ①*"1024>959 즉시 초과"* → 스케줄러는 **안 맞는 노드에 놓지 않는다**. 증상은 OOM 이 아니라 `Pending`, 원인은 **조각화** ②*"`podAntiAffinity` 로 해소"* → **불충분** (시스템 Deployment 배치가 통제 안 됨 → 노드그룹 분할 필요) ③설계서의 *"노드 2개는 **파드 슬롯만** 푼다"* → **이 문장이 D-009 의 2대 기각 근거였고 그 기각이 08-31 유료 세션을 날렸다**(슬롯은 애초에 제약이 아니었다). → **L-43 을 블로커 → 관찰 항목으로 강등**, CONTEXT↔L-43 모순 해소. **$0 부수 실측**: `describe-addon-configuration` 은 `resources` **스키마만** 주고 기본값을 안 준다(406Mi 분리는 $0 경로 **없음**) · ESO 3파드 requests 는 **비어 있고**(BestEffort → 압박 시 **가장 먼저 evict**) helm 수동설치라 **406Mi 에 안 잡혀 있었다**. QA 3라운드 F-1~F-5 전건 `fixed` — 🔴 **F-1 은 내 산술 오류**(다른 시나리오의 423 을 섞음, 정답 447), 🔴 **F-5 는 내가 `design-change-procedure` §4 를 어긴 것**(원본 삭제). 하필 **원본으로 안 돌아가서 생긴 PR 에서** 원본을 지웠다 — 취소선+`<details>` 로 복원하고 `gh api /markdown` 으로 실렌더 검증 **#408 (원장 L-44)**: 다음 세션 전 ECR 사전점검에서 **다른 결함**이 나왔다 — `ecr-push.yml` 이 `github.sha` 로 태깅하는데 **`pull_request` 이벤트에서 그 값은 GitHub 이 만든 일회성 머지 커밋**(`refs/pull/N/merge`)이다. 실측 `git log a9b3a49` → `fatal: bad object`, `gh api` → `Merge 74c2f843 into bd51e18f`. 🔑 **워크플로 자신의 논거를 무너뜨린다** (*"sha 태그를 쓰면 지금 뭐가 도는지 안다"*) — 조회가 안 되니까. 게다가 **PR 경로가 stage 브랜치의 주 경로**다. 부수 효과로 `latest` 가 서비스마다 **다른 코드**를 가리킨다. **하네스 동결 규칙에 따라 미수정** — 세 서비스 모두 **`bd51e18`(실재 main 커밋)** 에 있고 `be/` 는 이후 **변경 0건**, 셋 다 **arm64/linux** 라 제품이 안 막혔다. 🔑 **교훈: 개수가 아니라 태그 일치를 봐라** — 08-31 에 개수만 보고 `None` 을 놓쳤고, 이번엔 개수가 3/3 인데도 하나가 추적 불가였다. | 2026-09-03 |
| **#405 · #406** | **★ Stage C 1차 시도 — 실패했고 그 불가능이 실측으로 확정됐다 (08-31, 유료 51m56s ≈ $0.129, 고아 0건).** **과금 전 사전 점검이 두 건을 $0 에 잡았다**: ①**ECR 이미지가 3개 중 1개뿐**(`ai-api`·`daily-api` = `None`). C-3 은 *"레포 생성 + 워크플로 options"* 로 닫혀 있었는데 그건 **구울 수 있다**의 증명이지 **구웠다**가 아니다 ②**`ecr-push.yml` concurrency 결함** — 그룹 키에 service 가 없어 main 에서 3개 연속 dispatch 시 가운데가 **조용히 취소**(실측 `cancelled`, **job 0개·8초**). 🔴 `cancelled` 는 `failure` 가 아니라 **CI 에 빨간불이 안 뜬다** → 모르고 apply 했으면 **과금 중** `ImagePullBackOff` 를 팠을 것. 한 줄 수정(하네스 동결 규칙 **해제 조건 충족** — 실제로 막았다). **과금 중 배운 것**: t4g.medium launch 거부(Free Tier) + **Allocatable 실측**. `kube-reserved` 공식이 `255 + 11×max_pods(11)` + eviction 100 = **476Mi** 로 실측 차이와 **정확히 일치** — 🔑 **파드 상한이 메모리를 먹는다**(둘은 독립이 아니었다). 공칭 2 GiB 가 아니라 **capacity 부터 1841Mi**. 🔴 **퀴즈 중 사용자가 계획서의 오류를 찾았다** — *"스케일 아웃은 불가능한 상황이야?"*. D-009 의 *"노드 2대"* 기각 사유 셋 중 둘이 썩어 있었다: *"medium 보다 비싸다"* 는 medium 단가가 **추정**이던 시점 문장인데 **같은 세션에서 실측으로 정확히 2배임이 확인되며 동일 비용이 됐고**, 그런데도 **아무도 기각 사유로 돌아가지 않았다**(빨간 깃발 *"일지에 적어뒀으니 됐어"*). *"파드당 메모리 벽"* 은 **논점 이탈**(스케일 아웃이 푸는 건 총량). `node_max_size` 는 **처음부터 2** 였다 — 내가 안 봤다. 👉 **교훈: 추정이 실측으로 바뀌면 그 추정에 기대 기각했던 대안을 반드시 다시 연다.** 퀴즈 5문항 전부 통과(1차 정답은 1개 — 교육 후 재질문 패턴이 3a·3b 와 동일). 메타 교훈 = **다섯 중 셋이 "부재·실패가 성공과 똑같이 생긴 자리"**. QA 2라운드: 1차 HIGH 0 · MEDIUM 4 → F-1(자기모순)·F-2(`addons.tf` stale)·F-3(이 CONTEXT) 수정 후 **재QA 로 `fixed` 확인**, F-4(배치 미강제)는 **원장 L-43** 으로 미룸(배치 제약은 클러스터 없이 검증 불가). 🔴 **재QA 가 F-5 를 새로 잡았다** — 원장에 *"경고를 세 곳에 남겼다"* 고 썼는데 **계획서 D-010 에는 없었다.** 하필 거기가 `-var` 한 줄을 안내하는 자리라 `variables.tf` 를 안 열고 지나가는 경로였다. **개수를 세지 않고 주장한 것** — 이 세션 두 번째 반복이다(앞서 *"8행 전부 정규화"* 도 거짓이었다). 문구를 낮추지 않고 **실제로 세 곳으로 만든 뒤 grep 으로 셌다** | 2026-08-31 |
| **#401 · #402 · #403 · #404** | **★ Stage C 준비 완료 — 남은 건 apply 뿐 (08-31, 비용 $0 · 클러스터 미가동).** **#401**: PR 자동 머지 절차 수립(사용자 요청). 정지 조건 5개 중 핵심은 **`tofu plan` 에 `to add`/`to destroy` 가 있으면 정지** — 판별이 판단이 아니라 **조회**다(#399 `2 to add` → 확인 / #400 `No changes` → 자동). CONTEXT 갱신을 **그 PR 안에서** 하는 규칙도 함께 박았다(#396·#397 에서 두 번 연속 빼먹어 따라잡기 PR #398 을 만들었다). **#402 C-5 전제**: vpc-cni `enableNetworkPolicy`. 🔑 **스키마 실측으로 타입 함정 제거** — `{"type":"string","format":"boolean"}` **문자열이다**. 불리언으로 썼으면 과금 중 `InvalidParameterException`. 같은 스키마 안에서 `nodeAgent.enabled` 는 진짜 불리언이라 일관되게 쓰려다 틀리기 딱 좋았다. ⚠️ **내 걱정이 틀렸고 답이 일지에 있었다** — *"노드에이전트가 medium 여유를 먹는다"* 고 우려했으나 `aws-node` 는 처음부터 **2/2 컨테이너**(일지 :329). 플래그는 파드를 만드는 게 아니라 이미 붙은 사이드카를 켠다. **일지를 안 읽고 걱정부터 했다.** **#403 C-7**: ai-api 헬스. 🔑 **`daily-api` 의 `db,ping` 복사가 기동 실패를 낸다는 것이 실측으로 확정** — `NoSuchHealthContributorException` 으로 **컨텍스트 로드 자체가 실패**(조용히 무시 아님). QA 가 환경변수 오버라이드로 독립 재현. readiness 를 `ping` 만으로 구성(원장 **L-39** — liveness 와 등가, Judge0 트리거). `application-prod.yml` 은 **의도적으로 안 만들었다** — ai-api 의 유일한 실행 환경이 학습 클러스터이고 거기서 DEBUG 는 관측 수단이다(QA 가 근거 4개 전부 검증). **#404 매니페스트 3벌**: `ai-api.yaml`(8081, 스텁 env) · `daily-api.yaml`(8082, **C-8** `DEVQUEST_AI_HTTP_BASE_URL`, `core-api-db` 공유) · `networkpolicy-ai-api.yaml`(ingress-only). 🔴 **fail-open 3경로를 ruby YAML 파싱으로 기계 검증** — ①CNI 미강제 시 **조용히 수용**(코어 API 라 에러 없음) ②`podSelector` 는 **template 레이블**만 본다(0개 매칭해도 생성 성공) ③ns 어긋나면 무력화. ⚠️ **"막는다"의 증명은 아직 없다** — 차단/허용 쌍은 클러스터가 떠야 한다. 절차를 README §4.1 에 미리 적었다(파드 슬롯 0 소모) | 2026-08-31 |
...
> ② ~~t4g.small → medium 상향~~ → 🔴 **폐기(D-010)** — 계정이 medium 을 launch 하지 못한다. ~~재개 1순위는 **노드 2대**.~~ → 🔴 **재정정(D-011, #407): 노드 3대.** 2대는 비관 가정에서 **Pending 이 증명된다**(조각화). 3대는 미측정 값에 의존하지 않는다. 차액 세션당 $0.021
