...
| 항목 | 내용 |
|------|------|
| 진행 중인 트랙 | **Phase 2 — `daily-service` 추출**. Stage A ✅ · B ✅ · **Stage C 준비 완료** — C-3 ECR ✅(#399) · C-4/D-009 노드 medium ✅(#400) · C-5 전제 vpc-cni ✅(#402) · C-7 ai-api 헬스 ✅(#403) · **C-5 본체 + C-8 매니페스트 3벌 ✅** → **남은 것: apply → 이미지 3개 푸시 → e2e → destroy. 여기서부터 과금.** ⚠️ apply 전 `docs/eks-session-sop.md` 필독 + 사용자 승인. 예상 **$0.149/h**(medium, 실측), 왕복 40~50분 ≈ $0.12. 브랜치는 `stage/eks-*`, **PR 전 이해도 퀴즈 필수**(`assert-eks-quiz.sh` 차단) |
