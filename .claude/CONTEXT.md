...
- 🔴 **메타스페이스가 상한 87.6% — 배포 공백이 길어지면 prod 가 죽는다** (2026-09-24 실측)
  ...
  **조건 = `be/**` 배포 없이 ~85일** (재배포가 JVM 을 리셋한다). 관측된 최대 공백 15일이라 당장 🟢 지만,
  🔑 ***이 레포는 #361(08-06) 이후 `feat` 커밋이 0건이고, "제품 작업이 멈춘 상태" 가 바로 이 조건이다.***
