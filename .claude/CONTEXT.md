...
| 항목 | 내용 |
|------|------|
| 진행 중인 트랙 | **EKS Stage 3b** — terraform 소유 EBS + static PV로 전환 (영속 볼륨) |
...
| PR/커밋 | 내용 | 날짜 |
|---------|------|------|
| **#351 · 진행중** | **★ 영속 리소스 가드레일 + CONTEXT 자기참조 제거 (07-31, 비용 $0).** Stage 3b가 도입할 영속 EBS를 만들기 **전에** 관리 장치부터 세웠다(사용자 지시: *"문제를 피하기보단 어떻게 방지·추적·모니터링하는지가 더 중요"*). 원장 `PERSISTENT-RESOURCES.md`(각 항목에 **증가 상한 필수**) · SOP 고아검사를 **성격이 반대인 두 검사로 분리**(9=0건이어야 / 9b=원장과 일치해야 — 영속 볼륨은 destroy 후 영원히 available이라 기존 쿼리로는 매 세션 오탐이고, **매번 실패하는 검사는 곧 무시된다**) · 예산을 월간→**누적 크레딧 $10 단위 20단계**(월간은 매달 리셋돼 실사용 월 $14에선 $10만 반복 발동하고 19단계가 죽는다) · 세션 배너 · 리퍼 고아 경고(감지 전용 — 생존판정에 넣으면 무한 destroy 루프). **실측 4건**: 예산당 알림 **10개가 총계 하드 리밋**(`CreationLimitExceededException`, plan이 못 잡는 4번째 함정 → `chunklist`로 구조적 차단) · **알림 예산은 무료**(Pricing API `BudgetsUsage` $0.00, 유료는 Actions뿐 — "첫 2개 무료 후 $0.02/일"은 옛 모델) · ANNUALLY+과거 시작일 수용 · **누적 소진 $0.481/$200**(CONTEXT의 $0.45보다 정확). 🔴 **내 코드 결함 2건을 반증 테스트가 잡았다**: 고아 필터가 태그 *키 존재*만 봐서 `Persistent=false`와 대문자 오타 `True`가 빠져나갔고(→값까지 비교), `elif [ $? -eq 0 ]`이 aws가 아니라 앞 test 결과를 읽어 **AWS 조회 실패가 "볼륨 0개"로 표시**될 뻔했다. 둘 다 *안전함으로 잘못 표시되는* 방향이라 테스트 없이는 통과했을 것. **그리고 CONTEXT 자기참조 종결** — 사용자 지적(*"main의 CONTEXT가 자꾸 안 맞는다, 매번 이래"*)이 맞았고 원인은 성실성이 아니라 구조였다: PR은 자기 머지 사실을 못 담아 `머지 대기`가 머지 순간 **확정적으로 거짓**이 된다. 클린 클로즈 PR **24건**이 그 증거(git log 실측). `브랜치`·`열린 PR` 필드 삭제 + `session-status.sh` 신설로 **파생 가능한 상태 저장을 중단**했다(F-2 "$200 변수 중복"과 같은 병, blast radius만 다름). QA: HIGH 0·MEDIUM 0·LOW 5 → 4건 fixed(재QA 확인), F-5 원장 L-11. | 2026-07-31 |
