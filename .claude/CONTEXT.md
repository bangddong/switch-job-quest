...
| **#414** | **k6 램프 유료 세션 — W_peak 실측 + requests 재산정 (41m49s ≈ $0.1300, 고아 0).** 🔑 **무릎을 못 찾았다 — 1200 req/s 에서도 서버가 포화되지 않았다**(p95 17.54ms · 실패 0/72,003 · CPU 스로틀 0 · evict 0) → **W_peak 은 하한이다.** requests **1792 → 1568Mi**(core 480/576 · daily 512/640 · ai 320/448 · pg 유지). 🔑 **L-47 종료, 다만 전제가 틀렸다**: 현행 1792 로도 2노드에 **들어간다**(여유 **5Mi**) → D-012 의 *"1280 으로 줄여야 2대"* 는 부분적으로 틀렸고 재산정의 가치는 *가능하게*가 아니라 ***안전하게*** 로 재정의. 🔑 **L-43 종료**: 시스템 파드를 쪼갰다 — DaemonSet **104Mi/노드** · coredns **70Mi**(`addons.tf` 가 추정한 ~100Mi 는 **틀렸다**) · **ebs-csi-controller 232Mi = 시스템 최대 소비자**(아무도 주목한 적 없음). 합계 406Mi 로 08-31 실측과 **정확히 일치**(교차검증). 🔴 **postgres 는 공식 제외 — #410 의 거울상**: 68Mi 는 26행짜리 빈 DB 값이라 *용량*이 아니라 *만진 페이지의 양*이고 `shared_buffers` 기본 128MB 라 limits 128Mi 면 버퍼 풀만으로 상한. **규칙은 대상이 JVM 인지 DB 인지 모른다.** 🔴 **§9.3 판정 규칙 자체가 결함**: p95 가 부하와 함께 **내려간다**(JIT 워밍업) → 스테이지1 기준선은 콜드 스타트라 임계 68.85ms 가 **한 번도 발동 안 했다**. 기준선을 워밍업 후 스테이지로(추가 과금 0). **실수 3건**: ①ConfigMap 을 `apply -f k6.yaml` 로 자기가 덮어써 `PREVUS` 실험 무효화 — 수치가 우연히 좋아져 *"VU 로 고쳤다"* 는 **틀린 결론 직전**까지 갔다(`vus_max` 로 잡음) ②비용에 파일 mtime 을 써서 **destroy 시작보다 이른 종료 시각**이 나옴 ③🔴 **QA F-1 — 퀴즈 메타 교훈을 *다음 작업에서 그대로 재발*시켰다**: 합계를 공식값(ai 256·pg 96)으로 더해 **1344/−448** 로 적었는데 그 둘은 **공식 미적용 항목**이었다. 개별 행엔 320·256 을 옳게 적어 놓고 **합계만 옛 값** = 표 자기모순. 4개 문서에 복사됐고 하필 **블로그 헤드라인 절감 수치가 2배 부풀려짐**. ***교훈을 적는 것과 적용하는 것은 다른 일이다.*** **퀴즈 1차 정답1/부분1/오답3 → 2차 부분2/오답1** (JIT·taint/toleration·EBS 존 제약·오픈루프 지표 대부분 첫 접촉). 메타 교훈 = ***"숫자는 맞는데 그 숫자가 답하는 질문이 내가 던진 질문이 아니었다"*** → 조치로 계획서 §9.4 에 **"무엇의 실패를 잡나"** 열 신설, `quiz.md` §3.5 **해설 서사 규칙**(무슨 작업 중/무엇이 관측/왜/안 그랬으면/근거) 신설 | 2026-09-08 |
