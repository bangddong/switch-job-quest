...
| 항목 | 내용 |
|------|------|
| 진행 중인 트랙 | **Phase 2 — `daily-service` 추출**. Stage A ✅(08-21 prod 실측) · B-1 `daily-core` ✅(#392) · B-2a `client-ai-http` ✅(#393) · **B-2b `core:daily-api` ✅(#395)** → **다음 = Stage C (EKS 3서비스 배포)**. ⚠️ Stage C 는 만드는 단계가 아니라 **배포하는 단계**다 — 재료(모듈 4개 + 앱 2개)는 전부 나왔고, 없는 것은 **매니페스트**다(`k8s/base/` 에 core-api·postgres 만, **ai-api·daily-api 둘 다 0건**) |
