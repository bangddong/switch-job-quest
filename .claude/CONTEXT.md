...
| **#415** | **ⓒ 유료 완전 재현 + 3서비스 메모리 실환경 검증 (31m34s ≈ $0.0716, 고아 0).** 🔑 **재현 판정 = 조건부 통과** — `eks-tutorial-steps.md` **만** 보고 Stage 0→3b 최종 상태에 도달했고 **코드를 열어야 했던 지점은 없었다.** 다만 그대로 따르면 **오판하는** 결함 2건: ①`db_mode` 확인이 in-cluster 인데 **1** 을 낸다(`show -json` 의 `configuration` 절이 `count=0` 리소스 **선언**까지 담는다) → 따랐다면 `-var db_mode=rds` 로 **RDS 실수 기동**. 🔴 **이 검사는 *"개수는 db_mode 를 구분 못 한다"* 를 고치려 도입한 대체 검사였다 — 대체품이 같은 병에 걸렸다**(이 자리 세 번째라 새 검사의 한계도 함께 적었다) ②3b-7 기대 출력에 `components{db,ping}` 이 있으나 `show-details` 미설정(기본 `never`)이라 **`{"status":"UP"}` 만 나온다** → **그 단계가 증명하려는 것을 그 명령으로는 볼 수 없다.** 🔑 **L-47 실측 확정**: 신규 requests 1568Mi 로 **2노드에 4파드 전부 배치, Pending 0**(승인은 3대였으나 `allocatable=1365Mi/노드` 실측 후 2대로 축소 — 되돌림 비용 2분·$0.001 로 비대칭). 재시작 0 · OOMKilling 0 (core 345/576 · daily 282/640 · **ai 175/448** · pg 33/512). 🔑 **ai-api 위험 재서술**: 예약 천장 412.8Mi 를 컨테이너 안에서 직접 확인(플래그가 **이미지 ENTRYPOINT 에 구워져** 있다) — **그러나 Max/Reserved 는 천장이지 약정이 아니다.** 🔴실측 = 부하 208Mi 까지 안전 / ⚪미확인 = metaspace 가 160 캡에 접근하는 시나리오(부하 미실시 · `jcmd` 부재). 🔴 **QA 4라운드·지적 8건 — 라운드마다 새 지적이 나왔고 원인이 전부 "덧붙임"이었다**: F-1 내가 검증하러 온 **재현 게이트 표 자체를 안 고침** → F-2/5 누적 비용 칸에 산정치를 넣어 **바로 아래 인용한 규칙을 인용하면서 위반** → F-6 *"마지막 실측은 08-12"* 가 거짓(**같은 파일 안에** 09-08 $2.453 이 있었다) → F-8 새 교차검증이 09-08 세션 $0.13 을 누락했는데 오차 $0.002 → **검증이 아니라 lag 신호**였고 그 파일이 30줄 아래서 이미 경고 중이었다. **F-8 을 "삭제"로 잡자 신규 지적 0건.** F-4 → 원장 **L-49**(과금 중 자율 파라미터 축소 절차). 퀴즈 정답2/부분1/오답1, 메타 교훈 = ***"어떤 숫자가 맞는 답인지는 던진 질문이 정한다"*** — 퀴즈 4문제가 전부 그 형태였고 **그 직후 QA 에서 내가 같은 실수를 두 번 더 했다** | 2026-09-09 |
