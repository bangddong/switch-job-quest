...
| PR/커밋 | 내용 | 날짜 |
|---------|------|------|
| **#422** | **선행 조건 3 의 $0 선행분 — 학습 클러스터용 ACM 인증서 ($0, 클러스터 미가동).** 🔑 **계획서의 *"한 유료 세션"* 을 쪼갰다** — 뜯어보니 절반이 클러스터 없이 된다. Cloudflare 수동 검증은 전파 대기가 예측 불가라 컨트롤플레인 $0.100/h 가 도는 동안 하면 순손실(#416 의 *"ALB 를 마지막 7분만 켜서 전체의 4%"* 와 같은 레버). 🔴 **Blindspot 17건 중 4건이 설계를 바꿨다 — 네 항목 연속이다.** ⓐ**U-3**: `SecurityConfig.kt` 와 `.tf` 를 한 PR 에 담으면 `be-cd`(→**Fly prod 배포**)와 `infra-deploy`(→`tofu apply`)가 **같은 머지에서 발사**되고 롤백 경로가 다르다. 계획서가 둘을 묶은 근거(*"같은 `/actuator/**` 표면"*)도 **성립하지 않는다** — EKS 쪽 그 표면은 코드가 아니라 `ingress.yaml` 이 소유한다. ⓑ**U-6·U-7 — 앞선 네 번을 복사하면 틀린다.** `prevent_destroy` 는 ACM 의 SAN 변경이 replacement 를 강제해 **전환 때 매 머지 CI 실패**를 만들고, `for_each` 환경 축은 2a 의 전제(*"`random_password` 는 state 항목이라 $0"*)가 **실제 리소스인 ACM 에 전이되지 않아** 검증 불가한 prod 인증서를 72h 뒤 `VALIDATION_TIMED_OUT` 으로 박는다. 🔑 ***D-004 는 「레이어 선택」을 정당화하지 「래치」를 정당화하지 않는다*** — 네 번 연속 같이 와서 한 덩어리로 보였을 뿐이다. ⓒ**U-10**: Cloudflare 신규 CNAME 기본 **Proxied** → 브라우저가 ACM 이 아닌 엣지 인증서를 봐 **학습 목표가 조용히 무산**된다. 04-08 에 이미 겪은 사고(`CONTEXT.archive.md`). ⓓ**U-1 은 $0 curl 로 반증**: 인터넷→`/actuator/prometheus` **403** 실측 → `fdaa::/16` 삭제는 무엇도 닫지 않고 열지 않는다. 🔴 **QA 6라운드·지적 9건·HIGH 0.** F-1 이 **읽는 코드 없는 죽은 태그**(L-24)를, **F-6 이 내 규칙 오인용**을 잡았다 — *"배선은 하네스 동결 규칙에 걸려 안 한다"* 고 썼는데 **규칙 문언은 `.claude/`·`.github/`** 이고 `docs/` 는 범위 밖이었다. ***규칙을 문언 확인 없이 인용해 「필요한 일을 안 하는 핑계」로 쓴 것.*** 실제 이유는 범주였다(SOP §9 는 *"= 0"* 이 합격 기준인데 ACM 은 항상 1건). 🔑 **원장 L-54 신설** — 동결 규칙의 해제 조건이 **하나뿐**이라 **레포 소유자가 자기 규칙을 지시로 넘을 수 없고**, 같은 지적이 매번 재생산된다. ⚠️ 해제 조건을 더하는 것도 위험하다: **사용자 요청은 내가 유도할 수 있다**(*"이거 고칠까요?"*). 🔑 **U-8 이 일지에서 빠진 채 유실돼 있었다 → 「Blindspot 발견의 목적지」 규칙 신설**(`CLAUDE.md` 목적지 표 + orchestrator 3.5단계). 원장은 QA 지적 전용(훅이 `<브랜치>/<F-ID>` 를 검사), 일지는 시간순이라 *"다음에 뭘 해야 하나"* 로 검색되지 않는다 → **계획서**. QA 가 독립적으로 같은 구멍을 지적(F-2)해 **개인 실수가 아니라 구조**임이 확인됐다. 🔴 **고치는 과정이 새 지적을 낳은 것이 두 번**(F-1→F-6, F-3→F-5), 그리고 F-8→F-9 는 **고칠 때마다 설명을 늘려** 검증 대상을 늘린 결과였다 — ***주장을 줄이자 끊겼다.*** 🔑 ***레포 밖 근거는 인용하지 말고 레포 안 근거로 다시 세운다*** (`qa-cache/` 는 gitignore 라 QA 산출물은 영속 문서의 근거가 못 된다) | 2026-09-15 |
| **#421** | **산정↔실측 비용 대조 + 09-14 일지 소급 ($0 조회, CE 3회 = $0.03).** 🔑 **누적 실측 $2.9177** (예산 API `$2.936` 과 독립 일치) — CONTEXT 에 있던 `$0.481` 은 **07-31 값이라 6주간 썩어 있었다.** 🔑 **영속 모델이 검증됐다**: 8월 베이스라인 $1.0556 vs `PERSISTENT-RESOURCES.md` 추정 $1.08 (오차 **2.3%**). 🔴 **대조가 준 것은 숫자가 아니라 비중이다** — 세션마다 초 단위로 산정해온 $0.06~$0.10 을 다 더해도 누적의 **20% 남짓**이고, 나머지는 **켜둔 채 잊은 것**(EBS·ECR $1.56)과 **재는 것**(Cost Explorer **$0.190 = 6.5%**, `$0.01/요청`)이다. ***세션 최적화의 한계수익이 낮다*** — #417 의 *"노드 수가 세션 길이보다 비용을 더 움직인다"* 를 한 칸 더 옮긴다. ⚠️ CE 과금은 **08-12 에 이미 한 번 인지됐는데**(`증분 $0.04 (CE 호출)`) **상시 항목으로 안 옮겨져** 한 달 뒤 같은 크기로 또 났다 — ***일회성 기록과 모델 등재는 다른 일이다.*** 기존 문구 *"대조는 무료, 10초"* 도 정정(`budgets` 는 무료, 서비스별 내역은 CE 필요). 🔑 **09-14 세션(2a·2b)이 일지에 아예 없었다**(`grep → 0건`) — 무과금은 기록 면제가 아니다. 소급 명시 기록. 🔴 **원장 L-53 에 표본 ⓕⓖⓗ 3건 추가 — 어제 채택한 관례가 오늘 시험됐고 결과가 깨끗하다: 지킨 곳은 맞았고 안 지킨 곳은 틀렸다.** *"없다" 를 쓸 때 검색 범위를 병기한 주장(09-14 일지 누락)은 참이었고, 병기 안 한 둘(*"ECR 미등재"*·*"CE 미등재"*)은 **둘 다 거짓**이었다 — **관례는 옳았는데 발화되지 않았다.** 검색을 *결론을 쓰기 전*이 아니라 *쓴 뒤*에 하면 검증이 아니라 변명 찾기가 된다. ⓖ 는 새 형태 **지표 바꿔치기**: `CLAUDE.md` 의 *"8월 커밋 31건 중 18건이 하네스"* 를 *"8월엔 세션이 없었다"* 로 옮겼는데 실제로는 유료 세션이 셋(`08-07`·`08-12`·`08-31`)이다 — **출처 문서는 아무것도 틀리지 않았다**(#417 F-11 과 같은 병) | 2026-09-15 |
| **#418** | **선행 조건 2a — 시크릿 환경 축 도입 ($0, 클러스터 미가동).** 🔑 **B-2 종결.** `random_password.jwt_secret` 이 destroy-after-use 레이어에 있어 클러스터를 부술 때마다 서명 키가 바뀐다 = **전 사용자 강제 로그아웃**(30일 만료). L-14 와 같은 형태 — 그때는 데이터가 살아남았는데 자격증명만 안 붙었고, 여기서는 사용자가 살아남았는데 토큰만 안 붙는다. 🔴 **Blindspot 이 계획을 뒤집었다: 규칙을 글자 그대로 복사하면 경계가 *없어진다*.** D-004·L-14 의 앞선 세 적용(postgres 비밀번호·영속 EBS·백업 S3)은 전부 **학습 전용 단일 환경**이라 섞일 것이 없었는데, **JWT 는 처음으로 두 환경에 걸친다** → 키 하나를 영속 레이어에 두면 수명은 맞추고 환경을 섞어 **학습 클러스터가 prod 유효 토큰을 발급**할 수 있다(= `secrets.tf` 가 OAuth 에 대해 **스스로 금지해둔 상태**). → 키 2개 + 시크릿 이름 `<cluster>/<env>/app` + ESO key 파라미터화(기존 `*_PLACEHOLDER` sed 관례 재사용, 새 도구 0). 🔑 **Blindspot 이 두 전제를 더 정정**: ①B-2 서술이 두 시점을 섞었다 — 실사용자 토큰은 **Fly 의** 키로 서명되고 Fly secrets 는 **write-only**(B-9 근거)라 **전환 당일 로그아웃은 어느 레이어든 확정**이고, 이 항목이 사는 건 *전환 **이후** teardown 마다*뿐 ②**OAuth 경계는 이미 4중으로 닫혀 있었다**(할 일 0) — 계획서의 *"prod/학습 경계"* 가 **이미 성립한 것을 다시 하려는 것처럼** 읽혔다. 🔴 **QA 4라운드·지적 4건, HIGH 0. F-2 가 내 과대주장을 잡았다** — *"IAM 이 경계를 지킨다"* 를 조건 없이 썼는데 `backend.tf` state key·`cluster_name`·ESO 역할명이 전부 단일값이라 **두 배포가 아직 공존 못 한다** → **경계는 절반만** 세워졌다(세워진 절반은 IAM 이 지킨다). 🔑 **원장 L-53 신설 — *옮겨 적는 단계에서 정확성이 샌다*.** L-52(편집이 no-op)와 달리 **편집은 성공하는데 내용이 조금 거짓이 된다.** 표본 6건: 조건절 탈락 2(F-2, #417 퀴즈 Q4) · 인용 줄번호 3 · **검증 단계가 부재를 잘못 단정 1**. 🔴 **마지막 것이 가장 무겁다 — QA 가 *"git 이력에 존재한 적 없는 창작"* 이라 단정했는데 실재했다**(검색을 계획서 파일 하나로 좁혔던 것). 앞의 다섯은 옮겨 적을 때 샜는데 **이번엔 검사하는 쪽이 샜다**(#417 F-10 과 같은 층위). → ***부재 주장은 존재 주장보다 비싸다*** — 존재는 한 건만 보이면 참이지만 부재는 검색 범위가 완전해야 참이다. 관례 2개 채택: **인용에 줄번호 대신 선언문**(verify 마커가 이미 같은 이유로 그렇게 한다) · ***"없다"*** 를 쓸 때 **어떤 명령으로 어디를 봤는지** 병기. 💰 **CI 실측이 내 로컬 plan 을 독립 확인**(`2 added, 0 changed, 0 destroyed`, `jwt_secrets = (sensitive value)`) — QA 는 `tofu` 실행이 막혀 재현 못 한 유일한 항목이었다. 🛡️ 원장에 **「비용 $0 인데 영속인 것 — state 안의 값」** 절 신설(`random_password` 3개는 `aws` CLI 어디에도 안 나오는데 잃으면 접속 불가·전원 로그아웃). ⚠️ **실검증은 다음 유료 세션** — 2-cluster 는 apply 없이 검증 불가. **B-15 는 2b 로 분리**(`fly secrets set` 이 머지보다 먼저여야 한다 — `be-cd` 가 main push 마다 자동 배포하고 Fly prod 가 **같은 `prod` 프로파일**을 읽어 메트릭이 **조용히** 꺼진다) | 2026-09-14 |
