...
| PR/커밋 | 내용 | 날짜 |
|---------|------|------|
| **#411** | **부하 상태 메모리 실측 + 부하 테스트 재설계 (09-06 유료 $0.4622 · 09-07 재설계 $0).** 🔴 **비용 사고 — 과금 중에 사용자에게 질문했고 대기 127분이 그대로 과금됐다.** 예상 $0.10 → 실제 **$0.4622**, **85%가 아무것도 안 한 시간**. dead man's switch 가 13:36 자동 destroy(고아 0) — **안전장치는 성공, 절차가 실패.** 손익분기: 재apply+destroy **19분 고정 $0.059** vs 켜두기 **대기 비례 무한** → SOP §과금구간 맨 앞에 **"사용자에게 질문하지 않는다. 물어야 하면 먼저 destroy"** 를 박았다. 09-04 원칙의 두 번째 면 — 그때는 **진단**을, 이번엔 **의사결정**을 과금 전으로 빼야 했다. 🔑 **최대 수확(부하 품질 무관)**: `core-api` 부하 중 **nonheap 183.4 > heap 139.3Mi**, **Metaspace 125.6Mi**, 힙은 상한 316 중 **70Mi 만** 사용 → 09-05 가설 *"limits 조여도 힙만"* **실측 확인**. GC로그 139M == prometheus 139.3Mi 교차검증. 🔴 **그런데 사용자가 *"부하테스트 신뢰도가 부족해"* 라고 지적했고 맞았다** — closed loop 라 **에러 0 이 거의 보장**된다(서버가 느려지면 부하가 자동으로 줄어듦) = **반증 불가 관측**. *"부재가 성공과 똑같이 생긴"* 실패의 또 한 사례. → **k6 오픈 루프** 교체. 핵심은 `dropped_iterations` — **생성기 병목을 보고한다**. 로컬 반증 실측: RATE=20000 에서 **서버는 10,341 req/s 성공했는데** dropped 81,126 → **exit 99**. 셸 루프였다면 *"전부 200"* 이었을 것. 🔑 **규칙이 일한 지점**: 하한 448Mi 가 `ai-api` 를 구했다(320이면 JVM 예약 412 > 상한). **규칙의 구멍도 발견** — 하한은 **JVM 한정**(postgres 에 적용했으면 128→448, 3.5배). **QA 3라운드**: 🔴 **F-1(HIGH)은 내가 없는 상태값 `⚪보류` 를 발명**해 CI 크래시 — QA 가 스크립트를 **직접 실행**해 잡았다. 게다가 내가 *"가드 173/173 통과"* 라고 했는데 **그 숫자에 `check-design-integrity.sh` 는 없었다**(검사 범위보다 넓게 주장). F-2(k6 스테이지 값 불일치·RATE×3 캐비앗 누락)·F-3(`-upgrade` 가 tls 4.3→4.4, 일지 미기록) fixed · F-4 → **L-48**. **퀴즈 1차 0정답·2부분·3오답 → 2차 5/5**, 메타교훈 = ***"숫자는 맞는데 그 숫자가 답하는 질문이 다르다"***. 📌 **퀴즈 형식 변경(사용자 지시)**: 재확인을 **객관식 → 서술형** — 객관식 4문항 정답이 **전부 B**, 매번 정답이 가장 긴 보기라 *"통과했다고 믿게 만드는 검사"* 였다. 원장 **L-46**(하트비트 Stop훅, wontfix) **L-47**(노드2대 미검증, deferred) **L-48**(`🚧진행중` 이 두 의미를 뭉갬, deferred) | 2026-09-07 |
| **#410** | **부하 측정 사전 준비 — 절차·판정 기준 확정 ($0, AWS 접촉 0건).** $0 조사가 계획서의 측정 수단 **두 개를 무효화**했다: ①런타임 이미지가 `21-jre-alpine` 이라 **`jcmd` 가 없다** ②`ai-api`·`daily-api` 는 `/actuator/prometheus` 자체가 없고 `core-api` 는 `SecurityConfig` 가 IP 로 제한 (prod 외부 GET **403** 실측 · 허용목록 `fdaa::/16` 은 **Fly 사설망**이라 EKS 에선 무의미) → 파드 안 localhost 로만. 🔴 **더 큰 것 — 문제의 방향이 바뀌었다**: `limits: 900Mi` × 35% = **MaxHeap 316Mi**. `Dockerfile:38` 의 *"약 179MB"* 는 **Fly 512MB 예산 값**이고(실측 표에서 512Mi→180Mi 로 일치), **같은 이미지가 EKS 에서 힙 상한 1.77배**로 돈다. 09-04 의 *"실사용의 2배"* 는 idle 값이었다. 🔴 **레이트 리밋이 부하 테스트를 원천 봉쇄**한다(`explain` = IP당 하루 5회, 3서비스를 태우는 유일한 경로) → `SPRING_APPLICATION_JSON` 상향 + **6번째가 200인가**로 적용 검증(5회까지는 무관하게 성공하므로 그게 유일한 판별점). 준비물 2종은 **클러스터 없이 돌려서** 검증했다. 🔑 **검증이 자기 자신에게서 결함 3건을 잡았다**: ①첫 문법 검사가 **거짓 통과**(colima 마운트 실패 → 빈 디렉터리에 `sh -n` → exit 0) — 08-11 의 *"검사가 주장보다 헐겁다"* 가 **내 검증 코드에서 재현**됐다. stdin 으로 바꾸고 **반증 먼저** ②`code()` 가 도달불가 시 빈 문자열 → 집계에서 실패가 안 보임 → `ERR` 명명 ③내가 *"검증이 버그를 잡았다"* 고 **한 번 잘못 말했다** — 원인은 `docker run` 의 **pull 진행 로그**를 `2>&1` 로 섞은 것. **QA 2라운드 HIGH 0, F-1·F-2 전건 `fixed`(재QA 확인).** 🔴 **F-1 은 내가 근거도 숫자도 틀린 것**: 하한 384Mi 의 근거가 ①Fly 수치를 EKS 에 **환경 건너뛰고** 붙였고 ②07-14 Metaspace 사고는 **고정값** `MaxMetaspaceSize=160m` 에서 난 거라 힙 여유 논증과 **인과관계가 없다**. 검산하니 부등식 `0.35L+160+96≤L` 교차점이 **393.8Mi** → 384Mi 는 **예약만으로 6Mi 초과** → 하한 **448Mi**. 🔑 **그 지적이 정정 후 반대 방향의 진짜 위험으로 되살아났다** — `limits` 를 조일수록 Metaspace 고정비 비중이 커진다(900Mi 18% → 448Mi **36%**). **조여지는 것은 힙뿐이다.** 🔑 이 세션 **근거를 세우기 전에 결론 숫자를 먼저 쓴 게 두 번** (거짓 문법 통과 · 하한값) — 09-04 가 남긴 *"검산 결과가 문장보다 먼저 있어야 한다"* 의 같은 병 | 2026-09-05 |
| **#409** | **🎉 Stage C 완료 — 3서비스 + e2e + C-5 (09-04, 유료 33m06s ≈ $0.1029, 고아 0).** 노드 3대 apply 9m03s(노드그룹 1m48s — 08-31 에 medium 이 5회 Failed 하던 자리) · 영속 볼륨이 destroy 를 건너 **데이터 유지**(Flyway 13 · 테이블 15 · 뱅크 26행) · 앱 3개 전부 1회 rollout · **재시작 0**. 🔑 **C-5 가 드디어 검증됐다** — ②정책 전 성공 → ④타임아웃 → ⑤양성대조 성공. ②·⑤ 없이 ④만 보면 *"파드 미기동·DNS 실패"* 와 *"전면 차단 정책"* 이 둘 다 통과한다. 🔴 **과금 중 안전장치가 꺼진 채 돌았다** — `nohup tofu apply` 가 마커 훅 앵커를 빗나가 **dead man's switch 미작동**(`eks-reaper.sh:55` 는 마커 없으면 AWS 조회조차 안 한다). 즉시 수동 마커 + DRYRUN 으로 복구 확인. **하네스 동결 규칙 해제 조건 충족**(이론적 구멍이 아니라 실제 과금 중 발생) → 래퍼 허용 + **회귀 24건 신설**(종전 스위트에 마커 케이스 **0건**이었다, MIN_CASES 149→173). ⚠️ **그 수정이 새 오탐을 열었다**(따옴표 안 래퍼 → 커밋 메시지가 마커 생성). L-22 의 *"조일 때마다 새 우회"* 재현 — 실패 방향이 반대(과잉감시·자가치유)라 **명시적으로 받아들이고 케이스로 고정**(L-45). 🔴 **퀴즈 4/5 — Q4 는 내가 틀렸다**: 노드당 **루트 EBS 20GiB** 를 비용 모델에서 빠뜨린 것을 사용자가 지적, 문서 4곳 정정. 08-31 에도 사용자 질문이 계획서 오류를 찾았다 — **두 세션 연속**. 원장 L-43 closed(측정 완료) · L-44 obsolete(**SOP §2b 에 이미 있던 것을 중복 등재했다** — 등재 전에 검색하지 않음) · L-45 신설 | 2026-09-04 |
| **#407 · #408** | **★ 착수 전 $0 검산이 계획을 뒤집었다 — Stage C 재개 = 노드 2대 → **3대** (D-011).** 08-31 이 남긴 *"1순위 = 노드 2대"* 를 클러스터 없이 산술로 검산했더니 성립하지 않았다. 406Mi 의 DaemonSet÷Deployment 분리가 **미측정**이라 양극단을 다 따졌다: **비관**(모든 노드 여유 959Mi)이면 2대는 512Mi 세 개 중 하나가 **어디에도 안 들어가 Pending**, **낙관**(A 959·B 1245)이어도 **스케줄 순서에 따라** 갈린다. 🔴 **3대는 비관 가정에서도 성립** = 결론이 미측정 값에 **의존하지 않는다**. 차액은 세션당 ~~$0.021~~ **$0.0236**(노드 1대 = EC2 $0.0208 + 공인IPv4 $0.005 + 루트EBS $0.0025 = $0.0283/h — 09-04 정정) — *검증 불가능한 스케줄링 제약을 과금 중에 거는 것보다 2센트로 여유를 산다.* 🔴 **틀린 주장 3개를 함께 정정**: ①*"1024>959 즉시 초과"* → 스케줄러는 **안 맞는 노드에 놓지 않는다**. 증상은 OOM 이 아니라 `Pending`, 원인은 **조각화** ②*"`podAntiAffinity` 로 해소"* → **불충분** (시스템 Deployment 배치가 통제 안 됨 → 노드그룹 분할 필요) ③설계서의 *"노드 2개는 **파드 슬롯만** 푼다"* → **이 문장이 D-009 의 2대 기각 근거였고 그 기각이 08-31 유료 세션을 날렸다**(슬롯은 애초에 제약이 아니었다). → **L-43 을 블로커 → 관찰 항목으로 강등**, CONTEXT↔L-43 모순 해소. **$0 부수 실측**: `describe-addon-configuration` 은 `resources` **스키마만** 주고 기본값을 안 준다(406Mi 분리는 $0 경로 **없음**) · ESO 3파드 requests 는 **비어 있고**(BestEffort → 압박 시 **가장 먼저 evict**) helm 수동설치라 **406Mi 에 안 잡혀 있었다**. QA 3라운드 F-1~F-5 전건 `fixed` — 🔴 **F-1 은 내 산술 오류**(다른 시나리오의 423 을 섞음, 정답 447), 🔴 **F-5 는 내가 `design-change-procedure` §4 를 어긴 것**(원본 삭제). 하필 **원본으로 안 돌아가서 생긴 PR 에서** 원본을 지웠다 — 취소선+`<details>` 로 복원하고 `gh api /markdown` 으로 실렌더 검증 **#408 (원장 L-44)**: 다음 세션 전 ECR 사전점검에서 **다른 결함**이 나왔다 — `ecr-push.yml` 이 `github.sha` 로 태깅하는데 **`pull_request` 이벤트에서 그 값은 GitHub 이 만든 일회성 머지 커밋**(`refs/pull/N/merge`)이다. 실측 `git log a9b3a49` → `fatal: bad object`, `gh api` → `Merge 74c2f843 into bd51e18f`. 🔑 **워크플로 자신의 논거를 무너뜨린다** (*"sha 태그를 쓰면 지금 뭐가 도는지 안다"*) — 조회가 안 되니까. 게다가 **PR 경로가 stage 브랜치의 주 경로**다. 부수 효과로 `latest` 가 서비스마다 **다른 코드**를 가리킨다. **하네스 동결 규칙에 따라 미수정** — 세 서비스 모두 **`bd51e18`(실재 main 커밋)** 에 있고 `be/` 는 이후 **변경 0건**, 셋 다 **arm64/linux** 라 제품이 안 막혔다. 🔑 **교훈: 개수가 아니라 태그 일치를 봐라** — 08-31 에 개수만 보고 `None` 을 놓쳤고, 이번엔 개수가 3/3 인데도 하나가 추적 불가였다. | 2026-09-03 |
