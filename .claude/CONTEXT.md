...
| **#413** | **k6 생성기 노드 격리 — 3중 장치 + 노드 비우기 절차 ($0, AWS 접촉 0건).** 램프 유료 세션을 착수하려다 `k6.yaml` 에 **§9.5 가 요구한 노드 고정 장치가 하나도 없는 것**을 발견. k6 는 `cpu 500m` = t4g.small 의 **25%** 라 같은 노드에 뜨면 앱 자원을 빼앗은 채 그 앱을 잰다 — **09-06 을 폐기시킨 실패와 같은 종류**. 🔑 **1차 해법이 성립하지 않았다**: 측정 대상이 4개라 노드를 **전부 채울 수 있어**(비둘기집) `required` 는 k6 를 **과금 중 Pending 에 가두고**, `preferred` 는 **조용히 같은 노드에 올린다**. → 빈 노드는 **절차로 비운다**: `nodeSelector` + **cordon taint toleration**(cordon 은 taint 라 없으면 k6 자신도 못 들어간다) + `required` anti-affinity(**가드 — 오염을 막는 게 아니라 침묵을 막는다**), 비우기는 `pin-loadgen-node.sh`. **QA 3라운드 · 지적 8건 전건 fixed · 최종 신규 0.** 🔴 **F-6 은 자해**: 같은 커밋의 주석이 `kubectl get nodes \| tail -1` 로 **아무 노드나** 고르라고 적었다 — **바로 위 15줄의 비둘기집 설명과 정면 모순**. 과금 창에서 그대로 쳤으면 격리가 깨진 채 측정이 돌았다 → 절차의 **단일 출처를 스크립트로** 못 박음. 🔴 **F-7 이 실질 수확**: 내가 넣은 검증이 `grep -c x \|\| true` 라 **"정말 0개"와 "kubectl 실패로 입력이 빔"이 똑같이 0** → 조회 실패 시 `left=0 notready=0` 으로 **✅ 조기 선언**. ***이 PR 이 없애려던 실패를 검증 단계에 재도입한 것.*** `pod_snapshot()` 분리 + **종료코드 확인**, 센티널 `-1` 로 *"한 번도 못 쟀다"* 와 *"쟀는데 0 이 아니다"* 를 구분(전자는 **미확인**이지 실패가 아니다). 검증 7경로 무과금(가짜 kubectl), 마지막 둘에서 **✅ 출현 0회**. ⚠️ 목 하네스 버그를 **3번** 잡았다(인자 위치 2건 · 패턴이 부분문자열로 샘) — **검증 도구도 검증 대상이다.** 📌 `case` 를 `$( )` 안에 두면 **bash 3.2 가 패턴의 `)` 를 명령치환 종료로 해석**(macOS 기본 sh). 원장 L-47 에 **노드 예산 중복 보정** 요구 추가 — 생성기가 1대를 가져가므로 램프는 `node_desired_size=3`(앱 2 + 생성기 1) | 2026-09-08 |
