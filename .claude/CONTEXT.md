...
| 항목 | 내용 |
|------|------|
| 진행 중인 트랙 | **Phase 2 — `daily-service` 추출**. Stage A ✅ · B ✅ · **Stage C 준비 완료 — 블로커 C-1~C-8 전부 해소**(#397·#396·#399·#400·#402·#403·#404) → **남은 것은 apply 뿐. 여기서부터 과금.** 예상 **$0.149/h**(t4g.medium 실측), 왕복 40~50분 ≈ **$0.12**. 🔴 **착수 순서**: ①`docs/eks-session-sop.md` 정독(통시간 확보 확인이 1번 항목) ②사용자 승인 후 `tofu apply` ③`k8s/README.md` §3(배포·core-api 먼저) → §3.5(NetworkPolicy, **애드온 먼저**) → §4.1(차단/허용 쌍) → §4.2(e2e, 응답에 `[STUB]` 표식) ④**destroy**. 브랜치 `stage/eks-*`, **PR 전 이해도 퀴즈 필수**(`assert-eks-quiz.sh` 차단). 🔑 **그 세션에서 반드시 잴 것: 노드 `Allocatable.memory`**(레포 기록 0건 — requests 1792Mi 가 들어가는지가 여전히 추정) |
