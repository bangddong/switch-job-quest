...
| 항목 | 내용 |
|------|------|
| 진행 중인 트랙 | **`prod → EKS 상시 운영` 선행 조건** (**D-013**, `plans/2026-09-11-prod-eks-migration-prereqs.md`). 구축(Stage 0~4b)은 끝났다. **1 ✅ · 2a ✅ · 2b 재판정 해소 ✅ · 3 HTTPS ✅(#426)**. **4 RSS creep = 🟡부분** — *측정*은 $0 로 끝났고(#428) **판단이 안 끝났다**: creep 기울기 재측정(`fly ssh console` 며칠 반복 — 🔑 **스케줄러 발화 전후로 갈라 찍을 것**, 메타스페이스 +4.2 MiB 가 알려진 일일 점프와 크기가 같아 두 가설을 가르지 못한다) + 대응 선택지 결정(①limits 768Mi ②NodeSwap ③creep 해결 ④1GB 인스턴스). 🔴 **5 는 선행 조건에서 빠졌다 (09-19, #430)** — 📌D-008(`✅유효`)과 충돌. **이관 직후 첫 작업**으로 이동. 🔴 **6 은 쪼갰다 (09-21)** — `6a` 토폴로지 무관 $0 분 / `6b` **ArgoCD gitops → 이관 후**. 6b 하드 블로커 2개: Ⓐ`design-integrity` verify 앵커가 **렌더 매니페스트 커밋을 기계적으로 실패**시킨다(퍼블릭 레포 제약이 CI 로 구현돼 있다) Ⓑ**ArgoCD selfHeal + SOP §8 + 리퍼 = 고아 ALB 확정 경로**. 순서도 뒤집힌다(6 이 7·B-18 을 선취, D-002 를 건드림). 🔑 Ⓑ·순서·D-002 는 전부 **destroy-after-use 한 뿌리**라 이관 후 풀린다 — **Ⓐ만 남는다.** → 🔴 **다음 = 6a ② 치환 체인 일원화**(6종 PLACEHOLDER 가 문서 3곳에 산재, `--dry-run=client` 로 검증, 클러스터 불요) → **7 안전장치 개정($0)** → **블로그 원고($0)**. ⚠️ Stage 5(HPA·ArgoCD)는 README 가 "선택". 🔑 **규모 감각**: 첫 답(기술 2~3주)이 틀렸고 Blindspot 22건 중 6건이 *"이 계획은 성립하지 않는다"* 수준이었다 — 실제 **2~3개월**. |
...
| **#430** | **선행 조건 5 를 $0 재분류 — 📌D-008 과 충돌해 이관 후 작업으로 이동.** 착수 설계 ⑤가 시킨 *"5개 질문에 먼저 답하라"* 를 실행하니 **질문 목록에 없던 것**이 나왔다: D-008(`✅유효`)이 *"ANTHROPIC_API_KEY 를 학습 클러스터에 넣지 않는다"* 인데 **클러스터는 하나**라 선행 조건으로 태우면 곧 D-008 반전이다. 우회로(*별도 예산 키*)는 D-008 의 「기각한 대안」에 **이미 닫혀 있었다** — 새 길을 찾은 게 아니다. 🔑 **판별 기준이 생겼다**: D-008 의 근거는 *"**학습** 클러스터"* 에 걸려 있어 **이관 전엔 해소 불가·이관 후엔 적용 안 됨** → *"선행 조건"의 자격이 없다.* QA 표현이 정확하다 — ***"뒤집힌 게 아니라 강화됐다. 항목 5 가 D-008 에 맞춰 스스로 물러났다."*** 🔴 **QA F-1(MEDIUM)**: 표 주석에 *"6건이 아니라 5건(1·2a·3·4·6·7 중 5는 빠짐)"* 이라 썼는데 **괄호 안에 6개를 나열해놓고 5건이라 했다** — 세어보지 않고 숫자를 썼다. ⚠️ **원장 L-64**: 소스 주석 2곳의 `17개`→`16` 오프바이원을 **미룬다** — `be-cd` 가 `paths: ['be/**']` 라 **주석 한 글자에 Fly prod 가 재배포**된다 | 09-19 |
