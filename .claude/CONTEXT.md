...
| 항목 | 내용 |
|------|------|
| 진행 중인 트랙 | **`prod → EKS 상시 운영` 선행 조건 7건** (**D-013**, `plans/2026-09-11-prod-eks-migration-prereqs.md`). 구축(Stage 0~4b)은 끝났다. **1 백업·복구 ✅ · 2a 시크릿 환경 축 ✅ · 2b 재판정으로 해소 ✅ · 3 HTTPS ✅(#426)** → 🔴 **다음 = 4** — ~~메타스페이스 누수 검증~~ **재정의됨(09-18)**: 메타스페이스 누수는 이미 없음이 확정이고, 살아 있는 리스크는 **RSS creep + K8s swap 부재**다(B-5). **$0 로 실측 완료** — Fly 앱 실효 665 vs K8s 576 MiB. **$0 실측 완료(#428)** — 남은 것은 **creep 기울기 재측정($0)** 과 **대응 선택지 결정**. → 🔴 **다음 = 5 실 AI·메일·채점 경로.** ⚠️ **$0 사전 조사 완료(09-19) — 과금 세션이 아닐 수 있다**: 메일은 `MAIL_ENABLED:false` 기본값이라 **안전**하고, egress 는 **NetworkPolicy 가 Ingress 만 제한**해 AI·Judge0 도달에 **제약이 없다**(=$0 로 종결). ESO→env 주입은 2a 가 이미 증명했다. **착수 첫 작업은 '실행'이 아니라 「항목 5 — 착수 설계」 ④의 5개 질문에 답하는 것** — 특히 *"K8s 에서만 새로 드러나는 것이 무엇인가"*. 남지 않으면 항목 4 처럼 **$0 으로 재분류**한다 → 5 실 AI·메일 경로 → 6 EKS CD → 7 상시 운영 전제로 안전장치 개정. 그 뒤 **블로그 원고($0)**. ⚠️ Stage 5(HPA·ArgoCD)는 README 가 "선택". 🔑 **규모 감각**: 첫 답(기술 2~3주)이 틀렸고 Blindspot 22건 중 6건이 *"이 계획은 성립하지 않는다"* 수준이었다 — 실제 **2~3개월**. |
