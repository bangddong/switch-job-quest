...
| 항목 | 내용 |
|------|------|
| 진행 중인 트랙 | **Phase 2 — `daily-service` 추출**. Stage A ✅ · B ✅ · 🎉 **C ✅ 완료 (2026-09-04 유료 세션, 33m06s ≈ $0.1029, 고아 0).** 노드 **3대**로 3서비스 기동 + `daily→ai` e2e + **C-5 NetworkPolicy 차단/허용 쌍 검증**(여러 세션 미검증이던 것). 전 파드 재시작 0. 🔑 **D-011 이 실측으로 확증됐다** — 2대였다면 `daily-api` 에서 Pending(실측 A 959 · B 1261 대입). 🔑 **측정 2건 완료**: ①406Mi = DaemonSet **104Mi/노드** + Deployment **302Mi**(coredns 는 70Mi — `addons.tf` 의 ~100Mi 추정은 틀렸다) ②**`requests: 512Mi` 는 실사용의 2배**(합 842Mi vs 1792Mi: core 337 · daily 287 · ai 183 · pg 35). 🔴 **다음 착수 지점 = 부하 상태에서 메모리 재측정 → requests 재산정 → D-011(3대) 다시 열기.** ⚠️ 부하 측정 없이 줄이면 evict 되고 증상이 다시 "용량 부족" 으로 보인다(QoS Burstable, requests 초과분이 클수록 먼저 evict). 🔑 **비용 교훈**: 예상 65분/$0.19 → 실제 33분/$0.103. 진단·재빌드·산술을 전부 $0 구간에서 끝내고 과금 구간엔 실행만 남긴 결과(08-31 은 반대로 과금 중 진단해 51분). **"과금 구간에서 무엇을 하지 않을지" 가 비용을 정한다.** 🔴 **비용 모델 정정(퀴즈 Q4, 사용자 지적)**: 노드당 **루트 EBS 20GiB $0.0025/h** 를 빠뜨렸다 → 노드 1대 한계비용 $0.0258 → **$0.0283/h**. 정정할 때마다 다른 항목을 빠뜨렸다(D-009 는 IPv4 포함, 08-31 정정은 IPv4 누락, 09-03 재정정은 루트EBS 누락) |
