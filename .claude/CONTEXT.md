...
|------|------|
| 브랜치 | `main` |
| 열린 PR | 없음 — AI 메트릭 "0" 조사는 버그 아님으로 결론(비자명적 결정 참고), 코드 변경 없이 브랜치 정리 |
...
### 코드 작업
- [ ] **Grafana `DevQuest - AI Metrics` 대시보드 패널 쿼리 수정** (내일 이어서 — 진행 중 중단됨)
      - 목적: 모델별 토큰 사용량 보고 싶은데 현재 패널이 `increase(metric[$__rate_interval])`(짧은
        트레일링 윈도우)라 트래픽 뜨문뜨문한 저사용량 앱 특성상 항상 0으로 보임
      - 확인 완료(재조사 불필요): `sum(increase(ai_tokens_input_total[12h]))`로 직접 검증함 —
        Prometheus/Mimir는 재배포로 카운터가 리셋돼도 `increase()`가 리셋을 감지해 정확히 누적
        합산함(오늘 재배포 2번 넘어 0→1474→3139.18 정상 누적 확인). DB(AiCallLog) 연동 불필요,
        Grafana 패널 쿼리만 고치면 됨
      - 할 일: "Input/Output Tokens by Evaluator", "Cache Tokens" 패널을 Time series(라인)에서
        Bar gauge/Table + Instant 쿼리로 전환, `sum by (evaluatorType, model) (increase(metric[$__range]))`
        형태로 변경. Overview 섹션 stat 패널(Total AI Calls 1h, Success Rate 1h, Cache Hit Rate 5m,
        P95 Latency 5m, Retries 1h)도 고정 윈도우 대신 `$__range` 연동 고려
      - 작업 방식: Grafana Cloud UI 직접 편집(레포에 대시보드 JSON 없음, IaC 아님) — 브라우저 세션
        재개 필요. 중단 시점: 브라우저 선택 프롬프트에서 사용자가 "내일 할게"로 중단
