...
| **#417** | **선행 조건 1 — DB 백업·복구 리허설 (32m10s ≈ $0.0848, 고아 0).** 🔑 **B-8(백업 0건) 종결. EKS 트랙이 "세우는 법"에서 "관리형이 조용히 해주던 일을 직접 하는 법"으로 넘어간 첫 항목.** 볼륨을 **실제로 파괴**한 뒤 S3 덤프만으로 복구 성공 → core-api 기동에서 `Schema is up to date. No migration necessary.` (복구가 부분 실패였다면 여기서 migrate 를 시도한다). 🔴 **착수 직전 내가 단언한 것이 틀렸다** — *"볼륨을 실제로 파괴해야 명제가 반증 가능해진다"* → **아니다.** Flyway 가 26행을 재생성하므로 **파괴 강도와 무관하게** 두 가설이 같은 결과를 예측한다. ***판정력은 검사 절차가 아니라 저장소의 성질에서 온다.*** → **센티넬 행**(마이그레이션이 못 만드는 행) + **복구 전 부재 확인**(없으면 *"애초에 안 지워졌다"* 와 구별 불가). 🔑 **그 전제를 실측으로 올렸다**: `DROP SCHEMA public CASCADE` 후 복구 없이 core-api 만 기동 → 테이블 14 · `tech_question_bank` **26행** · `backup_sentinel` **부재(f)**. **두 가설이 갈리는 지점을 만들어서 봤다** — #416 의 메타 교훈을 주장이 아니라 실험으로 확정. 🛡️ **파괴 경로를 바꿨다**: `tofu destroy -target` 은 `prevent_destroy`(0-bootstrap 전체의 오타 방지 latch)에 막히고, 그걸 빼는 PR 이 머지되면 `infra-deploy.yml` 이 0-bootstrap 을 **자동 apply** 해 로컬 destroy 와 경쟁 → **`state rm` + `delete-volume`**(terraform 이 모르는 채 볼륨이 사라지는 **진짜 재해에 더 가깝다**). 🔑 **`Retain` 실물**: PVC/PV 가 **1초**에 사라져도 EBS 는 살아 있다 = *PVC 만 지우는 리허설이 무의미한 이유*. 💰 **#416 보다 길면서 15% 싸다** — 노드 3→2(**−$0.0152**)가 시간 증가(+$0.0024)를 **6배** 압도 — ⚠️ 처음엔 두 세션 노드 **항목의 차**($0.0132)를 적었는데 거기엔 **기간 차가 섞여** 있다. *"노드 한 대 줄여 아낀 돈"* 의 답은 **같은 기간 노드 1대 값**이다(QA F-11). ***#414 메타 교훈 그대로: 숫자는 맞는데 그 숫자가 답하는 질문이 내가 던진 질문이 아니었다***. **레버가 둘인데 당길 시점이 다르다**: 단가는 **apply 전에만**, 시간은 **세션 중에만**. 컨트롤플레인 $0.100/h 가 **63%** 라 **세션을 쪼개면 손해**(09-06 손익분기 19분의 산술). 🔴 **"통과했는데 엉뚱한 이유로 통과" 가 이 PR 에서만 다섯 번**: 목 시나리오 누수 · verify 마커 3개가 주석 매칭(**08-12 F-5 → #416 → 회귀 스위트 ④⑤ → 네 번째**. ⚠️ 처음엔 사슬에서 3번째를 빠뜨렸다 — 일지는 회귀 스위트 건을 *"3번째 재발"* 로 세어 뒀다) · 정적 검사 ④⑤ 무력 · ⑳이 trap 만으로 충족돼 ⑲의 중복 · 반증 하네스의 `ROOT` 가 `/` 가 되어 **두 arm 모두 통과**. 🔴 **QA 4라운드 · 지적 10건. F-7 은 고쳐진 적이 없는데 `fixed` 판정을 받았다** — 내가 준 *"처리 요약"* 이 닻을 내려 검토가 **주장과 코드를 대조하는 대신 주장을 확인**했다(`grep` 한 줄이면 드러났다) → 원장 **L-52**(패턴 편집의 no-op 이 종료코드 0). F-1·F-2 는 QA 가 직접 재현한 실제 결함(**복구 성공 후 앱이 죽은 채 남는다**) → `trap ... EXIT`. F-3 → **L-51**: **L-49 의 두 기준(방향 vs 되돌림 비용)이 서로 반대를 가리키는 첫 표본.** 퀴즈 정답2/오답2, 메타 교훈 = ***"조건절을 떨어뜨리면 경고가 거짓말이 된다"*** — Q4 가 내 주석의 과장을 잡았다(daily-api `repair()` 위험은 게이트로 이미 막힌 **잠재**인데 **현재**처럼 단정). **요약이 정보를 잃는 지점은 문장이 아니라 한정사다** | 2026-09-13 |
| **#416** | **EKS Stage 4 — ALB Ingress (30m44s ≈ $0.0996, 고아 0).** 🔑 **EKS 이관의 마지막 필수 구축 단계 완료.** Blindspot Pass 가 착수 전제 **3건을 뒤집었다**: ①서브넷 `role/elb` 태그가 **이미 있었다**(내 grep 이 `alb` 를 찾느라 `elb` 를 놓쳤다) ②LBC 는 **애드온이 아니다**(실측 `None`) → helm ③두 서비스가 `/api/v1/daily-question` 을 **겹쳐 서빙**(strangler 잔재) → prefix 분할 불가. **Blindspot 도 못 찾은 것**: 공식 IAM 정책 **8955자 > 상한 6144자**라 `jsonencode(jsondecode())` 필수(`file()` 이면 apply 실패). 🔴 **조용한 실패 3종 차단**: `target-type: ip`(기본 `instance` 는 NodePort 요구 → ClusterIP 면 Ingress 는 `created` 인데 **ALB 만 안 생긴다**) · `healthcheck-path: /health`(기본 `/` 는 401 → 전 타겟 unhealthy 502 인데 **파드는 1/1 Running**) · **actuator 미라우팅**(permitAll 된 readiness 는 **kubelet 용**이었고 그 판단 때 ALB 가 없었다 → 실측 404). 🔑 **ADDRESS 가 20초에 채워진 것 자체가 `target-type: ip` 의 증거**(LBC 가 ALB 를 만든 뒤에야 status 에 쓴다). 🔴 **응답으로는 라우팅을 판정할 수 없었다** — 두 서비스가 같은 DB 행을 읽어 **본문 완전 동일** + 액세스 로그 없음 → **반증 실험**(daily-api 를 0 으로 내리니 그 경로만 503, 다른 경로 403)으로 확정. ***존재로 구분 안 되는 것을 부재로 구분한다.*** 🛡️ **리퍼에 ALB 회수 추가 — 가장 심각했던 구멍**: 리퍼가 도는 조건이 곧 수동 teardown 이 안 되는 상황인데 `tofu destroy` 만 했다(ALB 는 state 밖, 월 $16.43). destroy **전** 삭제 + 후 경고, **타임아웃 필수**(finalizer hang 으로 멈추면 안전장치가 벽돌). 회귀 11건 신설. teardown 실전에서 `delete ingress` 가 **16초**(CPU 1%) = **finalizer 실물 증거**. 🔴 **verify 마커가 처음에 무력했다 — 08-12 F-5 재발**: 마커가 **코드를 설명하는 주석에 매칭**돼 코드를 지워도 통과. 교훈이 문서에 있었고 읽었는데도 같은 걸 만들었다 → 선언문 형태로 못 박고 4개 전수 반증. 💰 **ALB 가 전체의 4% 뿐**($0.0040) — 고정 단가는 세션의 25% 인데 **마지막 7분만 켰다**. 순서(앱→ALB→ALB 먼저 삭제)가 비용을 정한다. ⚠️ LCU 는 미포함(1200 req/s 면 ~$0.096/h). **QA HIGH·MEDIUM 0** (QA 가 회귀·반증을 직접 실행해 판정력 확인). 퀴즈 부분3·오답1 — **오답 1건이 기록 오류를 잡았다**: `Evicted 0` 으로 *"requests 명시가 유효했다"* 고 적었으나 두 가설이 같은 결과를 예측(여유 ~1300Mi 였으니 원래 안 났을 수도) → 정정. 메타 교훈 = ***"검사의 판정력은 정확도가 아니라 두 가설이 다른 결과를 예측하는가에서 온다"*** — 네 장면 중 둘은 갈렸고 둘은 안 갈렸는데, **한 번은 알아채 반증으로 갔고 한 번은 못 알아채 인과로 적었다** | 2026-09-11 |
