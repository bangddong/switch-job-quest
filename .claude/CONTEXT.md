...
### OOM 진짜 원인 확정 — 512MB에 앱이 안 들어감, 누적형 RSS 포화 (2026-07-07)
#239 JVM 튜닝 후에도 재발 (7/6 00:10 UTC, exit 137). Fly RSS 실측으로 인과 확정:
- 부팅 직후 가용 마진 **~44MB뿐** (총 459MB 중), 이후 무트래픽에도 **시간당 ~3MB씩 감소**
  (JVM이 커밋 페이지를 서서히 터치 — JVM 지표론 안 보임, 커밋 308MB 고정)
- 09:00 KST DailyMailScheduler가 -8MB 소비 → 마진 바닥이면 ~10분 뒤 kill (6/30·7/1·7/6 09:11 KST 패턴)
- **잡은 방아쇠일 뿐** — 잡 없어도 마진 소진되면 아무 때나 죽음 (7/2~7/5 새벽·오후 사망)
- 사망 시점 HTTP/AI 요청 항상 0건 — 메일 발송·AI 콜 영향 없었음. 리스크는 "AI 평가 도중 kill" 시나리오
- 대응: fly.toml `swap_size_mb = 256` (1차), 재발 시 1GB 스케일업 (~$5.7/월)
- 조사 방법 메모: Grafana 스택 datasource proxy fetch로 PromQL 직접 쿼리 + fly-metrics.net
  (`prometheus_on_fly` datasource, `fly_instance_memory_mem_available`) + `process_uptime_seconds` 리셋으로 재시작 전수 추출
...
| 항목 | 내용 |
|------|------|
| 브랜치 | chore/fly-swap-oom-mitigation |
| 열린 PR | #245 — fly.toml swap 256MB, OOM 완화 (머지 대기) |
...
| #239 | OOM 재시작 대응 — JVM 메모리 예산 튜닝 (힙 50%, Metaspace 160m, Xss512k). 배포 성공·health 200. 며칠간 OOM(exit 137) 재발 관찰 필요 | 2026-07-04 |
| #240 | 질문 뱅크 wiki 시드 2026-07 — 면접 concept 페이지 12건 wiki 생성(DB·JPA·JVM·인프라) + V11 질문 21건 시드. 머지·배포 완료(health 200), 뱅크 총 26건 | 2026-07-05 |
...
- [x] client-ai Jackson 2/3 혼재 정리 완료 (#243) — client-ai 전수 tools.jackson 통일.
      root 전역 Jackson 2 kotlin 모듈은 유지 (victools transitive 필요, SB4 병행 패턴) — 전체 교체는 별도 범위
- [ ] 에이전트 Disambiguation Gate / Closing Summary 미비점 보완 (Gate 횟수 상한, 트리거 기준 명시 — 실사용 경험 더 쌓은 뒤 결정)
- [x] 기술면접 질문 뱅크 1차 확충 완료 (#240, 21건 — 뱅크 총 26건). 이후 wiki 페이지 쌓이면
      SessionStart 훅 알림 따라 반복 — `/question-bank-seed` 스킬로 반자동화됨.
