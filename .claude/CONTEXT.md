...
| 항목 | 내용 |
|------|------|
| 진행 중인 트랙 | **Phase 2 — `daily-service` 추출**. Stage A ✅ · B ✅ · **Stage C 준비**: 감사 ✅(#396) · BE 블로커 ✅(#397) · **C-3 ECR ✅(#399, `devquest/daily-api` 실제 생성)** · **C-4/D-009 노드 t4g.medium ✅(#400, 단가·파드상한 실측)** → **남은 것: C-5 NetworkPolicy · 매니페스트 2벌(`ai-api.yaml`·`daily-api.yaml`) → apply/이미지3개/e2e/destroy**. 여기까지 전부 $0 이고 **과금은 `2-cluster` apply 에서만 시작**한다(자동 apply 대상 아님). ⚠️ apply 전 `docs/eks-session-sop.md` 필독 + 사용자 승인. 브랜치는 `stage/eks-*`, **PR 전 이해도 퀴즈 필수** |
...
| **#396 · #397** | **★ Stage C 착수 전 감사 + BE 준비 (08-28, 비용 $0 — 클러스터 미가동).** **#396 감사**: 계획서·설계서를 실측과 대조. 🔴 **내가 직전에 보고한 "계획서 자기모순"은 틀렸다** — `:83`(*"배포하는 단계이지 새로 만드는 단계가 아니다"*)은 **`be/` 산출물** 서술이고 `#31`(*"매니페스트 0건"*)은 **인프라** 서술이라 범위가 다르다. 내가 `:83` 을 매니페스트까지 확대 적용했다. 다만 그 범위 차이가 어디에도 안 적혀 있어 오독을 부르므로 명시했다. 🔑 **진짜 발견은 계획서가 반대로 적혀 있던 것**: 열린 결정 ②(스키마 분리)를 *"Stage C 에서 필요해진다"* 고 적었으나 **지금 시행하면 Stage C 가 깨진다** — ①daily-api 는 `validate` 로 db-core 엔티티 15개 전부를 검증해 기동 불가 ②마이그레이션 주체가 core-api 하나로 고정이라 **daily 스키마를 만들 주체가 없다** ③엔티티 클래스가 db-core 공유라 한쪽만 다르게 볼 수 없다 ④`V13:40` 백필의 `FROM daily_mail_log`(V6, core-api 소유)가 교차 스키마 참조가 되는데 V13 은 체크섬 때문에 수정 불가. **공유 스키마는 장애물이 아니라 전제다.** 실제 만기는 *"daily-api 가 생길 때"* 가 아니라 **"DB 물리 분리 또는 daily-api 가 자체 마이그레이션을 소유할 때"**. 확정 결정 3건: **D-008**(학습 클러스터 AI 는 **스텁** — `secrets.tf` 의 *"prod 크리덴셜 금지"* 원칙 + 더미 Grafana 키 유출 전례. 대가로 **Stage C 가 증명하는 것은 토폴로지이지 AI 품질이 아니다** — 완료 기준 문장을 정정) · **D-009**(노드 **t4g.medium** 1대. 노드 2대는 파드 슬롯만 풀고 **파드당 512Mi 메모리 벽**은 그대로, 단일 AZ 핀이라 가용성 이득도 없고 비용도 더 비싸다) · **C-6**(e2e 는 **port-forward**, Ingress 는 Phase 3 — 충돌이 아니라 범위 차이였다). 정정한 낡은 서술: Stage B `⬜`→✅, #10(*"HTTP 어댑터가 core-api 안에"*)→거짓, `:308` 11/11→**10/11**(coredns=1 반영), 설계서 *"앱 300~450MB×3"*→실측 **requests 512Mi/limits 900Mi**. **미확인 4건**(노드 allocatable·medium 단가·medium 파드상한·힙 실사용)을 표시하고 잴 명령을 남겼다. **#397 BE 준비**: C-1 daily-api 헬스(actuator 의존 0건 → `core-api.yaml` 복제 시 startupProbe 30회 실패 후 컨테이너 사망) + D-008 스텁. 🔴 **블로커를 고치며 새 구멍을 낼 뻔한 자리를 QA 가 실측으로 막았다** — daily-api 엔 Spring Security 가 없는데 actuator 를 붙였다. QA 가 **앱을 실제로 띄워** `/actuator` 를 조회: `health` 하나만 등록되고 `env`·`beans`·`heapdump`·`configprops` 는 **등록조차 안 됨**(DB 비번 노출 없음). 스텁 off-by-default 테스트도 **환경변수로 강제 활성화해 FAIL 을 확인**(공허한 단언 아님). 525 tests 0 failures · **산출물 334 불변**. 원장: L-36(`be-cd.yml` 모듈 무제한 `test` → daily-api 22건이 Fly 배포 게이트에 편입. **하네스 동결 규칙**상 막힌 증거 없어 미룸) · L-37(`spring.flyway.enabled` 는 Boot 4 에서 **죽은 키**). QA HIGH 0 · F-1(LOW, wontfix — **빌더 보고서가 틀렸고 코드가 옳았다**) | 2026-08-28 |
| **#395** | **★ Phase 2 Stage B-2b 완료 — `core:daily-api` 앱 신설 (08-27, 비용 $0).** 재료 4개(`daily-core`·`client-ai-http`·`db-core`·`core-domain`)를 조립한 얇은 앱. 26파일 신규(+1114), core-api 무변경. 🔴 **착수 전 Blindspot Pass 가 계획에 없던 파괴 경로 3개를 잡았다.** ①공유 `db-core.yml:11` 이 `ddl-auto: create-drop` 인데 안전판(`validate`)은 **core-api prod 프로필에만** 있었다 → import 하고 오버라이드를 빠뜨리면 **부팅 시 전체 스키마 drop-create**. Flyway 게이트는 이걸 하나도 못 막는다. **대응: `db-core.yml` 을 아예 import 하지 않는다** — 위험한 기본값을 상속할 경로 자체를 제거. ②**`@EntityScan` 이 레포 전체 0건** — core-api 가 도는 유일한 이유는 앱 클래스가 루트 `com.devquest` 에 있어서이고, **`scanBasePackages` 는 엔티티 스캔 기준에 영향을 주지 않는다.** `ai-api` 는 JPA 를 안 써서 이 함정을 밟은 적이 없다 — **선례가 없어서 안 보이는 종류**였다. ③HTTP 전송의 `@ConditionalOnProperty(havingValue="http")` 를 복사하면 core-api 엔 있는 인프로세스 폴백이 daily-api 엔 없어 **`TechInterviewPort` 0개 → 컨텍스트 붕괴**. 🔑 **빌더가 Blindspot 도 못 본 4번째를 스스로 찾았다** — 마이그레이션이 **두 모듈에 분산**돼 있다(core-api V1~V6·V8·V9 / db-core V7·V10~V13). daily-api 는 db-core 만 의존하므로 앞 8개가 클래스패스에 없고, Flyway 를 켰으면 `repair()` 가 그걸 DELETED 로 마킹 → core-api 가 다음 기동에 재실행하며 **영구 부팅 불가**(2026-07-01 V8 사고 경로). 🔴 **내 검증이 한 번 거짓 통과였다** — `bootJar` 가 `Unable to locate a Java Runtime` 으로 죽었는데 뒤 단계가 그대로 돌아 **기준선 만들 때 남은 옛 jar 를 자기 자신과 비교**하고 "불변 OK"를 찍었다. exit code 를 따로 출력해둬서 걸렸다. **이 환경은 foreground 셸에 Java 가 없다 — Gradle 은 background 로 돌려야 한다.** (같은 형태로 `${PIPESTATUS[0]}` 도 빈 값이었다 — **이 셸은 zsh** 라 `$pipestatus`, 1-index. push 는 exit code 대신 **SHA 대조**로 확인했다.) 검증: build exit 0 · **519 tests 0 failures**(직전 497 유지) · **산출물 334 불변**(diff 공집합, `com/devquest/daily/` 유출 0, MANIFEST·layers.idx·classpath.idx 동일 — QA 독립 재검증) · **단독 기동**(RANDOM_PORT 실서버 + JDK HttpClient, ai-api 없이 200/404). QA HIGH 0. 원장: **L-36**(be-cd `test` 가 모듈 무제한 → daily-api 22건이 Fly 배포 게이트에 편입. **하네스 동결 규칙**상 증거 없이 안 고친다) · **L-37**(`spring.flyway.enabled` 는 Boot 4 에서 **죽은 키** — `spring-boot-flyway` 아티팩트 미의존. 실제 게이트는 db-core `FlywayConfig` 하나) | 2026-08-27 |
