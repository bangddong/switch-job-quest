...
| **#399 · #400** | **★ Stage C 준비 — 배포 경로 + 노드 용량 (08-30·08-31, 비용 $0).** **#399 C-3**: `daily-api` ECR 레포 신설. 두 군데가 막혀 있었다 — `ecr_repositories` 기본값에 없고, `ecr-push.yml` 은 **PR 트리거에 `inputs` 가 없어 항상 core-api 로 폴백**해 다른 서비스 이미지를 구조적으로 만들 수 없었다. 🔑 **`ecr.tf` 가 이미 안전했다** — `aws_ecr_lifecycle_policy.app` 이 `for_each = aws_ecr_repository.app` 이라 **상한 없는 레포를 만들 수가 없는 구조**다. plan(`2 to add`)·apply(`Creation complete`) 로 정책이 함께 생성됨을 실측. Dockerfile 의 jar 선별 가드(`! -name '*-plain.jar'`)가 daily-api 에서도 **정확히 1개**를 잡는 것을 실행해 확인. `1-network` 자동 apply 를 보고 NAT 상시 과금($32/월)을 의심했으나 **의도적으로 없음**(`network.tf:15` 퍼블릭 서브넷으로 대체). **#400 C-4/D-009**: 노드 `t4g.small`→`t4g.medium`. 🔑 **핵심은 변수 한 줄이 아니라 근거 숫자 3개 중 2개를 처음 실측한 것**이다 — 08-28 감사에서 일지의 *"상한 17 · $0.13→$0.16"* 이 **근거 미기재**로 드러났었다. `describe-instance-types` 실측: medium ENI3×IPv4 6 → `3×(6-1)+2` = **17** ✅. **small 의 11 이 같은 조회로 재현돼 공식 자체가 검증된 뒤** medium 값을 얻었다(검산 가능한 것부터 검산). `pricing get-products`: medium **$0.0416/h**(small 의 정확히 2배) → 세션 총액 **$0.149/h**, **일지의 `$0.16` 은 $0.011 과다였다.** 컨트롤플레인 비중은 78%→**67%** 로 여전히 최대 — *"노드를 아끼자"* 보다 **"빨리 끄자"** 가 큰 레버. 🔴 **`Allocatable.memory` 는 여전히 미확인**(노드가 떠야 앎) — `variables.tf` 주석·계획서 표·일지 **세 곳**에 재측정 지시를 남겼다. 기각: 노드 2대(파드 슬롯만 풀고 **파드당 메모리 벽은 그대로**, 단일 AZ 핀이라 가용성 이득 0, 비용도 +$0.026 로 더 비쌈) | 2026-08-31 |
...
> ② ~~t4g.small → medium 상향~~ → 🔴 **폐기(D-010)** — 계정이 medium 을 launch 하지 못한다. 재개 1순위는 **노드 2대**.
