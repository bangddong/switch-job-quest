...
| 항목 | 내용 |
|------|------|
| 진행 중인 트랙 | **Phase 2 — `daily-service` 추출**. Stage A ✅ · B ✅ · 🔴 **Stage C 1차 시도 실패 — 원인은 계정 Free Tier 인스턴스 타입 제한 (08-31 유료 세션, $0.129).** `t4g.medium` 은 이 계정에서 **launch 자체가 안 된다**(ASG 5회 `Failed`; ⚠️ `describe-nodegroup` 의 `health.issues` 는 **빈 배열**이라 EKS 층에서 안 보인다 — 노드가 안 뜨면 **ASG 활동 로그를 먼저**). → **D-009 폐기 · D-010 신설 · C-4 재개봉.** 🔑 **레포 기록 0건이던 `Allocatable` 을 쟀다**: t4g.small = capacity 1841Mi / **allocatable 1365Mi** / pods 11. 시스템 파드가 406Mi 선점 → **가용 959Mi vs 필요 1792Mi = -833Mi**. **파드 슬롯은 제약이 아니었다**(6칸 여유). **앱 1개 + postgres 가 상한.** 🔴 **재개 1순위 = 노드 2대**(`-var node_desired_size=2`, `node_max_size` 는 이미 2). 이미지 재빌드 불필요, 비용 `2×small = 1×medium` **동일**. 2순위 x86(비용 2배 + arm64 상실). ⚠️ **착수 전 필수 2건**: ①**배치 강제**(`nodeSelector`/`podAntiAffinity`) — 용량 계산이 특정 배치를 전제하는데 지금 아무것도 강제하지 않는다(postgres nodeAffinity 는 **AZ 단위**). 스케줄러가 core-api+ai-api 를 한 노드에 얹으면 즉시 초과. 원장 **L-43** ②406Mi 의 **DaemonSet ÷ Deployment 분리 측정**(노드 B 여유 판정 근거). 🔑 미검증으로 남은 것: **C-5 NetworkPolicy 가 실제로 막는지**(클러스터가 떠야 함) |
