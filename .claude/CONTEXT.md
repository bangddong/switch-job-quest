...
| 항목 | 내용 |
|------|------|
| 진행 중인 트랙 | **Phase 2 — `daily-service` 추출**. Stage A ✅ · B ✅(#392·#393·#395) · **Stage C 준비 진행 중** — 감사 ✅(#396) · BE 블로커 ✅(#397 · C-1 헬스 + D-008 스텁) → **남은 것: C-3 `daily-api` ECR + `ecr-push.yml` · D-009 노드 t4g.medium · C-5 NetworkPolicy · 매니페스트 2벌 → apply/e2e/destroy**. 전부 $0 구간이고 **과금은 `tofu apply` 에서만 시작**한다. ⚠️ apply 전 `docs/eks-session-sop.md` 필독 + 사용자 승인 게이트. 학습 마일스톤이므로 브랜치는 `stage/eks-*`, **PR 전 이해도 퀴즈 필수**(`assert-eks-quiz.sh` 가 차단) |
