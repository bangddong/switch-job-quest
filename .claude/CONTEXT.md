...
| 항목 | 내용 |
|------|------|
| 진행 중인 트랙 | **Phase 2 — `daily-service` 추출**. Stage A ✅ · B ✅ · **C ✅ 완료**(09-04). 🔴 **지금 = 메모리 재산정 트랙**이고, **09-07 에 방법론이 뒤집혔다.** 09-06 유료 세션이 부하 실측을 냈지만(W_peak 401/212/363/63Mi → requests 1280 vs 현행 1792) **그 부하 테스트가 신뢰할 수준이 아니었다**(사용자 지적) — closed loop + 요청마다 프로세스 생성이라 **생성기가 병목이었을 수 있고**, 지연·CPU 를 안 재 **서버가 한계 근처였는지 알 수 없다**. 에러 0건은 잘 된 게 아니라 **포화를 못 시켰다는 신호**였다. → **W_peak·requests 1280·노드 2대(D-012)는 전부 ⚪약함으로 강등**, `k6` 오픈 루프로 재설계(계획서 §9). ✅ **k6 램프 유료 세션 완료 (09-08)** — **무릎을 못 찾았다: 1200 req/s 에서도 서버가 포화되지 않았다**(p95 17.5ms · 실패 0 · 스로틀 0). W_peak 실측 core 408 · daily 434 · ai 208 · pg 68Mi → requests **1792 → 1568Mi**(−224Mi). 🔑 **L-47 답**: 현행 1792 로도 2노드에 **들어간다**(여유 **5Mi**) → D-012 의 *"1280 으로 줄여야 2대"* 는 부분적으로 틀렸고, 재산정의 가치는 *가능하게 하는 것*이 아니라 ***안전하게 하는 것***. 🔴 **postgres 는 공식 제외** — 68Mi 는 26행짜리 빈 DB 값이라 *용량*이 아니라 *만진 페이지의 양*이고, `shared_buffers` 기본 128MB 라 limits 128Mi 면 버퍼 풀만으로 상한(= #410 의 JVM 하한 오적용의 **거울상**). 🔴 **§9.3 판정 규칙 결함 발견**: p95 가 부하와 함께 **내려간다**(JIT 워밍업) → 스테이지1 기준선은 콜드 스타트라 임계가 과도하게 관대했고 **한 번도 발동 안 했다**. ~~⚠️ 신규 requests 는 실환경 미검증~~ → ✅ **검증 완료 (09-09, #415)** — 2노드에 4파드 전부 배치·재시작 0·OOMKilling 0. **L-47 종결**. ⓒ 재현도 같은 세션에서 **조건부 통과**(결함 2건 수정). ~~🔴 다음 = Stage 4 (ALB Ingress)~~ → ✅ **완료 (09-11, #416)**. 🔴 **트랙이 바뀌었다 (09-11~13): 구축(Stage 0~4)은 끝났고 지금은 `prod → EKS 상시 운영` 을 향한 선행 조건 7건**(**D-013**, `docs/superpowers/plans/2026-09-11-prod-eks-migration-prereqs.md`). 계기는 *"이 템포로 실 서비스까지 얼마나 걸리나"* 였고 내 첫 답(**기술 2~3주, 월 $140**)이 **둘 다 틀렸다** — Blindspot Pass 22건 중 **6건이 "이 계획은 성립하지 않는다"** 수준이었다(전환 당일 **로그인 불가 + TLS 실패 + 메일 중복**). 실제 규모 **2~3개월**. 🔑 사용자 결정: **선행 조건부터 세션형으로** — 각 항목이 독립적으로 운영 경험이고 중간에 그만둬도 손실이 없다. **1 백업·복구 ✅(09-13)** → **2a 시크릿 환경 축 ✅(09-14)** — JWT 키를 `0-bootstrap` 으로, 다만 **L-14 패턴을 글자 그대로 복사하면 안 됐다**(앞선 세 적용은 단일 환경, JWT 는 처음으로 두 환경에 걸친다 → 키를 환경별로 분리). 🔴 **2b 는 재판정으로 해소됐다(09-14)** — `instance-id` 는 Basic auth 의 **username**(`Base64("$instanceId:$apiKey")` → `Authorization: Basic`)이라 시크릿이 아니고, *"학습/prod 메트릭 혼입"* 은 **#355(08-03)에서 끝난 사고를 09-11 계획서가 현재형으로 다시 적은 것**이었다. **`fly secrets set` 선행 요구는 사라졌다.** 남은 `fdaa::/16` 절 삭제는 **항목 3 으로 이관**(같은 `/actuator/**` 표면). → 🔴 **3 착수 (09-15). 계획서의 *"한 유료 세션"* 을 쪼갰다** — ACM 발급·Cloudflare 수동 검증·`fdaa` 삭제는 **클러스터가 필요 없다**($0). Cloudflare 전파 대기는 예측 불가라 컨트롤플레인 $0.100/h 가 도는 동안 하면 순손실. 유료분은 **Ingress annotation 3개 + 실 HTTPS + 2a 실검증**만. **ⓐ ACM 코드 ✅(#422)** → **ⓑ Cloudflare 수동 검증 ✅(09-16, `ISSUED`)** → **ⓒ `fdaa` 삭제 ✅(#424)** → **ⓓ 유료 세션 ✅(09-16, #426) → 선행 조건 3 완료.** 기술 목표 6/6 이지만 **비용이 21~24배 초과**됐다(예상 $0.13 → 실제 $2.77~3.14) — `destroy` 가 배경 전환돼 밤새 멈춰 **15.8h** 가 샜다. 🔑 쪼개기 판단은 옳았는데(ⓐⓑⓒ 전부 $0) **정작 유료 구간의 종료를 관리하지 못했다** — ***비용은 무엇을 켜느냐가 아니라 어떻게 끄느냐가 정한다.*** SOP §8b·§5 신설로 교정. 🔴 **남은 미검증 층**: Cloudflare **Proxied** 모드는 `--resolve` 로 원리적으로 못 잡고, `CORS_ALLOWED_ORIGINS` 기본값(U-11)도 curl 로는 안 잡힌다 — **둘 다 브라우저에서만 드러나므로 prod 전환 계획에서 함께 다룬다.** → 🔴 **다음 = 4 메타스페이스 누수 검증($0~0.1)** 호스트명은 prod `api.` 를 건드리지 않는 **학습 전용 서브도메인**(대가: ALB 가 세션마다 재생성돼 Cloudflare CNAME 수동 갱신 ~2분/세션). 인증서는 **`0-bootstrap`(영속)** — D-004·L-14 의 **5번째 적용**, 수동 검증을 세션마다 반복하지 않기 위해. 🔴 **`aws_acm_certificate_validation` 은 넣지 않는다** — `infra-deploy.yml` 이 0-bootstrap 을 main 머지 시 auto-apply 하므로 블로킹 리소스가 CI 를 멈춘다 → 5 실 AI·메일 경로 → 6 EKS CD → 7 상시 운영 전제로 안전장치 개정(**ACM 갱신 자격 항목 추가**). ⚠️ **Stage 5(HPA·ArgoCD)는 README 가 "선택"** 이고, 블로그 원고($0)는 여전히 남아 있다. 그전에 무료로 `aws budgets describe-budgets` 조회해 산정↔실측 대조할 것(F-8 로 미뤄둔 것). ~~다음 착수 지점 = k6 램프 유료 세션~~ — ①25/50/100/200/400 회전/s(**실제 req/s 는 ×3**) 스테이지별 실행 ②`dropped_iterations>0` 이면 **생성기 병목이라 그 회차는 버린다** ③무릎에서의 메모리가 진짜 W_peak ④**첫 15분에 노드 2대 검증**(원장 L-47 — 절차 확정돼 있어 질문 없이 실행만). ✅ **선행 $0 준비 완료(#413)**: 생성기 격리가 없으면 k6 가 측정 대상과 같은 노드에 떠 그 회차를 통째로 버려야 한다 → `sh k8s/loadtest/pin-loadgen-node.sh` 를 **k6 apply 전에** 돌린다(계획서 §9.7). ⚠️ **노드는 3대**(앱 2 + 생성기 1) — `node_desired_size=2` 로 내리면 생성기 노드가 없다. 🔑 **부하 품질과 무관하게 단단한 수확 3건**: nonheap 183.4 > heap 139.3Mi · **Metaspace 125.6Mi 가 최대 소비자** · SerialGC 는 OS 에 반환 안 함 → ***"`limits` 를 조여도 조여지는 건 힙뿐"*** (힙은 상한 316Mi 중 **70Mi 만** 사용). |
...
> 크레딧 **누적 소진 $2.936/$200** (1.47%, 2026-09-15 `aws budgets describe-budgets` 실측; CE 서비스별 합계 $2.9177 로 독립 일치). ~~$0.481 (07-31)~~ — **이 줄이 6주간 썩어 있었다.**
> 🔑 **대조가 준 것은 숫자가 아니라 비중이다**: 세션 컴퓨트는 누적의 20% 남짓이고 나머지는 **켜둔 채 잊은 것**(EBS·ECR $1.56)과 **재는 것**(Cost Explorer $0.190 = 6.5%, `$0.01/요청`)이다. → *세션 최적화의 한계수익이 낮다.* 상세·근거는 `docs/eks-migration-log.md` 「산정 ↔ 실측 대조」.
>
...
| PR/커밋 | 내용 | 날짜 |
|---------|------|------|
| **#426** | **ACM HTTPS 종료 실검증 — 선행 조건 3 완료. 기술 목표 6/6, 비용 21~24배 초과.** **실제 HTTPS 성립**: `--resolve` 로 DNS 레코드 없이 `-k` 없이 통과 (`CN=eks.quest.dhbang.co.kr` · Amazon RSA 2048 M04 · TLSv1.2 · h2). **ALB IP 2개 모두** 동일(200/403/**404**/404/404)이라 n=1 판정이 아니다. 성립 조건 둘: ⓐ`ingress.yaml` `rules:` 에 **`host:` 0건** ⓑ인증서는 리스너 고정 부착. ⚠️ **남는 것 — Cloudflare Proxied 는 원리적으로 못 잡는다**(엣지를 건너뛰므로). U-11(CORS 가 브라우저에서만 드러남)과 같은 형태. 🔑 ***`--resolve` 는 DNS 를 속이고 `-k` 는 검증을 끈다 — 앞은 판정을 유지하고 뒤는 파괴한다.*** 🔴 **비용 사고: 예상 $0.13 → 실제 $2.77~3.14.** `destroy` 가 도구 타임아웃으로 **배경 전환**돼 밤새 멈췄고 노드가 다음 날 09:12 에 종료됐다(**15.8h · ~$2.5**). 🔑 **SOP 는 §8② 에서 ALB 에 대해 이미 *"삭제 명령을 친 것과 사라진 것은 다르다"* 를 적어뒀는데 destroy 자체에는 안 적어뒀다** — 같은 문서에서 한쪽만 엄격했다. **리퍼는 구조적으로 못 막는다**: launchd 는 머신이 자면 안 돌고, 감지 대상이 *"사람이 사라졌다"*(하트비트 2h stale)인데 당시 하트비트는 신선했다 — **멈춘 destroy 는 건강한 세션과 똑같이 생겼다.** → **SOP §8b**(완료 게이트: `describe-instances`·`list-clusters` 가 **0 인 것을 AWS 에 직접 물어** 확인) · **§5**(apply·destroy 전경 실행 + 마커 육안 확인) 신설. ***이게 $2.8 로 산 것이다.*** 🔴 **`tofu apply` 가 세션 마커를 안 만들었다**(원장 L-58, MEDIUM) — 정규식은 매칭되는데 마커가 없었다(배경 실행 정황, 확도 🟡). 수동 무장으로 우회. **두 방어선이 동시에 비어 있었다.** 🔴 **항목 2a 검증 4종이 전부 동어반복이어서 재설계했다** — `environment` 기본값이 `learning` 이고 tfvars 0건이라 이름 확인은 기본값 확인이고, *"teardown 후 키 유지"* 는 키가 `0-bootstrap` `random_password` 라 2-cluster state 밖이므로 13분(≈$0.045)의 반증력이 **0**. → **지문 비교**(파드 `JWT_SECRET` = learning 키, prod 와 다름 = 4단계 체인 한 번에 판정) + **`simulate-principal-policy`**. 🔴 **그 IAM 검증도 첫 시도는 판정력 0 이었다** — 가짜 suffix 로 `learning` 까지 거부됐다(정책 `resources` 가 와일드카드 없는 정확한 ARN 목록이라 suffix 불일치만으로 막힌다). **항목 1 센티넬 문제 3번째 재발**이고 고친 방법도 같다 — **변인 통제**(suffix·리전·계정 고정, **이름만** 교체 → `allowed` ↔ `implicitDeny`). 🔴 **ACM 갱신 자격이 되돌아온다**(`ELIGIBLE`/`InUseBy:1` → teardown 후 `INELIGIBLE`/`0`) — destroy-after-use 에선 **거의 항상 `INELIGIBLE`** 이라 *"붙이면 갱신된다"* 가 위안이 못 된다. 갱신 시점(≈2027-02-01)에 클러스터가 꺼져 있으면 시도조차 안 되고, **크레딧 만료(2027-01-15)가 그보다 앞이라 "트랙이 그때 끝난다"를 아무도 보장하지 않았다** → 선행 조건 7. 🔴 **예측이 틀렸다 — `curl -L` 금지 경고는 오류**였다(LBC 가 **Host 헤더를 보존**한다). 그 경고는 Blindspot 의 **⚪ 추측**이었는데 문서로 옮기며 **🔴 로 격상**됐다. ***확도 표시를 함께 옮기지 않으면 추측이 한 번의 복사로 사실이 된다.*** ⚠️ **같은 형태로 이 세션에 세 번 틀렸다**(리퍼 설치 경로 오정정 · `-L` 경고 · `core-api 512Mi`를 기억으로) — **전부 확인할 수 있었는데 안 한 것**이고, 공교롭게 퀴즈 주제가 *"이 관측이 무엇을 배제하는가"* 였다. **U-8·U-13·U-14·U-15·U-17 전건 해소**(U-14 는 *"3곳"* → 2곳이고 퀴즈 파일은 **트랜스크립트라 수정 금지**, U-17 은 ALB DNS 앞부분이 **#416 과 동일**해 서술이 절반 틀렸음도 기록). **원장 L-54 종결** — 동결 규칙에 **해제 조건 ②(사용자가 먼저 꺼낸 경우)** 신설. L-54 가 스스로 경고한 위험(*"사용자 요청은 내가 유도할 수 있다"*)을 **조문 첫 줄**에 넣어 유도 승인은 해제 사유가 아님을 명시. **원장 중복 ID**(L-9·L-10)는 재채번을 **기각**(교차 참조 10곳 중 5곳이 append-only 일지 → 소급 변경은 기록 위조). QA 3라운드(HIGH0·MED0·LOW6 전건 처리) — **F-4 는 반증했다**(2앱이어도 ESO 3파드+LBC 가 얹혀 메모리 1344Mi·파드 12개로 **1노드 불가**, QA 재검증 후 wontfix). 퀴즈 9문제 — **1차 서술형 전멸 → 재확인 3/3** | 2026-09-17 |
| **#425** | **ACM `ISSUED` — 선행 조건 3 의 $0 구간 완료 (Cloudflare CNAME 입력 후 3분).** `ValidationStatus: SUCCESS` · 만료 2027-04-02 · 비용 $0 · 72h 마감 대비 51시간 여유. TASK-10 종결. 🔴 **예상 못 한 값 — `RenewalEligibility: INELIGIBLE` · `InUseBy: []`**: **ACM 자동 갱신은 인증서가 AWS 리소스에 연결돼 있을 때만 동작한다.** ALB 에 붙는 순간 `ELIGIBLE` 로 바뀐다. 이 트랙엔 무해하지만(만료 2027-04-02 > 크레딧 만료 2027-01-15) ***상시 운영에선 "안 쓰는 인증서는 갱신되지 않는다"가 함정*** → 선행 조건 7 재검토 대상으로 등재. 🔑 Stage 3b 의 *"관리형이 공짜로 주던 것에 TLS 가 있었다"* 와 같은 형태인데, 이번엔 **공짜가 아니라 조건부**로 주던 것이다. 💰 **항목 3 을 쪼갠 판단이 숫자로 확인됐다** — ⓑ가 사용자 손을 타고 09-15→09-16 으로 넘어갔다. 컨트롤플레인을 켜둔 채였다면 **19시간 ≈ $1.9 = 트랙 누적 $2.94 의 65%**, 실제 지출 **$0**. ***사람 손이 들어가는 단계는 과금 창 밖으로 뺀다.*** 🔴 **원장 L-55 — 경로를 지어내고 prod 장애로 오진, 롤백 권고 직전까지 갔다.** #424 배포 후 수용 검증에서 `/daily-question/today`·`/tech-interview/questions`·`/auth/github/url` 을 쳤는데 **셋 다 실재하지 않는다** → `NoResourceFoundException` 은 **버그가 아니라 정답**이었다. ***롤백은 무해한 기본값이 아니다 — 건강한 배포를 되돌리면 그 자체가 장애다.*** 오진 비용은 **비대칭**이다. 🔑 **레포에 답이 있었다**: `prod-smoke-daily.yml` 이 실제 경로를 매일 치며 **6일 연속 성공** 중이었다. 🔴 **이 세션 세 번째 *검사 도구가 먼저 틀렸다*** (①zsh 파라미터 수식자 ②macOS sed 로 ubuntu 추정 ③경로 날조) — 앞의 둘과 달리 ③은 **prod 에 닿았다.** ⚠️ **왜 actuator 검증은 멀쩡했나**: 그 경로는 **방금 읽은 코드**에서, API 경로는 **기억**에서 나왔다 — ***한 세션 안에서 출처가 코드인 주장과 기억인 주장이 섞였고 나는 구분하지 않았다.*** 관례 2개: **인터페이스를 기억에서 쓰지 않는다**(ⓐ그것을 호출하는 테스트·스모크 → ⓑ선언부 순. 선언부는 맞아도 **조합**이 틀릴 수 있다) · **장애를 의심할 때 마지막으로 성공한 관측을 먼저 찾는다** | 2026-09-16 |
| **#424** | **`/actuator/**` 에서 Fly 6PN(`fdaa::/16`) 절 삭제 — 항목 3 의 마지막 $0 조각 (2b 에서 이관).** 프로덕션 변경은 **1 file · +2 −1** 이 전부. 🔑 **실제 값어치는 테스트다.** 🔴 **기존 회귀 가드의 판정력이 0이었다**(Blindspot U-2): `remoteAddr("203.0.113.5")` 는 세 절 어디에도 안 걸려 **삭제 전후 모두 403** 이고 **세 절을 전부 지워도 초록**이었다. → **TDD 의 RED 가 전제를 증명**했다 (`fdaa:0:1::3` → 403 테스트가 **수정 전 코드에서 실패** = 구 코드가 fdaa 를 실제로 허용했다는 증거). 🔴 **고친 스위트가 반대 방향으로 또 판정력 0**(QA F-1) — 전부 *"거부돼야 할 것이 거부되는가"* 만 봐서 **남은 두 절까지 지워도 통과**한다 → positive 2개 추가. positive 는 그냥 통과하므로 **뮤테이션**(`denyAll`)으로 RED 를 만들었다. 🔴 **그 뮤테이션도 증명이 약했다**(F-2, 내가 올린 지적) — *"뭐라도 허용하면 통과"* 만 보이고 *"각 절이 각자 필요"* 는 못 본다. QA 는 *"boolean OR + 비중첩이라 **논리적으로** 보장"* 이라 했지만 이 트랙의 교훈은 ***논리적 보장 ≠ 실행 증거***(#416) → **프로덕션을 안 건드리고 `IpAddressMatcher` 를 jshell 로 직접 돌려** 3×3 매트릭스로 확정(대각선만 true = 상호 비중첩 = 각 테스트가 서로 다른 절을 지킨다). QA 가 독립 재현. 🔑 ***"논리적으로 그럴 수밖에 없다" 를 30초짜리 실행으로 바꿀 수 있으면 바꾼다.*** ⚠️ **뮤테이션 원상복구를 오케스트레이터가 독립 검증**했다(`git status` 공백 · 프로덕션 diff `+2 −1` · `grep denyAll` 0건) — **남은 채 머지되면 prod 접근 제어가 깨진다**(머지 시 `be-cd` → Fly 자동 배포). 근거: 소비처 전수 **0건** + prod 실측 **403**(인터넷→`/actuator/prometheus`) → 삭제는 **증명 가능하게 비확대**. ⚠️ **EKS CIDR 로 교체 금지** — Ingress 소스가 ALB 라 교체하면 인터넷에 열린다 | 2026-09-15 |
...
- [x] ~~📌 **`application-prod.yml` 의 `grafana.otlp.instance-id: "1680166"` 평문** (퍼블릭 레포, #355 범위 밖)~~
      → 🔴 **재판정 완료 (2026-09-14): 유출이 아니다. 항목을 닫는다.**
  - **이 항목의 서술은 처음부터 정확했다** — *"시크릿은 아니고 스택 식별자다. 키가 없으면 push 자체가
    안 돌아 당장의 위험은 무해화됐다(#355)"*. **틀린 것은 계획서 B-15**(09-11)이고, 그것을 읽은
    **내 요약**이었다(*"퍼블릭 레포 평문 커밋, 미해결 등재"* — 하위 한정을 떨어뜨렸다. 원장 L-53).
  - 미뤄뒀던 판정을 실제로 했다: `OtlpMetricsConfig` 가 `Base64("$instanceId:$apiKey")` 를
    `Authorization: Basic` 에 싣는다 → **instance-id 는 username, `GRAFANA_API_KEY` 가 password.**
    아이디만으로는 아무것도 못 한다. 값은 레포에 0건(`git grep`), 학습 클러스터 주입 0건, K8s 주입 0건.
  - **환경변수화는 안 한다 — 하는 쪽이 더 위험하다.** Fly prod 가 같은 `prod` 프로파일을 읽으므로
    주입을 놓치면 `enabled: true` 인 채 레지스트리만 없는 상태가 되고 `/health` 로 안 잡힌다.
    ~~Fly 시크릿을 사용자가 직접 추가~~ → **불필요해졌다**(사용자가 `fly` 를 쓸 수 없는 환경인 것과 무관).
