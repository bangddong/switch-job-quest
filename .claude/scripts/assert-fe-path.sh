#!/usr/bin/env bash
# fe-feature-builder 전용: be/ 경로 쓰기 차단
...
# ── 이 훅은 fe-feature-builder 전용이다 (2026-08-15 추가) ──
...
if [ -n "$AGENT_ID" ] && [ -n "$AGENT_TYPE" ] && [ "$AGENT_TYPE" != "fe-feature-builder" ]; then
  exit 0
fi

if echo "$FILE_PATH" | grep -qE "(^|/)be/"; then
  echo "차단: fe-feature-builder는 be/ 경로를 수정할 수 없습니다. (${FILE_PATH})" >&2
  exit 2
fi
