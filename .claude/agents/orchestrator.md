...
> **브랜치 규칙**: `.claude/` 파일 수정 포함 **추적되는 모든 파일 수정**은 작업 브랜치에서 진행한다.
> `chore/`, `docs/` 작업도 동일. main 직접 커밋 금지.
> 유일한 예외는 **gitignore되는 런타임 파일**(`qa-cache/`·`logs/`·`scratch/`·`review-cache/` 등)이다 —
> 커밋되지 않으니 리뷰 대상이 애초에 아니다. `assert-not-main.sh`가 `git check-ignore`로 판정한다.
> ⚠️ 2026-08-12 이전엔 `.claude/` **전체**가 면제였다(#41). 그래서 **훅 스크립트·에이전트 정의·
> `settings.json`을 main에서 리뷰 없이 고칠 수 있었고**, 훅은 워킹트리에서 실행되므로 그 변경이
> 즉시 반영됐다 — 가드를 끄는 게 한 단계였다. 되살리지 말 것.
...
```bash
bash .claude/scripts/session-status.sh   # 브랜치·PR·원장·EKS 과금 상태 (실시간 조회)
cat .claude/CONTEXT.md                   # 결정·다음 작업 (파생 불가능한 것)
```

- 열린 PR 있으면 → 이어서 진행 (`gh pr view <번호>` 확인)
- 새 작업이면 → 아래 워크플로우

> ⚠️ **PR 상태를 CONTEXT.md에서 읽지 마라.** 거기 없다. `session-status.sh`가 `gh`로 직접 조회한다.
> 예전엔 CONTEXT에 적어뒀는데 자기참조라 구조적으로 썩었다 — 상세는 CONTEXT "현재 상태" 절.
...
🔴 **브랜치명·PR 번호를 CONTEXT.md에 적지 마라.** `git`·`gh`가 이미 아는 것을 문서에 복사하면
반드시 어긋난다 — 특히 PR 상태는 **자기참조**라 원리적으로 맞출 수 없었다(PR은 자기 머지 사실을
담을 수 없다). 이 규칙이 있던 동안 CONTEXT 수정만이 목적인 PR이 24건 쌓였다.

대신 `## 현재 상태`의 **`진행 중인 트랙`** 한 줄만 본다. 새 트랙을 시작할 때만 갱신한다
(예: `EKS Stage 3b` → `Phase 2 daily-service 추출`). **PR 하나 단위로는 건드리지 않는다** —
트랙은 여러 PR에 걸쳐 살아 있고, 그래서 머지로 무효화되지 않는다.
...
### 🔴 이번 PR 에서 처리하지 않는 발견은 **계획서에 등재한다**

Blindspot 은 매번 15~22건을 내는데 그중 상당수가 *"맞는 말이지만 이번 범위는 아니다"* 다.
**그것들의 목적지가 정해져 있지 않으면 유실된다.**

| 산출물 | 목적지 |
|---|---|
| 이번에 처리 — 결정·에러·해결 | 작업 일지 (EKS 트랙이면 `docs/eks-migration-log.md`) |
| **이번에 안 함 — 나중에 할 일** | **계획서의 해당 항목 절** ← 이것 |
| 미룬 **QA** 지적 | `.claude/review-ledger.md` |

- **원장에 넣지 마라.** 원장은 QA 지적 전용이고, `assert-qa-run.sh` 가 `<브랜치>/<F-ID>` 형식으로
  등재를 검사한다. Blindspot 발견은 그 형식이 없어 검사에도 안 걸리고 섞이기만 한다.
- **일지로 충분하다고 여기지 마라.** 일지는 시간순 기록이라 *"다음에 뭘 해야 하나"* 로 **검색되지 않는다.**
- 🔴 **실제 유실 사례 (2026-09-15)**: 항목 3 Blindspot 17건 중 **U-8 하나만 일지에서 빠졌고**
  아무도 몰랐다. 같은 세션 QA 가 독립적으로 **F-2**(원본 계획 문서 미갱신)로 같은 구멍을 지적했다.
  두 경로가 같은 곳을 가리켰다 = 개인 실수가 아니라 **구조적 구멍**.
- 등재할 때 **왜 지금 못 하는지**를 함께 적는다. 근거 없이 미룬 항목은 다음 세션에 다시 판단해야 한다.

> ⚖️ **이 절을 만든 것 자체가 하네스 동결 규칙과 경계선상이다 (QA F-7, 2026-09-15).**
> 규칙은 *"제품 작업이 **막혔다**는 증거가 있을 때만"* 인데, 근거였던 U-8 유실은 *"막힘"* 이 아니라
> *"기록 유실"* 이고 같은 세션 QA 가 독립 검출해 **실제로는 자기수복됐다**. 규칙 문언에 딱
> 들어맞지 않는다. **사용자 명시 요청으로 진행했고, 스크립트·훅 없이 문서 한 절로 끝냈다.**
> 🔑 다음에 이 근처를 건드릴 사람에게: **이 절은 "증거가 충분했다"는 선례가 아니다.**
...
> 🔴 **`브랜치`·`열린 PR` 셀은 없다. 되살리지 마라.** `gh pr list`가 언제나 정확한 답을 준다.
> 문서에 복사하면 머지 순간 거짓이 되고, 그걸 고치려고 또 PR을 만들게 된다(24건의 전례).
> "머지 대기"라고 적는 순간 그 문장은 **곧 틀릴 것이 확정**된 문장이다.
...
### EKS 학습 마일스톤 (`stage/eks-*`) — **필수, 예외 없음**
브랜치가 `stage/eks-*`이면 퀴즈는 제안이 아니라 **PR 생성 전 필수**다.
- 구축·검증(apply 왕복 등)이 끝나면 **`gh pr create` 전에** `skills/universal/quiz.md`의
  "EKS 학습 마일스톤 모드"로 퀴즈를 진행한다. 재료는 `docs/eks-migration-log.md` 실측.
- 결과를 `docs/eks-quizzes/<브랜치를 -로>.md`에 기록하고 통과 시 `<!-- QUIZ-PASSED -->` 마커를 남긴다.
- **`assert-eks-quiz.sh` 훅이 이 파일·마커가 없으면 PR을 차단**한다 — 건너뛸 수 없다.
- 이건 "명령만 던지고 이해는 건너뛰는" 실패(Task 8에서 실제 발생)에 대한 기계적 교정이다.
  퀴즈를 피하려고 브랜치를 `chore/`로 바꾸지 말 것 — 그건 학습 자체를 우회하는 것이다.
