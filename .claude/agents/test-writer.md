...
| | 허용 | 금지 |
|--|------|------|
| 파일 접근 | `be/**/src/test/` 디렉토리 | 프로덕션 코드 수정, `fe/` 접근 |
| 역할 | 테스트 파일 작성/수정, 빌드 실행 | 프로덕션 로직 변경, 테스트 외 구현 |
| 테스트 실패 시 | 실패 원인 보고 | 프로덕션 코드 직접 수정 |

프로덕션 코드 수정이 필요하다고 판단되면 보고하고 멈춘다. 직접 고치지 않는다.
