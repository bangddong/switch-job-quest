...
| L-9 | `stage/eks-3a-postgres-dynamic-pvc/F-2` | MEDIUM | **`db_mode=rds` 경로가 실제 apply로 검증되지 않았다.** `one(aws_db_instance.main[*].master_user_secret[0].secret_arn)` 등 splat+중첩인덱스 표현식이 rds 모드에서 런타임에 올바른 ARN을 반환하는지 미확인. D-001이 `rds.tf`를 남겨둔 근거가 "Stage 2 재현성"이므로 이 경로가 실제로 죽어 있으면 결정의 전제가 무너진다 | **plan 단계까지는 QA 후 실측으로 해소했다**(07-30, $0): `tofu plan -var db_mode=rds` → `db_address`·`db_master_secret_arn`이 `known after apply`로 출력에 나타나고(= `one()`이 count=1에서 값 반환), `db_connection_rds[0]`·RDS 4종이 생성 목록에 복귀, `postgres_tls`·`random_password.postgres`는 제외됨. **남은 미검증 = 실제 apply**(RDS 생성 + ESO가 AWS 소유 `rds!db-<uuid>` 시크릿을 읽는 부분). 검증에 유료 세션 1회(~40분/$0.09)가 필요해 이번 PR에 넣지 않는다. **처리 시점: Stage 3b 세션 착수 시 첫 apply를 `-var db_mode=rds`로 한 번 돌려 확인** |
