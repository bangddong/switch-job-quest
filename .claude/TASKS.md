...
> 🔴 **먼저 할 것 — 만료일이 문서마다 다르다 (07-29 발견, 미해결).**
> 콘솔 **Billing → Credits**에서 실제 만료일을 확인해야 아래 캘린더 등록이 의미가 있다.
>
> | 날짜 | 어디에 적혀 있나 | 근거 성격 |
> |---|---|---|
> | **2027-01-15** | 튜토리얼 · `eks-session-sop.md` · 이 파일 · 일지 최상단 요약 | "계정 API 실측" |
> | **2027-07-15** | `infra/aws-eks/README.md` · 일지 07-16 원본 항목 | "가입 +1년" 도출 |
>
> **일지 내부에서도 어긋난다** — 07-16 원본 기록은 07-15인데 최상단 요약은 01-15다.
> CLI로는 조회 불가(`aws freetier`는 프리티어 사용량 전용, 크레딧 목록 API 없음).
> **추측으로 어느 한쪽을 지우지 말 것.** 확인 후 틀린 쪽을 전부 일괄 정정한다(위 표가 그 목록).

- **만료일**: **2027-01-15로 잠정 표기** (⚠️ 위 모순 미해결 — 확인 전까지 신뢰하지 말 것)
- **알림 권장**: 확정된 만료일 2주 전 — 상시 데모 계획이 있으면 이때 Paid 업그레이드 판단
