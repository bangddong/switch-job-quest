...
### TASK-10: Stage A prod 스모크 — **자동화됨. 사용자 조치 불필요** (2026-08-20)

> ✅ **사람이 09:00 전에 붙어 있을 필요가 없다.**
> `.github/workflows/prod-smoke-daily.yml` 이 매일 **05:23 KST**(20:23 UTC)에 자동 실행한다.
> <!-- verify: .github/workflows/prod-smoke-daily.yml ~ daily-question -->
>
> **자동화한 이유**: 08-18·19·20 **사흘 연속 창을 놓쳤다.** 관찰 창이 00:00~09:00 뿐인데
> 그 시간에 작업하지 않기 때문이다. 사람 규율에 맡기면 계속 이럴 것이 실측으로 증명됐다.
> (09:00 이후엔 `DailyMailScheduler` 가 이미 오늘 행을 만들어 **lazy 경로를 안 탄다** —
>  그 시각의 200 은 #387 덕분인지 알 수 없다)
>
> **결과 보는 법**: Actions 탭 → `Prod Smoke (daily-question)`.
> 빨강이면 로그에 확인 순서가 적혀 있다(배포 상태 → 뱅크 소진 여부 → 회귀 의심).
>
> **통과 후**: 계획서 진행 상황 표의 Stage A 를 `✅ 완료` 로 바꾸고 이 TASK 를 지운다.
> ⚠️ **워크플로는 남긴다** — 일회성 확인이 아니라 **상시 회귀 가드**다.

> 🔴 **기록 정정 (2026-08-20).** 머지된 `4582783`(#387) 커밋 본문에 *"TASKS.md TASK-10 신설"*
> 이라고 적혀 있으나 **그 커밋은 `.claude/TASKS.md` 를 건드리지 않았다.** 이 항목은 오늘 처음 생겼다.
>
> 원인: #387 당시 TASKS.md 를 고치는 python 과 `git commit` 을 **한 명령으로 묶었는데** 그게
> push 가드 오탐(L-29)에 막혔다. **PreToolUse 는 all-or-nothing 이라 python 부분도 안 돌았다.**
> 가드를 고친 뒤 **같은 커밋 메시지를 재사용**했고, 메시지의 주장을 검증하지 않았다.
> 관련 원장: **L-32**(가드 12개 중 1개만 all-or-nothing 경고를 갖고 있다).
