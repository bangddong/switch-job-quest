...
/**
 * core-api `DailyQuestionServiceTest`와 동일한 동작을 daily-api 자체 서비스에 대해 검증한다
 * (D-007 — core-api의 `DailyQuestionService`를 재사용할 수 없어 daily-api가 자체 조립).
 */
@ExtendWith(MockitoExtension::class)
class DailyQuestionApiServiceTest {
    ...
    @InjectMocks
    private lateinit var service: DailyQuestionApiService
    ...
    @Test
    fun `getTodayQuestion - 오늘 행이 없어도 뱅크에 후보가 있으면 뱅크에서 생성해 반환한다`() {
    ...
    @Test
    fun `getTodayQuestion - 오늘 행이 없고 뱅크도 소진이면 DailyQuestionNotFoundException을 던진다`() {
        whenever(dailyQuestionContentPort.findToday(eq("TECH_INTERVIEW"), any())).thenReturn(null)
        ...
        assertThatThrownBy { service.getTodayQuestion() }
            .isInstanceOf(DailyQuestionNotFoundException::class.java)
