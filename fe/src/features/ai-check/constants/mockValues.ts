...
export const MOCK_INTERVIEW_SAMPLE_ANSWERS: Record<string, string> = {
  ...
  q8: 'hashCode()는 객체를 해시 기반 컬렉션(HashMap, HashSet)에서 빠르게 탐색하기 위한 버킷 인덱스를 결정합니다. equals()는 동등성을 비교합니다. 두 메서드는 반드시 함께 재정의해야 하며, equals()가 true면 hashCode()도 같아야 합니다. 반대는 성립하지 않아도 됩니다. hashCode 구현 시 Objects.hash()를 사용하거나 핵심 필드를 31을 곱해 조합합니다. 롬복의 @EqualsAndHashCode 사용 시 JPA 엔티티에서 연관관계 필드 포함 여부를 주의해야 합니다.',
